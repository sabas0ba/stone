...
int gllm[2048];           ///< 仮引数の 64 bit 位置のビット表 (bit k = 語 k から
...
int lsg[256];             ///< 関数内 static: 実体の大域記号番号 (-1 = 普通の局所)
...
int mty[2048];            ///< メンバの型
int mbfw[2048];           ///< ビットフィールドの幅 (0 = 普通のメンバ)
int mbfo[2048];           ///< ビットフィールドのビット位置 (語内)
...
int ep;                   ///< eb への書込み位置
// 再配置の表は 16384 件 (cc15g で 8192 から倍にした。cc15f 自身の
// ソースが 8000 件を超え，次の世代のソースが入りきらなくなったため。
// cc14f が IR の配列を倍にしたのと同じ形の拡張である)
int rof[16384];           ///< 再配置: .text 内オフセット
int rsy[16384];           ///< 再配置: 対象シンボル (上記の符号化)
int rty[16384];           ///< 再配置: 種別 (R_RISCV_*)
int rad[16384];           ///< 再配置: 加数
int rcnt;                 ///< 再配置の件数
...
int bfbit;                ///< strudef: その語の次の空きビット                  ///< earr が 1 のときの配列全体のバイト数
...
int iop[16384];            ///< 命令種別 (c_* のいずれか。c_bin 以上は二項演算)
int ia[16384];             ///< 第 1 オペランド (意味は命令種別による。上表参照)
int ib[16384];             ///< 第 2 オペランド (同上)
int icnt;                 ///< 命令数

int lastu[16384];          ///< 値が最後に使われる命令位置。-1 = 一度も使われない
int vreg[16384];           ///< 割付け結果: >= 0 レジスタ番号 / -1 未割付 / -2-n スピルスロット n
int live[16384];           ///< dce の結果: 1 = 生存 (出力する)
int iret[16384];           ///< CALL の側情報: 構造体を返す呼出しの引取り先
                          ///< (フレームオフセット。0 = 構造体を返さない)

int labpos[2048];         ///< ラベル番号 -> 出力オフセット。-1 = 未確定 (まだ現れていない)
int labcnt;               ///< ラベル数
int lfix[4096];           ///< 前方ラベル参照の後埋め: 命令を書いた出力位置
int lflab[4096];          ///< 同上: 参照先のラベル番号
int lfixn;                ///< 後埋め待ちの件数
...
/// @return 常に 0
int lexnum() {
  int nl; int d; int ov; int hi2; int lo2; int nv;
  tval = 0;
  tvalhi = 0;
  tvalll = 0;
  ...
  if (getch() == '0' && src[pos + 1] == 'x') {
    ...
    while (isdig(getch())) {
      d = getch() - '0';
      // 10 倍を上位語込みで行い，そのあと 1 桁足す。**足す側の桁上げも
      // 上位語へ運ぶ**こと (2^32 のような値はここでしか桁が上がらない)
      hi2 = tvalhi * 10 + mulhi10(tval);
      lo2 = tval * 10;
      nv = lo2 + d;
      if (ultc(nv, lo2)) hi2 = hi2 + 1;
      if (hi2) ov = 1;
      tvalhi = hi2;
      tval = nv;
      adv();
...
int lexchr() {
  adv();
  // 64 bit の印は数値リテラルが立てる。文字リテラルでは必ず消す
  // (消し忘れると直前の 1LL などの印が残り，':' が 64 bit に見える)
  tvalhi = 0;
  tvalll = 0;
  if (getch() == eot) exit(1);
...
int nwords(int t) {
  ...
  if (isll(t)) return 2;       // 下位語・上位語の 2 語 (docs/stage015-tcc.md 6.2 の項目 11)
  return 1;
}
...
int rvany(int v) {
  ...
  if (elv) {
    ...
    if (isll(ety)) {
      // 64 bit は下位語・上位語の 2 語。上位語は アドレス + 4 にある
...
int ecallseq(int e) {
  ...
  if (gvar[e]) {
    ...
    while (i > 0) {
      ...
      // 可変部は語数と個数がずれると並べ替えられない。64 bit と構造体は
      // 名前つきの側にしか置けない
      if (aw[i] != 1) exit(5);
      emit(c_arg, av[i], 0);
    }
  ...
  if (isll(gty[e])) {
    // 64 bit の返却。下位語は呼出しの値そのもの，上位語はその 1 つ上に
...
int eadd() {
  int v; int r; int op; int lt; int lh; int ah; int bh;
  v = emul();
  while (tok == o_add || tok == o_sub) {
    ...
    if (isll(lt) || isll(ety)) {
      ...
      // ポインタとの組合せは未対応 (拡大の規則が別に要る)
      if ((lt >> 16) != 0 || (ety >> 16) != 0) exit(5);
      ah = wide1(v, lh, lt);
      bh = wide1(r, ehi, ety);
...
int eshift() {
  ...
  while (tok == o_shl || tok == o_shr) {
    ...
    lh = ehi;
    next();
    // 桁数は 32 bit である (64 bit の桁数は意味が無いので拒む)
    r = rv(eadd());
    if (isll(lt)) {
...
int assign() {
  ...
  if (tok == o_asn) {
    ...
    if (isstru(t)) {
      ...
      // 64 bit は 2 語書く。右辺が 32 bit なら符号 (あるいは 0) で伸ばす
      if (!isll(ety)) ehi = ext32(r, ety);
      emit(c_stw, v, r);
...
int stmt() {
  ...
  if (tok == k_return) {
    ...
    if (tok == o_semi) emit(c_ret, emit(c_const, 0, 0), 0);
      ...
      c = rvany(expr());
      if (isll(cretty)) {
        // 64 bit の返却。上位語を先に積むと，下位語 (c_ret が積む) の
        // 1 つ上に来る
        if (!isll(ety)) ehi = ext32(c, ety);
        emit(c_arg, ehi, 0);
      } else if (ehi) {
...
int emitins(int i) {
  ...
  if (k == c_call) {
    ...
    a = 4;
    if (iret[i] && isll(gty[b])) {
      // 64 bit の返却。上位語が 1 語だけ 1 つ上に積まれている
...
int memb(int se, int t, int fp) {
  ...
  if (!fp) {
    ...
    if (tok == o_col) {
      ...
      if (sunion[se]) {
        moff[m] = 0;
        mbfo[m] = 0;
        if (ssize[se] < 4) ssize[se] = 4;
      } else if (bfword >= 0 && bfbit + mbfw[m] <= 32) {
        moff[m] = bfword;
        mbfo[m] = bfbit;
        bfbit = bfbit + mbfw[m];
      } else {
  ...
  else { marr[m] = 0; sz = tsize(t); if (sz > 4 && !isstru(t) && !isll(t)) sz = 4; }
  msz[m] = sz;
  if (sunion[se]) {
...
int pparam() {
  ...
  if (isll(b)) {
    // 実引数の格上げ (ecallseq) のために語の位置を控える。ビット表は
...
int linit(int i) {
  ...
  v = rvany(assign());
  if (isll(t)) {
    // 64 bit の局所変数は 2 語。右辺が 32 bit なら伸ばす
    if (!isll(ety)) ehi = ext32(v, ety);
  ...
  if (ehi) exit(5);            // 64 bit を狭い型の初期化子にはまだ使えない
...
int slocnew(int li) {
  int e; int n; int c;
  copyn(tname, lname + li * 32);
  n = 0;
  while (n < 26 && tname[n]) n = n + 1;
  tname[n] = '.';
  c = slocnt;
  slocnt = slocnt + 1;
  if (c > 999) exit(6);
...
int plocal() {
  ...
  while (1) {
    ...
    if (sta) {
      ...
      else if (isll(b)) frame1(8);   // 64 bit は下位語・上位語で 2 語
      else frame1(4);
    }
...
int ginit(int e) {
  int t; int n; int i; int es; int flat; int ibase;
  next();
  align4();
  gval[e] = outp;
  gdef[e] = 1;
