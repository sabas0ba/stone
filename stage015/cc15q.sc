/// @file cc15q.sc
/// @brief C コンパイラ 第 15 世代 その 17。cc15p との差は**1 か所だけ**。
///
/// 関数内 static の初期化子が文字列リテラルへのポインタのとき，予約した
/// ローカルシンボルの位置 (lsoff) が誰にも埋められず，ポインタが別の場所を
/// 指していた。**通ってしまうが値が違う**という，我々が台帳で bad と呼ぶ
/// 状態である (docs/stage017-cc.md 24〜25 章)。
///
...
/// そのまま使える。
///
/// **鎖の成果物は 1 バイトも変わらない。** 既存のソースはこの形を使って
/// いない (24.3 で全体を探して 0 件だった)。tcc の tccgen.c が使うので
/// 表に出た。
///
/// ---- 以下は cc15p から引き継いだ来歴 ----
///
