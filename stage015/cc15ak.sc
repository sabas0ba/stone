/// @file cc15ak.sc
/// @brief C コンパイラ 第 15 世代 その 37。cc15aj との差は 64 bit の値を受ける 4 か所 (添字・複合代入・switch・?:)。
///
/// ## 64 bit の値を 32 bit の文脈で使う
///
/// GCC の gcc/ は HOST_WIDE_INT (long long) を添字・複合代入の右辺・switch の
/// 制御式・?: の片方に使う。rv は「教えていない経路では黙って壊れるより
/// 拒む」ために 64 bit を 5 で拒んでいた。経路ごとに意味を決めて通す
/// (docs/stage017-gcc.md 8.13)。
///
///   添字         下位語だけを使う。32 bit の対象ではアドレスの計算が 2^32 で
///                巡回するので，上位語は結果に効かない
///   複合代入     左辺が 32 bit のとき。+ - * & | ^ << >> は結果の下位 32 bit が
///                右辺の下位語だけで決まるので切り詰める。/ と % は上位語が
///                効くので左辺を広げて 64 bit で計算し，下位語を書き戻す
///   switch       上位語が下位語の符号拡張でなければ，どの case にも当たら
///                ない (case の値は 32 bit で読む)。そのときは default へ送る
///   ?:           then が 1 語・else が 2 語のとき。then 側の jmp の行き先を
///                else の後ろの経路へ付け替え，そこで then の値を広げる
///
/// ## 括弧で囲んだ typedef 名
///
/// `typedef const char* (lto_get_section_data_f) (struct lto_file_decl_data *, ...);`
/// (GCC の lto-streamer.h) を，名前の位置の `(` で 1 に止めていた。C89 6.5.4 の
/// 「( declarator )」で，括弧は意味を変えない。宣言 (dcont) と同じく
/// isparennm / unparen1 / unparen2 で括弧を外す。
///
/// ## 局所名が typedef 名を隠す
///
/// C89 6.1.2.1 の有効範囲の規則では，内側で宣言した名前は外側の同じ名前を
/// 隠す。typedef 名も例外ではない。GCC は `typedef struct partition_def
/// *partition;` の下で仮引数を `int partition` と名づけ，本体で
/// `(partition >= 0 && ...)` と書く。cc15ai までの istype は typedef 表だけを
/// 見るので，これをキャストと読んで 1 で止まっていた。局所変数 `edge` (typedef
/// 名 `edge` と同じ) で始まる式文も宣言と読んでいた。
///
/// 関数本体の中 (infn) では，file scope の typedef 名と同じ名前の局所名
/// (仮引数を含む) があれば型名と見なさない。関数の中で宣言した typedef と
/// 局所名の前後関係は持っていないので，関数の中の typedef は従来どおり型名と
/// 見る。
///
/// あわせて tdfind を内側 (後から登録した方) から引くようにした。外側と同じ
/// 名前を内側で typedef したとき，cc15ai までは外側の型を返していた。
///
/// ## 構造体を返す関数を関数へのポインタで呼ぶ
///
/// 名前つきの呼出しは，返却された構造体を呼出し先の記号の型 (gty) から
/// 大きさを求めて引き取る (@section strret)。関数へのポインタを通した
/// 呼出し (c_calli) には記号が無いので，出力段で引き取る語数を決められず，
/// cc15ah までは ecalli が 5 で拒んでいた。
///
/// GCC の gcc/ はフックの表 (targetm など) を関数へのポインタで呼び，
/// double_int などの構造体を値で返す (gcc/ の 51 単位。
/// docs/stage017-gcc.md 8.12)。
///
/// 返却型の大きさを呼出し命令の側情報 irsz に持たせ，出力段は名前つきの
/// 呼出しと同じ引取りを行う。呼ばれた側の返し方は変わらない。
///
/// ## 括弧で囲んだメンバの宣言子
///
/// GCC の reload.h は struct target_reload のメンバを
/// `enum machine_mode (x_regno_save_mode [N][M]);` と書く。C89 6.5.4 の
...
/// ## 大域記号表の容量
///
/// 大域記号表は 8192 個で，GCC の gcc/ の単位は extern の宣言と VEC マクロが
/// 展開する static 関数でこれを使い切る (6 で止まる)。32768 個へ広げ，
/// 線形探索だった gfind をハッシュ表で引くようにした。.strtab の器 (stt) も
/// 2 MiB へ広げ，これまで無かった上限の検査を足した。
///
/// ## 型の基底番号の範囲が重なっていた
///
/// 型番号は「ポインタの深さ << 16 | 基底」で，基底は範囲で種類を表す。
/// cc15ag までは構造体表 (2 から 2048 個)，void と算術型 (300..308)，
/// 配列型表 (1024 から 4096 個)，関数型表 (2048 から 1024 個) が重なって
/// いた。表の大きさを増やしたときに起点を動かしていなかった。
///
/// 構造体が 298 個を超えると 2 + 298 = 300 以降が void や unsigned int と
/// 同じ番号になる。GCC の gcc/ の単位は構造体を 300 個以上宣言するので，
/// `v->prefix.num = n;` (num は unsigned int) の左辺が isstru で構造体と
/// 判定され，型が合わないとして 5 で止まっていた (gcc/ の 75 単位。
/// docs/stage017-gcc.md 8.12)。同じく構造体が 1022 個を超えると配列型と，
/// 配列型が 1024 個を超えると関数型と取り違える。
///
/// 起点を 4096 (void と算術型)，8192 (配列型)，12288 (関数型) へ移し，
/// 範囲を重ならないようにした。いずれも 65536 未満なので，ポインタの段の
/// 符号化は変わらない。
///
/// ## 関数へのポインタへのポインタ `(**)`
///
...
///     int (**pf)(int) = &f;          /* 関数へのポインタへのポインタ */
///
/// fnpdec1 は `(` の直後の `*` を 1 つだけ読み，名前か `)` を期待していた
/// ので，2 つめの `*` で構文エラー (1) になった。GMP の gmp.h がこの形を
/// 持ち，GCC の gcc/ の単位は double-int.h から gmp.h を読むので，
/// **gcc/ のほとんどの単位がここで止まっていた** (docs/stage017-gcc.md 8.11)。
///
/// 2 つめ以降の `*` の数だけポインタの段を足す。符号化は pstars と同じ
/// (1 段ごとに 65536)。
///
/// ## 不完全型の extern 宣言
///
/// `extern struct alloc_zone rtl_zone;` (GCC の ggc.h) を，型の大きさが
/// 判らないとして 5 で拒んでいた。大きさが要るのは記憶域を取る定義だけで，
/// extern の宣言は不完全型のままでよい (C89 6.5.2.1 / 6.7.2)。
///
/// ## ビットフィールドの幅は整数定数式
///
/// C89 6.5.2.1 の幅は「整数定数式」である。memb は数のリテラル 1 個だけを
/// 受けていたので，`unsigned int uexp : (32 - 6);` (GCC の real.h の
/// EXP_BITS の展開) で構文エラー (1) になった。配列の大きさと同じ ccond で
/// 畳み込む。
///
/// 以下は cc15af から引き継いだ註である。
///
/// @file cc15af.sc
/// @brief C コンパイラ 第 15 世代 その 32。cc15ae との差は 1 か所 (funcdef)。
///
/// ## 空の括弧の宣言は仮引数について何も言わない
///
/// C89 6.5.4.3 —— 関数宣言子の識別子並びが空で，それが定義の一部でない
/// ときは，**仮引数の個数も型も与えない**。`int f();` はプロトタイプでは
/// なく，どの個数の実引数でも呼べる。先にプロトタイプがあれば，後から来た
/// `int f();` と合わせた合成型 (6.1.2.6) はプロトタイプのままである。
///
/// cc15ae までは `int f();` を**仮引数 0 個のプロトタイプ**として控えて
/// いた。形は 2 つあり，どちらも実引数の個数の不一致 (5) で拒んでいた。
///
///     int g();              g(1, 2, 3);      /* 宣言だけ・呼出しに実引数 */
///     int f(int, int);      int f();         f(1, 2);   /* 再宣言 */
///
/// GCC 4.7.4 の libiberty/regex.c は後者である。`<stdlib.h>` の後で
/// `char *malloc ();` `char *realloc ();` と宣言し直し，`realloc` を
/// 2 引数で呼ぶ (docs/stage017-gcc.md 8.10)。
///
/// 直し方: funcdef で `()` の直後が `;` なら (定義ではない宣言なら)
/// 仮引数の情報に触らずに戻る。初めて見る名前なら個数は未知 (-1) のまま
/// で，宣言の判らない呼出しと同じに扱われる。**定義** (`int f() {`) は
/// これまでどおり仮引数 0 個である。
///
/// 以下は cc15ae から引き継いだ註である。
///
/// @file cc15ae.sc
/// @brief C コンパイラ 第 15 世代 その 31。cc15ad との差は 1 か所。
///
/// ## 仮引数の `register`
///
/// C89 6.5.1 は，仮引数の宣言に書ける記憶域クラス指定子を `register`
/// **1 つに限って許している**。
///
...
/// 読み捨てていなかった。`plocal()` と `topdecl()` は前から読み捨てて
/// いる —— **仮引数並びの側だけが持っていなかった。**
///
...
///
/// `libiberty/hashtab` には**未対応箇所が 2 つあった**。1 つめは 1691 行の
/// `size_t (htab_size) (htab_t htab)` (8.3 の 3) で，cc15ad が通した。
/// その先の 2257 行にこれがある。8.2 が何度も書いている
/// 「**1 つ通すと次が見える**」がここでも出た。
///
/// 以下は cc15ad から引き継いだ註である。
///
/// ## GCC 4.7.4 を測って残っていた C89 適合の不足 —— 5 つすべて
///
/// docs/stage017-gcc.md 8.3 の 1・2・3・5・6 である。**`gap` の 5 単位が
/// ちょうどこの 5 つで，1 つずつ 1 単位の翻訳を妨げていた。**
///
///   1. **複文の中の `struct tag ;`** (タグだけの宣言)。`ansidecl.h` の
///      `VA_OPEN` が `{ va_list ap; va_start(ap, v); { struct Qdmy` と
///      展開するので，**可変長引数を使う単位すべてに影響する**。大域では
///      dcont が同じ形を受けていて，複文の側だけが持っていなかった
///      (`concat`)
///   2. **ビットフィールドへの `++` / `--`**。代入は読み・修正・書きを
///      持っていたのに，`incdec` だけが持たずに拒んでいた (`fibheap`)
///   3. **括弧で囲んだ名前の宣言子** (`int (f) (int x)`)。C89 6.5.4 の
///      宣言子は「( 宣言子 )」を含む。GCC の `hashtab.c` がマクロとの
///      衝突を避けるためこの形で書く (`hashtab`)
///   5. **block scope の `typedef`**。C89 6.5.1 は `typedef` を記憶域
///      クラス指定子として挙げており，置ける場所を file scope に限って
///      いない (`sort`)
///   6. **block scope の `extern` 宣言**。実体は外にあるので，フレームを
///      取らず大域として登録する (`xmalloc`)
///
/// 4 と 7 は先の世代で通した (8.3 の表)。
///
/// **5 と 6 は名前の見える範囲の話である。** どちらも複文を抜けると
/// 見えなくなるので，局所名 (`lblk`) と同じ形で入口の位置を控えて出口で
/// 戻す。typedef 名にも `tdblk` を置いた。
///
/// 以下は cc15ac から引き継いだ註である。
///
/// ## 差分試験がコンパイラ側で見つけた 5 つの不具合
///
/// どれも **C89 の内側**にある。GCC の C を stone の処理系で翻訳する
/// (docs/stage017-gcc.md 6.1 の 4) 前に修正しておく。見つけたのは
/// `tests/stage015/probe` の bitfld / funcptr / intconv / aggr で，
...
/// **通ったうえで値が違うもの (bad)**
///
///   4. **静的な初期化子のビットフィールド**。メンバごとに記憶単位を
///      書いていたので，同じ単位を共有する 2 つめ以降が隣を 0 で上書きして
///      いた。`static struct a g = {1,0,5,200};` が `{1,0,0,0}` になる
///   5. **関数ポインタ越しの 64 bit 返却**。名前つきの呼出し (`ecall`)
///      にはある上位語の引取りが `ecalli` に無く，直前の式の上位語が
///      残ったまま返る。`d(2.5, 4.0) * 100` が 400 になり，しかも
///      **データスタックが 1 語ずれる**
///
/// 4 と 5 は `cc15u` と同じ種類の不具合である —— 固定点もバイト一致も往復検査も
/// 検出しない。stone のソースがどちらの形も使っていないので，
/// **自分自身を組む限り永久に表に出ない**。外部のソースをビルド対象として読ませるか，
/// ホストと突き合わせる以外に見つける方法が無い。
///
...
int gty[32768];            ///< 型 (変数は自身の型，関数は返却型)
int gval[32768];           ///< 変数: 絶対アドレス / 関数: 定義済みならコード位置，未定義なら未解決呼出しリストの先頭
int gdef[32768];           ///< 1 = 定義済み。0 のまま入力が終われば未解決の前方参照 (エラー 2)
int garr[32768];           ///< 1 = 配列。式中では先頭要素へのポインタに退化する
int gna[32768];            ///< 引数個数。-1 = 未知 (前方参照で個数がまだ判らない)
int gsz[32768];            ///< 大域変数の大きさ (バイト)。ELF シンボルの st_size になる
int gsta[32768];           ///< 1 = static。ELF のローカルシンボルとして出す
int gtxt[32768];           ///< 1 = 初期値を持ち，実体が .text にある
int gvar[32768];           ///< 1 = 可変長引数を取る関数 (gna は名前つきの個数)
int gllm[32768];           ///< 仮引数の 64 bit 位置のビット表 (bit k = 語 k から
                          ///  64 bit の仮引数が始まる)。実引数の格上げに使う。
                          ///  32 語を超える位置は表せないので宣言時に拒む
int gdbm[32768];           ///< 同じく double の位置 (2 語)
int gflm[32768];           ///< 同じく float の位置 (1 語)
int gused[32768];          ///< 1 = 個数が判らないまま呼出しを出した (前方参照)
int gidx[32768];           ///< 大域記号 -> ELF シンボル番号
int nsta;                 ///< ローカル側へ集約した static 記号の数
int gcnt;                 ///< 大域記号の登録数
/// 大域記号の探索用のハッシュ表 (cc15ai)。ghtab[h] は鎖の先頭の添字 + 1
/// (0 = 空)，gnext[e] は同じハッシュ値の次の添字 + 1 である
int ghtab[65536];
int gnext[32768];
...
int tdty[2048];            ///< 対応する型
int tdcnt;                ///< 登録数
/// 現在の複文で登録し始めた位置 (局所名の lblk と同じ役どころ)。
/// **これより手前は外側の名前**なので，同じ名前を内側で宣言してよい。
/// file scope では 0 である (cc15ad で足した。8.3 の 5)
int tdblk;
/// 関数本体に入った時点の登録数。関数を抜けるときにここまで戻す
int tdfn;
/// 1 = 関数本体の中。局所名が typedef 名を隠すかを istype で見るのに使う。
...
char stt[2097152];         ///< .strtab の内容 (cc15ai で 512 KiB から広げた)
int stp;                  ///< stt の有効長
...
int t_void;               ///< void の型番号 (構造体の 2..2049 と離した値)
...
int init() {
  ...
  // 基底の番号は範囲ごとに重ならないように置く (cc15ah)。
  //   0 / 1         char / int
  //   2..2049       構造体表 (2048 個)
  //   4096..4104    void と残りの算術型
  //   8192..12287   配列型表 (4096 個)
  //   12288..13311  関数型表 (1024 個)
  // cc15ag までは void を 300，配列を 1024，関数を 2048 から置いていた。
  // 構造体が 298 個を超えると void 以降と重なり，unsigned int のメンバへの
  // 代入が構造体の代入と判定されていた (docs/stage017-gcc.md 8.12)
  t_void = 4096;
  t_arr = 8192;
  t_schar = 4097; t_short = 4098; t_ushort = 4099; t_uint = 4100;
  t_llong = 4101; t_ullong = 4102;
  t_float = 4103; t_double = 4104;
  t_fn = 12288;
  return 0;
}
...
// ---- 記号表 (scc と同一) ----
// ローカルは線形探索である。大域は cc15ai からハッシュ表で引く。
// GCC の gcc/ の単位は大域記号を 8192 個より多く宣言し (extern の宣言と，
// VEC マクロが展開する static 関数)，線形探索では識別子 1 個ごとに表全体を
// なめることになる。
// 探索の鍵は常に直近に読んだ識別子 tname である。
// 見つからない場合は -1 を返す。0 は正当な添字なので不可を表せない。

/// @brief tname のハッシュ値 (0..65535)。
int ghash() {
  int h; int i;
  h = 0;
  i = 0;
  while (tname[i]) { h = (h * 31 + tname[i]) & 65535; i = i + 1; }
  return h;
}

/// @brief tname と一致する大域記号を探す。
/// @return エントリ添字。見つからなければ -1
/// @note 同じ名前が複数あれば**最も小さい添字**を返す (線形探索だった
///       cc15ah までと同じ)。鎖は新しいものが先頭なので，最後に一致した
///       ものを返す。
int gfind() {
  int i; int r;
  r = -1;
  i = ghtab[ghash()];
  while (i) {
    if (streq(gname + (i - 1) * 64, tname)) r = i - 1;
    i = gnext[i - 1];
  }
  return r;
}
...
int tdfind() {
  int i;
  // 内側 (後から登録した方) を先に見つける。局所名の lfind と同じ形で，
  // これが遮蔽になる。cc15ai までは外側から引いていたので，内側で同じ
  // 名前を typedef しても外側の型が返った (cc15aj)
  i = tdcnt - 1;
  while (i >= 0) {
...
int istype() {
  ...
  if (tok == t_id) {
    k = tdfind();
    if (k < 0) return 0;
    // **局所名は file scope の typedef 名を隠す** (C89 6.1.2.1。cc15aj)。
...
/// @brief 括弧で囲んだ名前の宣言子「( 名前 )」か。
/// @return 1 = そうである
/// @note C89 6.5.4 の宣言子は「( 宣言子 )」を含むので，`int (f) (int x)`
///       は `int f (int x)` と同じ意味である。GCC の `hashtab.c` が
///       マクロとの衝突を避けるためこの形で書く
///       (docs/stage017-gcc.md 8.3 の 3)。
///
///       **ポインタを含む形 (`int (*f)(int)`) は isfnp が先に受ける。**
///       ここが見るのは括弧の中が名前 1 つだけの場合に限る。
int isparennm() {
  int r;
  if (tok != o_lp) return 0;
  lsave();
...
/// @brief 括弧で囲んだ名前の宣言子から括弧を外す。
/// @return 常に 0
/// @note 呼ぶ前に isparennm() が真であること。'(' を捨てるので，
///       呼んだ後は名前が現在のトークンになる。閉じ括弧は名前を読んだ
///       後に unparen2() で捨てる。
int unparen1() { next(); return 0; }

/// @brief 括弧で囲んだ名前の宣言子の閉じ括弧を捨てる。
int unparen2() {
  if (tok != o_rp) exit(1);
  next();
  return 0;
...
int fnpdec1(int b, int anon) {
  ...
  //
  // **`*` は 1 つとは限らない** (cc15ag)。C89 6.5.4.1 の pointer は
  // 「* type-qualifier-list_opt pointer_opt」で，`void *(**)(size_t)` は
  // 「関数へのポインタへのポインタ」である。GMP の gmp.h が
  // __gmp_get_memory_functions の仮引数をこの形で書き，gcc/ の単位は
  // double-int.h から gmp.h を読む (docs/stage017-gcc.md 8.11)。
  // 2 つめ以降の `*` の数だけポインタの段を足す (pstars と同じ符号化)
  np = 0;
  while (tok == o_mul || tok == k_const || tok == k_volatile) {
    if (tok == o_mul) np = np + 1;
    next();
  }
  fpnam[0] = 0;
  if (tok == t_id) {
  ...
  // [n] が並ぶ。GCC の hooks はこの形で並ぶ (cc15ac で足した)
  nd = 0;
  while (tok == o_lb) {
    if (nd > 7) exit(6);
    next();
...
int epost() {
  ...
  while (tok == o_lb || tok == o_dot || tok == o_arrow
    ...
    if (tok == o_lb) {
      ...
      lpt = ety;
      next();
      // **64 bit の添字は下位語だけを使う** (cc15ak)。32 bit の対象では
      // アドレスの計算が 2^32 で巡回するので，上位語は結果に効かない。
      // GCC は HOST_WIDE_INT (long long) を添字に使う
      i = rvany(expr());
      if (isfp(ety)) exit(5);
      ehi = 0;
      ipt = ety;
      ...
      // 無いので `3[p]` も `p[3]` と同じ意味になる。cc15ab までは左が
      // ポインタであることを要求していた (cc15ac で足した)
      if ((lpt >> 16) == 0) {
        if ((ipt >> 16) == 0) exit(5);
        tsw = v; v = i; i = tsw;
        pt = ipt;
...
int incdec(int a, int t, int isadd, int post) {
  ...
  if (bw) {
    ...
    w = ldval(a, 1);
    cur = w;
    if (bo) cur = emit(c_bin + b_srl, cur, emit(c_const, bo, 0));
    cur = emit(c_bin + b_and, cur, emit(c_const, msk, 0));
    one = emit(c_const, 1, 0);
    if (isadd) nv = emit(c_bin + b_add, cur, one);
    else nv = emit(c_bin + b_sub, cur, one);
    // **幅からはみ出た桁は捨てる。** 3 bit の 7 に 1 を足せば 0 である
    nv = emit(c_bin + b_and, nv, emit(c_const, msk, 0));
    res = emit(c_bin + b_and, w,
               emit(c_const, (msk << bo) ^ 0xffffffff, 0));
    if (bo) res = emit(c_bin + b_or, res,
                       emit(c_bin + b_sll, nv, emit(c_const, bo, 0)));
    else res = emit(c_bin + b_or, res, nv);
    emit(c_stw, a, res);
    ety = t; elv = 0; earr = 0;
    if (post) return cur;
    return nv;
...
int econd() {
  ...
  int w2; int off2;
  int jt; int l3; int c; int ch; int te;   // then を後から広げる経路 (cc15ak)
  v = ecor();
  ...
  if (!w2 && is2w(ety)) {
    // **逆向き: then が 1 語で else が 2 語** (a < b ? a : (hwi) x など。
    // cc15ak)。then の値は 1 語のまま off に書いてあり，then 側の命令は
    // 出し終えている。then 側の jmp の行き先を l3 に付け替え，else の後ろに
    // 置いた l3 の経路で then の値を読み直して else の型へ広げる。IR に命令を
    // 挿し込まずに済み，順向きの形の出力は変わらない
    emit(c_stw, emit(c_laddr, off, 0), b);
    emit(c_stw, emit(c_laddr, off2, 0), ehi);
    l3 = newlab();
    ia[jt] = l3;
    emit(c_jmp, l2, 0);
    emit(c_lab, l3, 0);
    c = emit(c_loadw, emit(c_laddr, off, 0), 0);
    te = ety;                            // else の型 (結果の型)
    if (isfp(t) && !isfp(te)) exit(5);   // float と 64 bit 整数は会ってから
    if (isfp(te)) {
      // int / float -> double。cvtto は変換元の型を ety から読む
      ety = t;
      ehi = 0;
      c = cvtto(c, te);
    } else {
      ehi = ext32(c, t);                 // 32 bit -> 64 bit (then の符号で)
    }
    ety = te;
    ch = ehi;
    emit(c_stw, emit(c_laddr, off, 0), c);
    emit(c_stw, emit(c_laddr, off2, 0), ch);
    emit(c_lab, l2, 0);
    v = emit(c_loadw, emit(c_laddr, off, 0), 0);
    ehi = emit(c_loadw, emit(c_laddr, off2, 0), 0);
    // 結果の型は広い側 (else) である
    elv = 0; earr = 0;
    return v;
  }
...
int assign() {
  ...
  if (isfp(ety)) exit(5);
  if (ehi) {
    // **右辺が 64 bit** (cc15ak)。a op= b は a = a op b で，演算は 64 bit で
    // 行い左辺の型へ戻す (C89 6.3.16.2)。/ と % は上位語が結果に効くので
    // 左辺を広げて 64 bit で計算する。それ以外は結果の下位 32 bit が右辺の
    // 下位語だけで決まるので切り詰める
    if ((t >> 16) == 0 && (op == b_div || op == b_rem)) {
      ch = ext32(cur, t);
      rh = ehi;
      if (isull(ety)) {
        if (op == b_div) res = ll_call2("__udiv64", t_ullong, cur, ch, r, rh);
        else res = ll_call2("__umod64", t_ullong, cur, ch, r, rh);
      } else {
        if (op == b_div) res = ll_call2("__div64", t_llong, cur, ch, r, rh);
        else res = ll_call2("__mod64", t_llong, cur, ch, r, rh);
      }
      ehi = 0;
      stval(v, res, t);
...
int stmt() {
  ...
  int sw64; int sfit;           // 64 bit の制御式の switch (cc15ak)
  if (tok == o_lc) {
    next();
    ...
    b0 = cloff;
    d = lblk;
    // **typedef 名も複文を抜けると見えなくなる。** 局所名と同じ扱いで，
    // 入口で数を控えて出口で戻す。tdfind は 0..tdcnt-1 しか見ないので，
    // 戻せば内側の typedef は引けなくなる (8.3 の 5)
    s = tdcnt;
    w = tdblk;
    tdblk = tdcnt;
    lblk = lcnt;            // ここから内側。外側と同名を宣言してよい
    // extern と typedef は istype() が見ない (型指定子ではなく記憶域
    // クラス指定子である)。宣言の始まりとしてはここで受ける
    while (istype() || tok == k_extern || tok == k_typedef) plocal();
    while (tok != o_rc) stmt();
    next();
    lcnt = n;
    cloff = b0;
  ...
  if (tok == k_switch) {
    ...
    c = rvany(expr());
    if (isfp(ety)) exit(5);
    // **64 bit の制御式** (cc15ak)。case の値は 32 bit で読むので，上位語が
    // 下位語の符号拡張でなければどの case にも当たらない。収まるかどうかを
    // 先に計算して控え，振分けの前に見る
    sw64 = 0;
    if (ehi) {
      sw64 = 1;
      sfit = hslot();
      emit(c_stw, emit(c_laddr, sfit, 0),
           emit(c_bin + b_seq, ehi, emit(c_bin + b_sra, c, emit(c_const, 31, 0))));
      ehi = 0;
    }
    if (tok != o_rp) exit(1);
    next();
    ...
    emit(c_lab, l0, 0);
    if (sw64) {
      // 32 bit に収まらない値は default へ (無ければ switch の外へ)
      c = emit(c_loadw, emit(c_laddr, sfit, 0), 0);
      if (swdflt >= 0) emit(c_bz, c, swdflt);
      else emit(c_bz, c, l1);
    }
    i = b0;
    while (i < swn) {
...
int emitins(int i) {
  ...
  if (k == c_calli) {
    ...
    outw(iw3(0x2003, d, 9, 0));
    if (irsz[i]) {
      // 構造体の返却。c_call と同じく，1 語目の上に語 0 から順に積まれて
      // いるのをフレームの一時領域へ引き取る。大きさは記号の型ではなく
      // 側情報から得る (cc15ai)
      k = irsz[i];
      j = 0;
      while (j < k) {
...
int memb(int se, int t, int fp) {
  ...
  int al;
  int u;
  int w;                        // ビットフィールドの幅 (cc15ag)
  int anon;
  int paren;                    // 宣言子を括弧で囲んだ形 (cc15ai)
  int t0;
  ...
  // として使う形で，C89 6.5.2.1 が定めている。cc15ab までは名前が無い
  // 時点で拒んでいた (cc15ac で足した)
  // **括弧で囲んだ宣言子** (`enum machine_mode (x_regno_save_mode [N][M]);`。
  // GCC の reload.h)。C89 6.5.4 の「( declarator )」で，括弧は意味を
  // 変えない。名前と配列の次元を括弧の中で読む (cc15ai)
  anon = 0;
  paren = 0;
  if (!fp) {
    if (tok == o_col) anon = 1;
  ...
  if (!fp) {
    if (!anon) next();
    if (paren) {
      // 次元は括弧の中か外のどちらか一方だけを受ける。両方に書く形
      // (`(a[2])[3]`) は内側と外側で次元の順が入れ替わるので扱わない
      t0 = t;
      t = pdims(t);
      if (tok != o_rp) exit(1);
      next();
    ...
    if (tok == o_col) {
      ...
      next();
      // **幅は整数定数式である** (C89 6.5.2.1。cc15ag)。cc15af までは
      // 数のリテラル 1 個だけを受けていた。GCC の real.h は
      // `unsigned int uexp : EXP_BITS;` と書き，EXP_BITS は (32 - 6) に
      // 展開される。配列の大きさと同じ ccond で畳み込む (式の後ろまで読む)
      w = ccond();
      if (w < 0) exit(2);
      if (w == 0) {
...
int pparam() {
  int b; int i;
  // **仮引数に書ける記憶域クラスは register だけである** (C89 6.5.1)。
  // 割付けは変えないので読み捨てる。plocal() と topdecl() は前から
  // 同じことをしていて，仮引数並びの側だけが持っていなかった
  // (docs/stage017-gcc.md 8.3 の 14)
  while (tok == k_register) next();
  b = pstars(ptype());
  if (isfnp()) {
    // 仮引数の関数ポインタ
    b = fnpdec1(b, 1);
...
/// @brief block scope の extern 宣言を読む (C89 6.5.1)。
/// @param base 基底型
/// @return 常に 0
/// @note **実体は外にあるので，フレームは取らず大域として登録する。**
///       名前の解決は局所を先に見るので，局所を作らなければ大域へ届く。
///
///       大域の dcont へ渡す方式は採れない —— 名前の次が '(' だと
///       funcdef() へ行き，lcnt / lblk を 0 に戻して**外側の関数の
///       局所名を失う**。ここで読み切る (docs/stage017-gcc.md 8.3 の 6)。
///
///       既に宣言済みなら何もしない。同じ名前を外でも中でも宣言するのは
///       正しい (どちらも同じ実体を指す)。
int plocext(int base) {
  int b; int g; int n; int fp; int pn;
  while (1) {
    b = pstars(base);
    fp = 0;
    if (isfnp()) {
    ...
    // **記号表の鍵は tname である。** next() は識別子を読んだときしか
    // tname を書き換えないので，名前の次が '(' / '[' / ',' / ';' の
    // うちはまだ名前が残っている。引くのは [n] を読む前にすること
    g = gfind();
    if (!fp && tok == o_lp) {
      // 関数の宣言。仮引数は括弧の対応だけ数えて読み飛ばす
      // (fnpdec と同じ手)。個数は判らないままにしておく
      next();
      n = 1;
      while (n > 0) {
        if (tok == t_eof) exit(1);
...
int plocal() {
  int base; int b; int i; int fp; int sta; int g; int ext; int pn;
  // **block scope の typedef** (C89 6.5.1 は記憶域クラス指定子として
  // typedef を挙げており，置ける場所を file scope に限っていない)。
  // GCC の sort.c がこの形で書く (docs/stage017-gcc.md 8.3 の 5)。
  // 名前の有効範囲は複文を抜けるまでで，抜けるときに stmt() が
  // tdcnt を戻す
  ...
  while (tok == k_register || tok == k_auto || tok == k_static
    ...
    // **block scope の extern** (C89 6.5.1)。実体は外にあるので
    // フレームは取らず，大域として登録するだけにする。名前の解決は
    // 局所を先に見るので，局所を作らなければ大域へ届く
    // (docs/stage017-gcc.md 8.3 の 6)
    else if (tok == k_extern) ext = 1;
    next();
  }
  base = ptype();
  // タグだけの宣言 (`struct Qdmy;`)。型を登録するだけで変数は作らない。
  // 大域では dcont が同じことをしている。ansidecl.h の VA_OPEN が
  // 複文の先頭にこの形を展開するので，可変長引数を使う単位すべてに
  // 影響する (docs/stage017-gcc.md 8.3 の 1)
  if (tok == o_semi) { next(); return 0; }
  if (ext) return plocext(base);
  ...
  while (1) {
    ...
    // 括弧で囲んだ名前「int (v);」(C89 6.5.4)。外せばふつうの宣言である
    pn = 0;
    if (!fp) {
      pn = isparennm();
      if (pn) unparen1();
...
int krdecl() {
  ...
  // K&R 形式の宣言も仮引数の宣言である。register を読み捨てる
  while (tok == k_register) next();
  base = ptype();
  while (1) {
    b = pstars(base);
...
int funcdef() {
  ...
  // **空の括弧の宣言は仮引数について何も言わない** (C89 6.5.4.3)。
  // `int f();` はプロトタイプではなく，どの個数の実引数でも呼べる。
  // 先にプロトタイプがあれば，それが合成型として残る (6.1.2.6)。
  // したがって仮引数の情報には触らずに戻る。初めて見る名前なら個数は
  // 未知 (-1) のままで，呼出しは宣言の判らない呼出しとして扱われる。
  // 定義 (`int f() { ... }`) はこれまでどおり仮引数 0 個である
  if (tok == o_rp && !vexp) {
    lsave();
    next();
    if (tok == o_semi) {
  ...
  spcnt = 0;
  hcnt = 0;
  // extern と typedef は istype() が見ない (型指定子ではなく記憶域クラス
  // 指定子である)。複文の側 (stmt) と同じ形でここでも受ける。
  // **関数本体の typedef は関数を抜けると見えなくなる** ので，入口の
  // 位置を控えて出口で戻す (8.3 の 5・6)
  tdblk = tdcnt;
  tdfn = tdcnt;
  infn = 1;
  while (istype() || tok == k_extern || tok == k_typedef) plocal();
  ...
  // 関数本体で登録した typedef 名を捨てる。file scope へ戻るので
...
///       cc15ab まではそうなっていて，`static struct a g = {1,0,5,200};`
///       の 3 つめ以降が別の語へ書かれていた (cc15ac で直した)
    ...
    if (isstru(t)) {
      ...
      while (m < mcnt) {
        if (msid[m] == se) {
          if (tok == o_rc) break;
          if (mbfw[m]) {
            // ビットフィールドは**記憶単位へ溜めてから書く**。メンバ
            // ごとに単位を書くと，同じ単位を共有する隣を 0 で上書きする
...
int dcont(int b) {
  ...
  while (1) {
    ...
    if (isfnp()) {
      ...
      // 括弧で囲んだ名前「int (f) (int x)」。括弧を外せば
      // 「int f (int x)」と同じである (C89 6.5.4)
      pn = isparennm();
      if (pn) unparen1();
      if (tok != t_id) exit(1);
      next();
    ...
    if (skip == 2) {
      ...
      if (!fp) cty = pdims(cty);
      // 大きさが要るのは記憶域を取る定義だけである (cc15ag)。extern の
      // 宣言は不完全型のままでよい (C89 6.5.2.1 / 6.7.2)。GCC の ggc.h は
      // `extern struct alloc_zone rtl_zone;` と書き，struct alloc_zone を
      // どこにも定義しない。後で同じ名前を定義すれば，そのときの型で
      // 大きさを取り直す (下の本定義の経路)
      if (isstru(cty) && ssize[cty - 2] == 0 && !cext) exit(5);
      gty[e] = cty;
      gsta[e] = cstat;
...
int typedef1() {
  ...
  if (isfnp()) {
    ...
    // `typedef T (name) (params);` (GCC の lto-streamer.h)。括弧は意味を
    // 変えないので外す (C89 6.5.4。宣言の側の dcont と同じ扱い)
    pn = isparennm();
    if (pn) unparen1();
    if (tok != t_id) exit(1);
  }
  // 外側と同じ名前は内側で宣言してよい (局所名の lfind() >= lblk と
  // 同じ形)。file scope では tdblk が 0 なので，今までどおり重複は 4
  if (tdfind() >= tdblk) exit(4);
...
int stadd(char *nm) {
  int o; int i;
  // 名前は 63 バイト以内なので，1 個ぶんの余裕を見て止める。cc15ah までは
  // 検査が無く，大域記号が多いと後ろの表を黙って壊すおそれがあった (cc15ai)
  if (stp > 2097152 - 80) exit(6);
  o = stp;
  i = 0;
