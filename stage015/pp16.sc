...
/// 第 9 部)。外部ソース (zlib) は入力 256 KiB・識別子 31 バイトの容量に
...
///   - マクロ数: 1024 -> 4096 / 仮引数の領域: 32 Ki -> 128 Ki
///   - 入力・アリーナ: 1 MiB -> 4 MiB
...
/// マクロを実引数に渡すと，抑止して展開せずに出力した名前が置換結果の再走査で
/// もう一度展開され，s1->s1->eh_frame_section になっていた。
/// @section hygiene 抑止の印
/// 抑止して展開せずに出力する識別子には，事前展開の出力 (eb) へ書くときだけ
...
/// pp の処理はすべて「文字を 1 個引き出す」操作を基盤にしている。引き出し元は
