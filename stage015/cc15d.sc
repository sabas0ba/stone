...
int cuna() {
  int v;
  if (tok == o_sub) { next(); return 0 - cuna(); }
  if (tok == o_add) { next(); return cuna(); }
  if (tok == o_not) { next(); return !cuna(); }
  if (tok == o_tilde) { next(); return cuna() ^ (0 - 1); }
  if (tok == t_num) { v = tval; next(); return v; }
  if (tok == t_id) {
    v = ecfind();
    if (v < 0) exit(2);
    next();
...
/// @return 値番号
int emul() {
  int v; int r; int op; int lt; int rt; int lh;
  v = euna();
  while (tok == o_mul || tok == o_div || tok == o_mod) {
    ...
    rt = ety;
    if (isll(lt) || isll(rt)) {
      // 除算・剰余はソフトウェアで割る必要があり，まだ入れていない
      if (op != o_mul) exit(5);
      v = ll_mul(v, wide1(v, lh, lt), r, wide1(r, ehi, rt));
