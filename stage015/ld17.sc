...
/// bzip2) を stone の処理系で組む測定 (docs/stage017-gcc.md 5.1) で、これが
/// そのまま行き止まりになった —— 落ちた理由を知る手段が無く，ライブラリを
/// 総当たりで削るしかなかった。
///
/// 足したのは 3 つだけで，**正常系の経路のバイト列は 1 ビットも変わらない**。
...
/// sys_write が無い (ld16.bin はベアメタルのフラットなイメージである)。だから
/// **書ける先は標準出力しかない**。イメージを書き出すのは relocate の後なので，
/// 診断を出す経路ではイメージは 1 バイトも出ていない —— 混ざることはない。
/// 呼ぶ側が出力をファイルに向けていれば，そのファイルに診断が入る。
///
/// 以下は ld16 の説明を引き継ぐ。
/// @brief リンカ (Stage 15 第 6 部世代)。ld14 の複製に**容量だけを足した**。
///
/// tcc の .o (約 1 MB・シンボル数千) が ld14 のバッファ容量・シンボル数の上限を超える
/// (docs/stage015-tcc.md 12 章)。
///   - 入力・出力イメージ: 1 MiB -> 8 MiB
///   - 大域シンボル: 2048 -> 8192 / オブジェクト内シンボル: 8192 -> 65536
///   - 名前スロット: 32 -> 64 バイト (最長識別子 39 文字。31 文字での
///     切詰めは同名衝突を黙って起こす)
/// 仕組みは ld14 と同一である。以下は ld14 の説明を引き継ぐ。
/// @brief 実行イメージを作るリンカ (Stage 14 世代)。出力形式を 3 つ持つ。
///
...
/// stage008 と同じく「出力の先頭からのオフセット」で計算し，ロードアドレスは
/// 再配置の値を作る時にだけ足す。'E' では出力の先頭に ELF ヘッダが入るので，
/// オフセット 0 がそのままロードアドレス (0x8600_0000) に対応する。
///
/// @section priv 特権命令は前置部が持つ
/// カーネルが要る mtvec 設定・レジスタ退避・mret は C では書けないので，
/// 前置部が機械語で持つ (docs/stage012-os.md 5.2)。cc は変更しない。
// ---- 領域 ----
char inp[8388608];        ///< 入力オブジェクトを連結して保持する
int inn;                  ///< inp の有効長
char img[8388608];        ///< 出力する実行イメージ
int imgn;                 ///< img の有効長
...
/// @brief リンカ本体。標準入力のオブジェクト列から実行イメージを書き出す。
