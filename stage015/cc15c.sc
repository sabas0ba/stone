...
/// @brief v * 10 が 32 bit をあふれるか (v は符号なしとみなす)。
int mulov10(int v) { return mulhi10(v) != 0; }
...
int lexnum() {
  ...
  if (getch() == '0' && src[pos + 1] == 'x') {
    ...
    while (isdig(getch())) {
      d = getch() - '0';
      // 10 倍を上位語込みで行う。下位語の桁上げは符号なし比較で見る
      if (mulov10(tval) || tvalhi) ov = 1;
      tvalhi = tvalhi * 10 + mulhi10(tval);
      tval = tval * 10 + d;
      adv();
    }
  }
