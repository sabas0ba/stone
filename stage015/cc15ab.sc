/// @file cc15ab.sc
/// @brief C コンパイラ 第 15 世代 その 28。cc15aa との差は 3 か所。
///
/// ## 差分試験がコンパイラ側で見つけた 3 つの穴
///
/// どれも **C89 が許す形を我々が拒む** ものである (値が壊れる類では
/// ないので gap であって bad ではない)。GCC の C を我々の器へ入れる
/// (docs/stage017-gcc.md 6.1 の 4) 前に，先に潰しておく。
///
///   1. **無名のビットフィールド** (`unsigned : 5;` / `unsigned : 0;`)。
///      詰め物として使う形で，C89 6.5.2.1 が定めている。幅 0 は
///      「次の記憶単位へ送る」の意で，名前は付けられない
///   2. **関数ポインタの配列** (`int (*tab[3])(int, int)`)。宣言子は
///      内側から読むので「関数へのポインタの配列」である。typedef を
///      挟めば通っていたが，直に書く形を拒んでいた
///   3. **逆向きの添字** (`3[p]`)。`E1[E2]` は `*(E1 + E2)` であり，
///      足し算に順序は無い (C89 6.3.2.1)
///
/// 見つけたのは `tests/stage015/probe` の bitfld / funcptr / intconv で，
/// どれもホストの処理系と突き合わせて出たものである。
///
/// **受けない形を 1 つ残した** —— 関数ポインタを返す関数
/// (`int (*pick(int))(int, int)`) である。宣言子の途中で「これは関数
/// 定義である」と判る形になり，宣言の読み方そのものを組み直すことに
/// なるので，次の世代へ回す (docs/stage018-ext.md に記録)。
///
/// 以下は cc15s から引き継いだ註である。
///
/// ## 文字列リテラルの族の，最後の 1 つ
///
/// ## 文字列リテラルの族の，最後の 1 つ
...
///
/// **決めるのは要素ごとである。** 宣言どおりの要素型 (esd) と最後まで
/// 畳んだスカラ型 (es) の両方を持ち，`{` か「文字型配列に対する文字列」
/// なら esd で，そうでなければ es で読む。数え上げはバイトで行う。
///
...
/// | 17 | `char a[1][2][2] = {{'a','b','c','d'}}` | 正 | 正 |
/// | 18 | `char a[2][2][4] = {{"ab","cd"},{"ef","gh"}}` | **拒む** | 正 |
/// | 19 | `int a[2][2] = {1, {2}, 3, 4}` | 正 | 正 |
/// | 20 | `char a[1][8] = {{"ab",}}` | **拒む** | 正 |
///
/// 11〜20 はレビューで指摘を受けて足した。
///
/// そのうえで **`static int a[2][2] = { {1,2,3} }` は拒む**。内側が溢れて
/// いるのに外側に余りがあると，総量の検査を素通りして**溢れた分が別の
/// 行に入る**。台帳では staticstr3 が gap 6 で見ている。最初の直しは先頭 1 回だけ見て
/// 全体を決めていたので，C が許す**波括弧の省略** (C89 6.5.7) を拒んで
/// いた —— cc15r が通す形を壊していた。要素ごとに決める形へ直した。
///
/// **1 つの形で確かめて族ぜんぶが無事だと思ったのが 3 度の誤りの元
/// だった** (docs/stage017-cc.md 27.4 / 31.4)。4 度目は測ってから書いたが，
/// **測る形を自分で選んでいた**。
///
/// **鎖の成果物は変わらない。** 既存のソースに多次元の char 配列を文字列で
/// 初期化する形は無い。stage017 まで作り直しても全段が cached である。
///
/// **tcc は変わる。** 最初「tcc にも無い」と書いたが探し方が誤っていた ——
/// `static char const ab_month_name[12][4]` (tccpp.c 3384 行) の const が
/// 間に入る形を見落としていた。tcc は 974,204 -> 974,156 バイトになる。
/// **cc15r までは，この 12 個の要素に文字の実体ではなく再配置が書かれて
/// いた** —— 我々が組んだ tcc の __DATE__ は別の場所を読んでいた
/// (docs/stage017-cc.md 33.5)。
///
/// ---- 以下は cc15r から引き継いだ来歴 ----
///
...
int swval[2048];           ///< case のラベル値 (範囲つきなら下端)
int swhi[2048];            ///< 範囲つき case の上端 (その 27。普通の
                           ///< case では swval と同じ値が入る)
...
int k_typeof;              ///< typeof / __typeof__ (その 26。docs/stage018-ext.md 9)
...
int lexid() {
  ...
  // GNU の別綴り (docs/stage018-ext.md 3.1)。**意味は素の綴りと同じ**で
  // ある。ヘッダは `#if __STRICT_ANSI__` の側でこちらを使うので，
  // 綴りを知らないと「型指定子のはずの位置に識別子がある」に化ける。
  //
  // restrict は C99 の予約語だが，我々は const / volatile と同じく
  // **読み飛ばす修飾子**として扱う。最適化に使わないので意味は変わらない
  if (streq(tname, "typeof")) { tok = k_typeof; return 0; }
  if (streq(tname, "__typeof__")) { tok = k_typeof; return 0; }
  if (streq(tname, "__typeof")) { tok = k_typeof; return 0; }
  if (streq(tname, "__inline__")) { tok = k_auto; return 0; }
  if (streq(tname, "__inline")) { tok = k_auto; return 0; }
  if (streq(tname, "__const__")) { tok = k_const; return 0; }
  if (streq(tname, "__const")) { tok = k_const; return 0; }
  if (streq(tname, "__volatile__")) { tok = k_volatile; return 0; }
  if (streq(tname, "__volatile")) { tok = k_volatile; return 0; }
  if (streq(tname, "__signed__")) { tok = k_signed; return 0; }
  if (streq(tname, "__signed")) { tok = k_signed; return 0; }
  if (streq(tname, "restrict")) { tok = k_const; return 0; }
  if (streq(tname, "__restrict__")) { tok = k_const; return 0; }
  if (streq(tname, "__restrict")) { tok = k_const; return 0; }
...
int next() {
  ...
  if (isidh(c)) {
    ...
    // __extension__ は「この先で拡張を使う。診断を出すな」という印で
    // あり，中身を持たない。我々は診断を出さないので落とすだけでよい
    // (docs/stage018-ext.md 3.1)
    if (tok == t_id && streq(tname, "__extension__")) {
      next();
      return 0;
    }
...
/// @brief 式の型だけを求める (typeof の中身。docs/stage018-ext.md 9)。
/// @return 型番号
/// @note **我々の器は読みながらコードを出す。** 型だけが要る場所では
///       出したものを捨てる —— IR (icnt)・ラベル (labcnt)・文字列
///       (spcnt) の位置を控えて戻す。式そのものは普通に読むので，
///       型の決め方は式の道と 1 か所も違わない。
///
///       配列は退化させない。`typeof(a)` の a が配列なら型も配列である。
int typeexpr() {
  int si; int sl; int ss; int t;
  si = icnt;
  sl = labcnt;
  ss = spcnt;
  assign();
...
int ptype() {
  ...
  if (tok == k_void) { next(); k = t_void; }
    ...
    k = 1;
  } else if (tok == k_typeof) {
    // typeof(式) / typeof(型) (GNU C。docs/stage018-ext.md 9)。
    // 式のほうは**コードを出さずに型だけ**が要る。我々の器は読みながら
    // 出すので，出したものを捨てる形で読む (typeexpr)
    next();
    if (tok != o_lp) exit(1);
    next();
    if (istype()) k = pstars(ptype());
    else k = typeexpr();
    if (tok != o_rp) exit(1);
    next();
  } else if (tok == t_id) {
    ...
    while (1) {
      ...
      // 修飾子は宣言指定子の列のどこに来てもよい (unsigned const char *)。
      // 先頭の const だけを読んでいたので，型指定子が始まった後の
      // const / volatile でこの環を抜けてしまっていた
      else if (tok == k_const || tok == k_volatile) { next(); }
...
/// @brief 配列型を最後まで畳んだときの要素型 (スカラ) を返す (cc15s)。
/// @note 波括弧を省いた並びを読むときに要る。**畳まずに読むための
///       宣言どおりの要素型 (aelem) と対にして使う** —— 初期化子は
///       要素ごとにどちらの書き方でもよい (C89 6.5.7)。
int flatel(int t) {
  int e;
  e = aelem[t - t_arr];
  while (isarr(e)) e = aelem[e - t_arr];
  return e;
}
...
int fnpdec(int b) {
  ...
  copyn(fpnam, tname);
  next();
  // **関数ポインタの配列** (int (*tab[3])(int, int))。宣言子は内側から
  // 読むので，これは「関数へのポインタの配列」である。名前の直後に
  // [n] が並ぶ (cc15ab で足した)。GCC の hooks はこの形で並ぶ
  nd = 0;
  while (tok == o_lb) {
    if (nd > 7) exit(6);
    next();
    dims[nd] = 0;
    if (tok != o_rb) dims[nd] = ccond();
    if (tok != o_rb) exit(1);
    next();
    nd = nd + 1;
  }
  ...
  t = 65536 | ftype(b);
  // 内側の次元から組む (pdims の再帰と同じ順)
  while (nd > 0) { nd = nd - 1; t = atype(t, dims[nd]); }
  return t;
}
...
/// @return 値番号
/// @brief GNU の文式 ({ 文...; 式; }) を解析する (docs/stage018-ext.md 7)。
/// @return 値番号 (最後の式文の値)
/// @note 呼ばれた時点で '(' は読み終えており，現在のトークンは '{' である。
///       名前の見える範囲は複文と同じで，抜けると消える。
///
///       **最後が式文でなければ値が無い。** GNU C は「その場合の値は
///       void」と定めるが，我々がここへ来るのは値の要る場所だけなので，
///       黙って前の式の値を返すより拒む (exit 5)。
int estmtx() {
  int n; int b0; int d; int v; int t; int hi; int lv; int last; int k;
  n = lcnt;
  b0 = cloff;
  d = lblk;
  lblk = lcnt;
  ...
  while (tok != o_rc) {
    ...
    // 文の始まりか式の始まりか。ラベル (名前 :) は 1 つ先を見ないと
    // 式と見分けられない —— stmt() と同じ手である
    k = 0;
    if (tok == t_id) {
      lsave();
      next();
    ...
    if (k || tok == o_lc || tok == o_semi
        || tok == k_if || tok == k_while || tok == k_for || tok == k_do
        || tok == k_switch || tok == k_case || tok == k_default
        || tok == k_break || tok == k_continue || tok == k_goto
        || tok == k_return) {
      stmt();
      last = 0;
      continue;
    }
    // 式文。**値を右辺値にしてから控える** —— 左辺値のまま返すと
    // 呼び手はアドレスを値として使う
    v = rvany(expr());
    t = ety;
    hi = ehi;
...
int eprim() {
  ...
  if (tok == o_lp) {
    next();
    // GNU の文式 ({ ... })。括弧つきの式と見分けるのは次の 1 文字だけ
    if (tok == o_lc) return estmtx();
    v = expr();
    if (tok != o_rp) exit(1);
    next();
    return v;
...
int ecalli(int f, int rt) {
  ...
  n = emit(c_calli, f, n);
  if (is2w(rt)) {
    // **2 語の返却 (64 bit 整数と double) の上位語を引き取る。**
    // 名前つきの呼出し (ecall) と同じ仕組みで，こちらには無かった ——
    // ehi を触らずに戻るので**直前の式の上位語が残り**，
    // `d(2.5, 4.0) * 100` が 400 になっていた (最後の引数 4.0 の上位語と
    // 返却値の下位語が混ざる)。値が壊れる類の誤りで，差分試験の
    // funcptr が見つけた (cc15ab で直した)
    iret[n] = frame1(4);
    ehi = emit(c_loadw, emit(c_laddr, iret[n], 0), 0);
  } else {
    ehi = 0;
  }
  ety = rt;
...
int epost() {
  ...
  while (tok == o_lb || tok == o_dot || tok == o_arrow
    ...
    if (tok == o_lb) {
      ...
      if (tok != o_rb) exit(1);
      next();
      // **E1[E2] は *(E1 + E2) である** (C89 6.3.2.1)。足し算に順序は
      // 無いので `3[p]` も `p[3]` と同じ意味になる。cc15aa までは左が
      // ポインタであることを要求していた (cc15ab で足した)
      if ((lpt >> 16) == 0) {
        if ((ipt >> 16) == 0) exit(5);
        tsw = v; v = i; i = tsw;
        pt = ipt;
...
int assign() {
  ...
  // **複合代入も二項演算と同じ規則で符号を見る。** a op= b は
  // a = a op b と同じ意味なので (C89 6.3.16.2)，除算・剰余・右シフトの
  // 命令の選び方も同じでなければならない。トークンから来る op は
  // 符号つきの種別 (b_div / b_rem / b_srl) に並んでいるだけで，
  // 符号を見ていなかった —— 二項の道 (o_div / o_shr) は見ている。
  //
  //   unsigned int a; a %= 65521;   符号つき剰余になっていた
  //   int a;          a >>= 1;      論理シフトになっていた
  //
  // どちらも**通ってしまうが値が違う**。前者は zlib の adler32 が
...
int stmt() {
  ...
  if (tok == o_lc) {
    ...
    // **宣言は文の後ろにも置ける** (C99 6.8.2。docs/stage018-ext.md 3.2)。
    // C89 は複文の先頭にまとめることを要求するが，実物のソースは
    // 「使う直前で宣言する」書き方をする。読む順に 1 つの環で受ける。
    //
    // 見分けは istype() ただ 1 つである。**typedef 名と同じ名前の
    // 局所変数を作ると取り違える** —— C の構文が元から持つ曖昧さで，
    // 我々はまだ解いていない (docs/stage018-ext.md 3.2 の「残る穴」)。
    // 先頭にまとめる書き方でも同じ曖昧さはあったので，新しく生じた
    // 穴ではなく，当たる場所が増えただけである
    while (tok != o_rc) {
      if (istype()) plocal();
      else stmt();
    }
  ...
  if (tok == k_for) {
    ...
    if (tok != o_lp) exit(1);
    next();
    // **初期化の場所で宣言できる** (C99 6.8.5.3。docs/stage018-ext.md 3.3)。
    // 名前は for を抜けると見えなくなるので，複文と同じに退避して戻す。
    // plocal は末尾の ';' まで食べるので，宣言のときだけ道を分ける
    n = lcnt;
    b0 = cloff;
    d = lblk;
    if (istype()) {
  ...
  if (tok == k_switch) {
    ...
    i = b0;
    while (i < swn) {
      n = emit(c_loadw, emit(c_laddr, s, 0), 0);
      if (swhi[i] == swval[i]) {
        c = emit(c_bin + b_seq, n, emit(c_const, swval[i], 0));
      } else {
        // **範囲つき case** (`case lo ... hi:`。GNU C。
        // docs/stage018-ext.md 11)。下端と上端の 2 つを見る。
        // 下端が上端より大きい範囲は空で，この形のまま偽になる
        w = emit(c_bin + b_sge, n, emit(c_const, swval[i], 0));
        k = emit(c_bin + b_sle, n, emit(c_const, swhi[i], 0));
        c = emit(c_bin + b_and, w, k);
      }
  ...
  if (tok == k_case) {
    ...
    next();
    n = caseval();
    // 範囲つき case (GNU C)。`case lo ... hi:` の 3 点は 1 つのトークン
    // (可変長引数の '...' と同じもの) として来る
    k = n;
    if (tok == o_ellip) {
      next();
...
int emitins(int i) {
  ...
  if (k == c_calli) {
    ...
    if (iret[i]) {
      // **2 語の返却 (64 bit 整数と double)。** 上位語が 1 語だけ 1 つ上に
      // 積まれている。c_call には同じ引取りがあったが，こちらには無く，
      // 上位語を積んだまま 1 語しか捨てていなかった —— 値が壊れるうえに
      // データスタックがずれる。差分試験の funcptr が見つけた
      // (cc15ab で直した)
      outw(iw3(0x2003, 10, 9, 4));
      if (iret[i] < 2048) outw(sw3(0x2023, 8, 10, iret[i]));
      else { laddr(11, iret[i]); outw(sw3(0x2023, 11, 10, 0)); }
...
int memb(int se, int t, int fp) {
  ...
  // 名前の読取りと pdims は行わない
  // **無名のビットフィールド** (unsigned : 5; / unsigned : 0;)。詰め物
  // として使う形で，C89 6.5.2.1 が定めている。cc15aa までは名前が無い
  // 時点で拒んでいた (cc15ab で足した)
  anon = 0;
  if (!fp) {
    if (tok == o_col) anon = 1;
    else if (tok != t_id) exit(1);
  }
  if (!anon && mfind(se) >= 0) exit(4);
  if (mcnt > 16383) exit(6);
  m = mcnt;
  mcnt = mcnt + 1;
  msid[m] = se;
  // 無名は**引けない名前**で入れる (名前で探す側は空文字と一致しない)
  if (anon) mname[m * 64] = 0;
  else copyn(mname + m * 64, tname);
  mty[m] = t;
  if (!fp) {
    if (!anon) next();
    if (tok == o_col) {
      ...
      if (tval == 0) {
        ...
        // (C89 6.5.2.1)。メンバは作らず，詰めの位置だけを戻す
        if (!anon) exit(2);
        next();
        u = tsize(t);
        ...
        mcnt = mcnt - 1;
        bfword = 0 - 1;
        if (!sunion[se]) ssize[se] = roundup(ssize[se], u);
        return 0;
      }
      if (tval > 31) exit(2);
      if ((t >> 16) != 0) exit(2);
...
int linit(int i) {
  ...
  if ((t >> 16) == 0 && isstru(t) && tok == o_lc) {
    ...
    m = 0;
    pe = 0;
    // **指示つき初期化子** (.name = v。docs/stage018-ext.md 3.5)。
    // 全体を 0 で埋めた後なので，位置を選ぶだけでよい。指示子が無ければ
    // 従来どおり宣言順に埋める (出る IR も同じ)
    while (tok != o_rc) {
      if (tok == o_dot) {
        next();
        if (tok != t_id) exit(1);
        ...
        if (m < 0) exit(2);
        next();
        if (tok == o_dot || tok == o_lb) exit(5);
        if (tok != o_asn) exit(1);
        next();
        pe = 0;
      } else {
        while (m < mcnt && !(msid[m] == t - 2 && moff[m] >= pe)) m = m + 1;
        if (m >= mcnt) exit(6);
      }
      {
  ...
  if (isarr(t)) {
    ...
    hw = 0;
    while (tok != o_rc) {
      // **指示つき初期化子** ([k] = v)。前へ跳ぶなら隙間をその場で 0 に
      // する —— 末尾の 0 埋めは「書いた最後尾から先」しか見ないので，
      // 跳んだ手前が埋まらないまま残る (docs/stage018-ext.md 3.5)
      if (tok == o_lb) {
        next();
        k = ccond();
        if (tok != o_rb) exit(1);
        ...
        if (k < 0) exit(6);
        while (hw < k) {
          a = emit(c_bin + b_add, emit(c_laddr, loff[i], 0), emit(c_const, hw * w, 0));
...
int funcdef() {
  ...
  spcnt = 0;
  hcnt = 0;
  // 制御構造の状態も関数ごとに作り直す。**宣言より前に置く** ——
  // 本体の最外は宣言と文が混ざるので (C99 6.8.2)，文がいつ来ても
  // よい状態にしてから環へ入る (docs/stage018-ext.md 3.2)
  brkn = 0;
  conn = 0;
  swn = 0;
  swdep = 0;
...
/// @brief 静的な初期化子で溜めているビットフィールドの記憶単位。
/// @note 位置は語の中のビットなので，メンバごとに語を書くと隣を壊す。
///       cc15aa まではそうなっていて，`static struct a g = {1,0,5,200};`
///       の 3 つめ以降が別の語へ書かれていた (cc15ab で直した)
int gbfo;                 ///< 溜めている単位の位置 (-1 = 溜めていない)
int gbfv;                 ///< 溜めた値
int gbfu;                 ///< 単位の大きさ (バイト)

/// @brief 溜めたビットフィールドの記憶単位を書き出す。
/// @param base 構造体の先頭の出力位置
int gbfflush(int base) {
  int i;
  if (gbfo < 0) return 0;
  outp = base + gbfo;
  i = 0;
  while (i < gbfu) { outbyte((gbfv >> (i * 8)) & 255); i = i + 1; }
  gbfo = 0 - 1;
  return 0;
}
...
int ginit1(int t) {
  ...
  int se; int sf; int sk; int m; int base; int i;
  int hw;                        // 指示つき初期化子: 書いた最後尾 (base から)
  w = tsize(t);
  ...
  if (tok == o_lc) {
    next();
    if (isstru(t)) {
      // **指示つき初期化子** (.name = v。C99 6.7.8。docs/stage018-ext.md 3.5)。
      // 指示子が無ければ宣言順に埋める従来どおりの道で，出るバイト列も
      // 1 バイト変わらない。指示子があれば「書く位置」をそのメンバへ
      // 移す。前へ跳ぶ隙間は 0 で埋め，後ろへ戻るときは書いた最後尾
      // (hw) を覚えておいて，最後にそこまで戻す
      se = t - 2;
      base = outp;
      hw = 0;
      m = 0;
      // 溜め場は空から始める。入れ子へ降りる前には必ず書き出すので，
      // ここで消しても外側の溜めを失うことはない
      gbfo = 0 - 1;
      while (tok != o_rc) {
        if (tok == o_dot) {
          next();
          ...
          if (m < 0) exit(2);
          next();
          // 連ねた指示子 (.a.b = v / [k].f = v) は**受けない**。
          // 黙って別の場所を埋めるより拒むほうがよい
          if (tok == o_dot || tok == o_lb) exit(5);
          if (tok != o_asn) exit(1);
          next();
          // **戻るときに 0 で詰めてはいけない。** 詰めるのは「まだ
          // 誰も書いていない場所」だけである。書いた最後尾 (hw) より
          // 手前は既に値が入っているので，位置を移すだけにする ——
          // ここを詰めていたので `{ .b = 2, .a = 1, .c = 3 }` の b が
          // 0 に戻っていた
          if (outp - base > hw) hw = outp - base;
          if (moff[m] > hw) {
            outp = base + hw;
          ...
          while (m < mcnt && msid[m] != se) m = m + 1;
          if (m >= mcnt) exit(6);
          while (outp - base < moff[m]) outbyte(0);
        }
        if (mbfw[m]) {
          // ビットフィールドは**記憶単位へ溜めてから書く**
          if (gbfo >= 0 && gbfo != moff[m]) gbfflush(base);
          if (gbfo < 0) { gbfo = moff[m]; gbfv = 0; gbfu = msz[m]; }
          gbfv = gbfv | ((ccond() & ((1 << mbfw[m]) - 1)) << mbfo[m]);
        } else {
          // 普通のメンバが来たら，溜めている単位を先に書き出す
          if (gbfo >= 0) gbfflush(base);
          ginit1(mty[m]);
        }
        if (outp - base > hw) hw = outp - base;
        m = m + 1;
      ...
      if (tok == t_str && ischararr(t)) {
        ...
        next();
        // 末尾のコンマを許す。**上の道と同じ規則にする** ——
        // 同じ規則を書く場所が 2 つあることを数え落とすのが，この
        // 直しで何度も踏んだ形である (33.6)
        if (tok == o_comma) next();
      } else {
        // **ここでも要素ごとに決める。** ginit と同じ規則である ——
        // `{` か「文字型配列に対する文字列」なら宣言どおりの要素型
        // (se) で，そうでなければ最後まで畳んだスカラ型 (sf) で読む。
        //
        // ginit の側だけ直して**ここを直さないと，1 段深いところで
        // 同じことが起きる** ——
        // `char a[1][2][2] = {{'a','b','c','d'}}` の内側の 4 つが
        // char[2] の幅で書かれ，8 バイト出て弾かれる (33.6)
        se = aelem[t - t_arr];
        sf = flatel(t);
        sk = tsize(se);
        hw = 0;
        while (tok != o_rc) {
          // **指示つき初期化子** ([k] = v)。添字は宣言どおりの要素型で
          // 数える (docs/stage018-ext.md 3.5)
          if (tok == o_lb) {
            next();
            i = ccond();
            if (tok != o_rb) exit(1);
            ...
            if (i * sk > hw) {
              outp = base + hw;
              while (outp - base < i * sk) outbyte(0);
            } else {
              outp = base + i * sk;
            }
          }
          if ((outp - base) % sk == 0
              && (tok == o_lc || (tok == t_str && ischararr(se))))
            ginit1(se);
          else
            ginit1(sf);
          // **この対象の幅で止める。** 外側が持つ総量で数えるだけでは
          // 足りない —— `int a[2][2] = {{1,2,3}}` は内側が溢れているが
          // 外側にはまだ余りがあるので，外側の検査を素通りして
          // **3 が 2 行目に入る**。溢れた側ではなく別の場所が埋まる
          // という，いちばん判りにくい壊れ方である (33.6)
          if (outp - base > w) exit(6);
          if (tok == o_comma) next();
          else if (tok != o_rc) exit(1);
        }
        if (outp - base < hw) outp = base + hw;
      ...
      // 「スカラの初期化子は，波括弧で囲んでよいが，単一の式でなければ
      // ならない」。`{2,3}` のように 2 つ以上あるのは制約違反である。
      //
      // ここは元々「多次元を平らにした列の一部を透過的に読む」道
      // だった。cc15s が ginit / ginit1 の側で要素ごとに対象を決める
      // ようにしたので，**この道へ来るのは対象が本当にスカラのとき
      // だけ**になった。透過的に読み続ける理由はもう無い。
      //
      // 数え落とすと **溢れた分が隣の要素に入る** ——
      // `int a[2][2] = { 1, {2,3}, 4 }` が `1 2 3 4` になっていた。
      // ホストの gcc は診断を 2 つ出したうえで `1 2 4 0` にする
      // (3 を捨て，4 を次の行の先頭へ)。**制約違反に対して黙って
      // 値を作っていたのは我々だけ**である。
      //
      // staticstr3 と同じ壊れ方なので，同じに 6 で拒む
      // (tools/diff17.sh がこの食い違いを見つけた)
      base = outp;
      while (tok != o_rc) {
        ginit1(t);
        if (outp - base > w) exit(6);
...
int ginit(int e) {
  int t; int n; int i; int es; int esd; int flat; int ibase; int k;
  int tot; int used;
  int hw;                        // 指示つき初期化子: 書いた最後尾 (ibase から)
  next();
  align4();
  gval[e] = outp;
  gdef[e] = 1;
  t = gty[e];
  if (isarr(t)) {
    esd = aelem[t - t_arr];             // 宣言どおりの要素型 (畳む前)
    es = esd;
    n = acnt[t - t_arr];
    ...
    if (tok == o_lc && ischararr(t)) {
      ...
      if (k) {
        ...
        next();
        // 末尾のコンマを許す (C89 6.5.7)。下の一般の道は
        // `while (tok != o_rc)` で自然に許しているので，ここだけ
        // 拒むと `char a[8] = { "ab", };` が通らない
        if (tok == o_comma) next();
        if (tok != o_rc) exit(1);
        next();
        while (outp & 3) outbyte(0);
    ...
    // **畳んだ型 (es) は「波括弧を省いた並び」を読むためのもので，
    // 宣言どおりの型 (esd) は「下位の対象そのもの」を読むために要る。
    // 両方を持ち，要素ごとに使い分ける** (33 章)
    // **es は必ずスカラまで畳む。** 外側の数を書いていない
    // (`int a[][2]`) ときも畳む —— 数えられないのは要素数 n だけで，
    // 「平らな並びの 1 つ」を読む型は決まっている。cc15r は
    // ここで exit(1) しており，`char m[][8]` を受け付けなかった
    flat = 0;
    while (isarr(es)) {
      if (n != 0) n = n * acnt[es - t_arr];
      es = aelem[es - t_arr];
      flat = 1;
    }
    if (tok == t_str) {
    ...
    next();
    ibase = outp;
    // 宣言どおりの総バイト数。外側の数を書いていなければ 0 で，
    // 読み終えてから決める
    tot = 0;
    if (acnt[t - t_arr] != 0) tot = tsize(t);
    // **要素ごとに，どちらの型で読むかを決める。**
    //
    //   `{` で始まる     … 下位の対象そのものを初期化している。esd で読む
    //   文字列で，esd が文字型の配列 … 同上 (`char m[2][8] = {"ab","cd"}`)
    //   それ以外         … 波括弧を省いた平らな並びの 1 つ。es で読む
    //
    // **1 つ目の形で全体を決めてはいけない。** C は下位の波括弧を
    // 途中から省いてよく (`int a[2][2] = {{1,2}, 3, 4}`)，全体を
    // 1 つ目で決めると後ろの数が配列の幅で書かれてしまう
    k = tsize(esd);
    hw = 0;
    while (tok != o_rc) {
      // **指示つき初期化子** ([k] = v。C99 6.7.8。docs/stage018-ext.md 3.5)。
      // 添字は宣言どおりの要素 (esd) で数える。前へ跳ぶ隙間は 0 で
      // 埋め，後ろへ戻るときは書いた最後尾 (hw) を覚えて最後に戻す
      if (tok == o_lb) {
        next();
        i = ccond();
        if (tok != o_rb) exit(1);
        ...
        next();
        if (i < 0) exit(6);
        if (outp - ibase > hw) hw = outp - ibase;
        if (i * k > hw) {
          outp = ibase + hw;
          while (outp - ibase < i * k) outbyte(0);
        } else {
          outp = ibase + i * k;
        }
      }
      // **いま埋めている場所が要素の切れ目にあるかで決める。**
      //
      // 波括弧は「今から埋める対象そのもの」を囲む。要素の切れ目に
      // 居るなら対象は要素 (esd) だが，要素の途中に居るなら対象は
      // その中のスカラである。`int a[2][2] = {1, {2}, 3, 4}` の
      // `{2}` は a[0][1] を埋めるのであって，行を埋めるのではない。
      //
      // **トークンだけを見て決めてはいけない。** 見るのは
      // 「トークン」と「位置」の 2 つである
      if ((outp - ibase) % k == 0
          && (tok == o_lc || (tok == t_str && ischararr(esd))))
        ginit1(esd);
      else
        ginit1(es);
      if (tok == o_comma) next();
    ...
    next();
    if (outp - ibase > hw) hw = outp - ibase;
    if (outp - ibase < hw) outp = ibase + hw;
    used = hw;
    // 外側の数を書いていなければ，出たバイト数から決める。要素の
    // 途中で終わっていたら 1 要素ぶんへ切り上げる
    if (tot == 0) {
      k = tsize(esd);
      tot = ((used + k - 1) / k) * k;
    }
    if (used > tot) exit(6);
    // 足りない分は 0 で埋める。**バイトで数える** —— 要素の大きさが
    // 1 / 2 / 4 とは限らない (char[8] のような要素が来る)
    while (used < tot) { outbyte(0); used = used + 1; }
    while (outp & 3) outbyte(0);
    if (acnt[t - t_arr] == 0) {
      gty[e] = atype(esd, tot / tsize(esd));
      gsz[e] = tot;
    } else if (!flat) {
      gty[e] = atype(es, n);
      gsz[e] = n * tsize(es);
