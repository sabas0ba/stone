...
///
/// ## 文字列リテラル関連の不具合の，最後の 1 つ
///
/// `static char m[2][8] = { "ab", "cd" };` が壊れていた。**通ってしまうが
/// 値が違う** —— 台帳で bad と呼ぶ状態である (tests/stage015 の staticstr2)。
///
/// `ginit()` は多次元を平坦化してから要素を読む。平坦化した後の要素型は
/// char であって配列ではないので，`cc15q` が `ginit1()` に入れた
/// 「対象が配列なら実体を並べる」が適用されない。要素 1 つに 4 バイトの
/// 再配置が書かれて中身がアドレスになっていた。
///
/// ## 直し: 平坦化してよいかを，初期化子の書かれ方で決める
///
/// 平坦化するのは `int a[2][2] = {1,2,3,4}` のような書き方のための
/// 簡略化である。`{"ab","cd"}` の波括弧の中の 1 つ 1 つは **行 (char[8]) を
/// 初期化する**のであって，平坦化した後の要素 (char) を初期化するのでは
/// ない。
///
/// 見分け方は「`{` の次が `{` か文字列か」——そうなら下位の対象そのものを
/// 初期化しているので**平坦化しない**。そうでなければ平坦な並びなので今まで
/// どおり平坦化する。**平坦化する経路のバイト列は 1 バイトも変わらない。**
///
/// 平坦化しなくなった分，`ginit1()` に**配列を波括弧で受ける経路**が要る。
/// 要素型で読み，宣言した幅に足りない分を 0 で埋める。`{ "ab" }` の形は
/// 波括弧が無いときと同じに実体を並べる (C89 6.5.7)。
///
/// **決めるのは要素ごとである。** 宣言どおりの要素型 (esd) と最後まで
/// 平坦化したスカラ型 (es) の両方を持ち，`{` か「文字型配列に対する文字列」
...
/// | 5 | `char b[8] = { "ab" }` | **誤** | 正 |
/// | 6 | `char p[8] = "ab"` | 正 | 正 |
/// | 7 | `int a[2][2] = {1,2,3,4}` (平坦化する経路) | 正 | 正 |
...
/// いるのに外側に余りがあると，総量の検査で検出されずに**溢れた分が別の
...
///
/// **1 つの形で確かめて同種の形すべてが正しいと思ったのが 3 度の誤りの元
/// だった** (docs/stage017-cc.md 27.4 / 31.4)。4 度目は測ってから書いたが，
/// **測る形を自分で選んでいた**。
///
/// **ビルドチェーンの成果物は変わらない。** 既存のソースに多次元の char 配列を文字列で
...
/// を見なかった。docs/stage017-cc.md 27.4 に書いた「同類とみなす範囲を
/// 言語の規則ではなく振る舞いで決めた」を，**3 度目に繰り返した**ことになる。
/// 今回は規則の側 —— 「文字列リテラルの型は char[n+1] である」——
/// から直している。
///
/// ## 表に出た形
///
/// tcc が stone の OS の上で書庫 (.a) を読めなかった。tccelf.c は
///
...
///
/// **ビルドチェーンの成果物は変わらない** —— 既存のソースは文字列リテラルの
...
///
/// どちらも「静的な集成体の初期化子に文字列リテラルが出てくると値が壊れる」
...
/// ginit() が多次元を平坦化した後は要素型が char になっていて，
/// 「配列」として見えなくなるためである (27 章)。
///
/// **ビルドチェーンの成果物は 1 バイトも変わらない。** 既存のソースはどちらの形も
...
///
/// @section struval 構造体の値は「アドレス」で受け渡す
/// 式の値は 1 語 (レジスタ 1 本) に収まるという前提で IR を組んでいる。
/// 構造体はそこに収まらないので，**構造体型の式の値はその実体のアドレス**
...
/// 名前つき引数を後ろから 1 語ずつ取り出してフレームへ移す。単純に
...
/// そのまま置く**。ベアメタルで MMU が無く，イメージはフラットに読み込まれるので
...
/// ことをブートストラップ 1 段目と 2 段目の一致で保証してきたが，本段では
...
/// 求めて 2 回使う」を単純に書ける (docs/stage010-c89.md 4.2)。
///
...
int nsta;                 ///< ローカル側へ集約した static 記号の数
int gcnt;                 ///< 大域記号の登録数
...
int fpexd;                ///< 仮数に入り切らず読み捨てた整数部の桁数
                          ///< (10 の冪で lexfp の指数へ戻す)
int coty;                 ///< 定数アドレス式 (cofs) が畳み込んだ式の型
...
int next() {
  ...
  // 小数点から始まる浮動小数点定数 (`.0001`)。C89 3.1.3.1 の
  // fractional-constant は
  //
  //   digit-sequence_opt . digit-sequence
  //
  // で，**整数部は省ける**。次が数字なら数として読む —— そうでなければ
  // 構造体のメンバの '.' なので触らない。lexnum() は整数部を 0 桁読んでから
  // '.' を見て lexfp() へ渡すので，そのまま通せばよい
  if (c == '.' && isdig(src[pos + 1])) return lexnum();
  if (isidh(c)) {
    lexid();
    // __attribute__ (( ... )) は字句の段ですべて捨てる。宣言の前にも
...
/// @brief シンボルのアドレスを lui + addi でレジスタへ置き，再配置を作成する。
/// @param d 書込み先レジスタ番号
...
// この表現なら「* を 1 つ付加/除去する」が 65536 の加減算になり，
...
///       「配列へのポインタ」になり，添字を 1 つ進めるたびに 1 段ずつ除去される。
...
/// @brief 配列型を最後まで平坦化したときの要素型 (スカラ) を返す (cc15s)。
/// @note 波括弧を省いた並びを読むときに要る。**平坦化せずに読むための
...
/// @param b    返却型
/// @param anon 1 = 名前を省略した「( * ) ( 仮引数 )」も受ける
/// @return 関数へのポインタの型。名前は fpnam に入る (省略時は空)
/// @note 仮引数の型は持たないので，括弧の対応だけ数えて読み飛ばす。
///       呼出しの検査は名前つきの関数と違って個数も見ない。
int fnpdec1(int b, int anon) {
  int n;
  next();
  if (tok != o_mul) exit(1);
  next();
  // * の後ろの型修飾子を読み飛ばす。C89 6.5.4.1 の pointer は
  // 「* type-qualifier-list_opt」で，`int (*const f)(int)` は妥当である。
  //
  // **括弧の外では pstars() が既に同じことをしている** (2085 行)。
  // 括弧の中の宣言子は pstars を通らないので，ここだけが持っていなかった。
...
/// @brief 名前つきの関数ポインタの宣言子だけを受ける。
/// @note 仮引数並び以外はすべてこちら。名前を省いた形は宣言子として
///       意味を持たないので，受けてしまうと記号の無い宣言が通る。
int fnpdec(int b) { return fnpdec1(b, 0); }

// ---- 宣言子の定数式 ----
//
// 配列の大きさ [n] に書ける整数定数式。値をその場で畳み込んで返す。
...
/// @brief 定数アドレス式のオフセットを畳み込む (offsetof の展開の中身)。
/// @return 先頭からのバイト数。畳み込んだ式の型は coty に残す
...
int cuna() {
  ...
  if (tok == o_lp) {
    next();
    if (istype()) {
      // 定数式のキャスト。幅の狭い整数型だけ値を切り詰め，あとはそのまま
      t = pstars(ptype());
      if (tok != o_rp) exit(1);
      next();
      v = cuna();
...
int cbin(int minp) {
  ...
  while (cprec(tok) >= minp && cprec(tok) > 0) {
    ...
    lft = ccll;
    w = cbin(p + 1);
    // 64 bit リテラルを含む二項の定数式はまだ畳み込めない。黙って 32 bit で
    // 畳み込むと値が壊れるので拒む (終了コード 5)
    if (lft || ccll) exit(5);
...
int euna() {
  ...
  if (tok == o_lp) {
    ...
    if (istype()) {
      ...
      // 型名の中の関数ポインタ「( * ) ( 仮引数 )」。
      //
      // **C89 6.5.5 の型名は識別子を含まない。** 仮引数並び (pparam) と
      // 同じ宣言子を読むが，名前が書いてあれば型名ではないので拒む。
      // 型は「関数へのポインタ」—— 値の幅は 4 なので narrow は恒等変換に
      // なり，出るコードは今までのポインタ間のキャストと変わらない
      if (isfnp()) {
        t = fnpdec1(t, 1);
        if (fpnam[0] != 0) exit(1);
      }
...
int econd() {
  ...
  //
  // **ただし空ポインタ定数は別である** (C89 6.3.15) ——
  //
  //   一方の被演算子が空ポインタ定数であれば，結果は他方の型を持つ。
  //
  // 順序に依存しない規則なので，then 側から取るだけでは
  // `(k ? 0 : p)` が int になる。`->` を直に当てたときだけ表に出る
  // (代入や返却は左辺の型へ変換する経路を通るので隠れる)。
  //
  // 空ポインタ定数かどうかは IR で見る —— then 側の値が定数 0 の命令で
  // あればよい。`(void *)0` は既にポインタ型なのでここへは来ない。
  // 逆向き (then がポインタ・else が 0) は then 側を採るのがそのまま
  // 正しいので何もしない
  if ((t >> 16) == 0 && (ety >> 16) != 0
...
int assign() {
  ...
  // 符号を見ていなかった —— 二項演算の経路 (o_div / o_shr) は見ている。
  //
...
// fold が定数を畳み込むと使われなくなる値が出るので，dce はその後に置く。
...
///       順に畳み込んでいけば，オペランドは自分の番が来る前に確定している。
int fold() {
  int i;
  i = 0;
  while (i < icnt) {
...
// スピルされた値は読むときに x10/x11 へロードし，書いたら即座に書き戻す
...
int dstore(int v) {
  ...
  if (o < 2048) outw(sw3(0x2023, 8, 10, o));
    ...
    // x11 のオペランドは結果を作った時点で用済みなので上書きしてよい
    laddr(11, o);
...
///       条件分岐を B-type で直接出力していたが，B-type の変位は ±4KiB しか
...
int pparam() {
  ...
  if (isfnp()) {
    // 仮引数の関数ポインタ
    b = fnpdec1(b, 1);
    // プロトタイプの抽象宣言子「( * ) ( 仮引数 )」。名前が無いので
    // 記号は作らず，占める語だけ数える (関数ポインタは 1 語)
    if (fpnam[0] == 0) {
      cna = cna + 1;
      return 0;
    }
...
int ginit1(int t) {
  ...
  if (tok == o_lc) {
    ...
    if (isstru(t)) {
      ...
      // **配列そのものを波括弧で受ける** (cc15s)。ginit が平坦化しなく
      ...
      //
      // ここは元々「多次元を平坦化した列の一部を透過的に読む」経路
      // だった。cc15s が ginit / ginit1 の側で要素ごとに対象を決める
      // ようにしたので，**この経路へ来るのは対象が本当にスカラのとき
...
int ginit(int e) {
  ...
  if (isarr(t)) {
    ...
    // 多次元は平坦に並べる。要素型が配列でなくなるまで平坦化しておくと，
    // 以降は 1 次元と同じ扱いで済む。平坦化した場合は型と大きさが宣言で
    // 決まっているので，読み終えた後に書き直してはならない。
    // **平坦化した型 (es) は「波括弧を省いた並び」を読むためのもので，
    // 宣言どおりの型 (esd) は「下位の対象そのもの」を読むために要る。
    // 両方を持ち，要素ごとに使い分ける** (33 章)
    // **es は必ずスカラまで平坦化する。** 外側の数を書いていない
    // (`int a[][2]`) ときも平坦化する —— 数えられないのは要素数 n だけで，
...
int typedef1() {
  ...
  if (tok == t_id) next();
  // 配列型の typedef (typedef char t[4]; の形)。**ふつうの宣言と同じ
  // pdims() で後置の [n] を読む。** ここだけが宣言子の後置を読んで
  // いなかったので，名前の次が '[' だと o_semi の検査に落ちて 1 に
  // なっていた (docs/stage017-gcc.md 8.3 の 10)。
  //
  // 関数型の typedef は名前の次が '(' なので pdims は何もしない
  b = pdims(b);
...
///       大域の番号を書く決まりである。static の記号はローカル側へ集約する。
