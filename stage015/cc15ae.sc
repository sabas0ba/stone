...
///
/// `libiberty/hashtab` には**壁が 2 つあった**。1 つめは 1691 行の
...
///
/// ## GCC 4.7.4 を測って残っていた C89 適合の穴 —— 5 つ全部
///
...
///
/// ## 差分試験がコンパイラ側で見つけた 5 つの穴
///
/// どれも **C89 の内側**にある。GCC の C を stone の器へ入れる
/// (docs/stage017-gcc.md 6.1 の 4) 前に潰しておく。見つけたのは
...
///      書いていたので，同じ単位を共有する 2 つめ以降が隣を 0 で潰して
...
///      **データスタックが 1 語ずれる**
///
/// 4 と 5 は `cc15u` と同じ族である —— 固定点もバイト一致も往復検査も
/// 捕まえない。stone のソースがどちらの形も使っていないので，
/// **自分自身を組む限り永久に表に出ない**。外を的として読ませるか，
/// ホストと突き合わせる以外に見つける道が無い。
///
...
int incdec(int a, int t, int isadd, int post) {
  ...
  // 行う** (assign の bw の枝)。cc15ac までは拒んでいた ——
...
///
///       大域の dcont へ渡す道は採れない —— 名前の次が '(' だと
///       funcdef() へ行き，lcnt / lblk を 0 に潰して**外側の関数の
...
int plocal() {
  ...
  // 効く (docs/stage017-gcc.md 8.3 の 1)
  if (tok == o_semi) { next(); return 0; }
