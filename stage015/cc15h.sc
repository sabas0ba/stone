...
int lexnum() {
  ...
  if (getch() == '.' || getch() == 'e' || getch() == 'E') {
    if (ov) exit(5);         // 整数部だけで 64 bit を超える仮数は扱えない
    return lexfp();
  }
  if (ov) tvalll = 1;
...
int rvc(int v) {
  v = rvany(v);
  if (isfp(ety)) exit(5);   // -0.0 が偽にならないといけない。比較は cc15h
  if (ehi) {
...
int ecallseq(int e) {
  ...
  if (tok != o_rp) {
    ...
    while (1) {
      if (np > 31) exit(6);
      av[np] = rvany(assign());
      if (isfp(ety)) exit(5);   // 浮動小数点の実引数は cc15h で (変換先の
                                // 仮引数の型を記号表がまだ持っていない)
      ah[np] = ehi;
      aw[np] = nwords(ety);
      at[np] = ety;
...
int emul() {
  ...
  while (tok == o_mul || tok == o_div || tok == o_mod) {
    ...
    r = rvany(euna());
    rt = ety;
    if (isfp(lt) || isfp(rt)) exit(5);   // 浮動小数点の四則は cc15h で
    if (isll(lt) || isll(rt)) {
      ah = wide1(v, lh, lt);
      bh = wide1(r, ehi, rt);
      u = isull(lt) || isull(rt);
...
int eadd() {
  ...
  while (tok == o_add || tok == o_sub) {
    ...
    next();
    r = rvany(emul());
    if (isfp(lt) || isfp(ety)) exit(5);   // 浮動小数点の四則は cc15h で
    if (isll(lt) || isll(ety)) {
...
int erel() {
  ...
  while (tok == o_lt || tok == o_gt || tok == o_le || tok == o_ge) {
    ...
    next();
    r = rvany(eshift());
    if (isfp(lt) || isfp(ety)) exit(5);   // 浮動小数点の比較は cc15h で
    if (isll(lt) || isll(ety)) {
      ah = wide1(v, lh, lt);
      bh = wide1(r, ehi, ety);
...
int assign() {
  ...
  if (elv == 0) exit(5);
  if (isfp(ety)) exit(5);   // 浮動小数点の複合代入は cc15h で
...
int pparam() {
  ...
  if (isfp(b)) exit(5);   // 浮動小数点の仮引数は cc15h で
  if (isll(b)) {
...
int linit(int i) {
  ...
  else if (ehi) exit(5);       // 64 bit を狭い型の初期化子にはまだ使えない
