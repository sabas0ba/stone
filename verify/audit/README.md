...
本書は，ブートストラップのビルドチェーンが適切に成立していることを **ビルドチェーン自体に依存せずに**
...
1 と 2 も実行主体はビルドチェーンの成果物そのものである。したがって「seed (hex0.bin) と
listing が揃って間違っている」「ゴールデンと被検体が同じ誤解を共有している」
という形の誤りは，ビルドチェーンの内部だけでは原理的に検出できない。

本監査はこれを **仕様書だけから独立に書き起こした参照実装** で外側から
突き合わせることで補う。ビルドチェーン (RV32 バイナリ, QEMU 上) と参照実装 (Python,
...
| # | 検査 | 意味 |
|---|---|---|
| 1 | sha256(hex0.bin) == hex0.md 記載値 | seed の完全性 (テストと同じ照合をビルドチェーンの外で再確認) |
...
| 9 | ref_asm(asm.hex1 の #: 転記) == ref_hex1(asm.hex1) | 自己アセンブル固定点を参照実装のみで再現 (手エンコードと転記コメントの相互検証をビルドチェーンの外で閉じる) |
| 10-11 | 参照実装の出力 == tmp/build の生成物 | ビルドチェーンが実際に作った hex1.bin / asm.bin との突き合わせ (ビルド済みの場合のみ) |

## 4. 監査の範囲と限界

- 独立再導出が届くのは **Stage 1〜3 (信頼根とアセンブラまで)** である。
  Stage 4 以降を参照実装で覆うのは実装量が釣り合わない。そこから先の正しさは
  ビルドチェーンの三重検証 (ゴールデン・固定点・SHA ピン) と，Stage 6 の B1 == B2
  (sol 実装の sc と sc 実装の scc という **別実装 2 系統の出力一致**) が担う。
  信頼根さえ独立に確立できれば，以降の各段は「前段の成果物のみでビルドされる」
...
Stage 1〜3 の独立監査 (上記) に加えて，以下でビルドチェーン全体を検証した。

### 5.1 実機での全段再ビルドとテスト

```
sh tools/env.sh build        # コンテナイメージの構築と packages.lock の照合
...
| ゲスト内再生成 == ビルドチェーンの成果物 (OS 上で cc 自身を作り直す) | tests/stage013/test.sh |
