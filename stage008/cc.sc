...
///
/// 設計は docs/stage008-elf-ld.md，入力言語の仕様は docs/stage005-sc.md 2 章
/// (Stage 5 から変更なし)。Stage 7 の occ.sc を出発点とし，出力を
/// 「実行可能なフラットバイナリ」から「ELF ET_REL オブジェクト」へ変えたもので，
/// 字句解析・構文解析・IR・最適化・レジスタ割付けはそのまま引き継ぐ。
///
/// @section diff occ からの変更点
/// 1. 大域変数のアドレスをコンパイル時に決めない。.bss 内オフセットを
///    割り当てるだけにし，参照は lui + addi と HI20/LO12_I 再配置で表す
/// 2. 関数呼出しは常に変位 0 の jal として出し，JAL 再配置を付ける。
///    同一オブジェクト内で解決できる場合もリンカに委ねて処理を 1 本化する
/// 3. 文字列リテラルは .text 内に置き，無名のローカルシンボルを定義して
///    そこへ HI20/LO12_I 再配置を作成する
/// 4. ランタイム前置部は出力しない。どの翻訳単位が先頭に来るか決まらないため，
///    前置部 (crt0 相当) はリンカの仕事になった。これに伴い getc / putc / exit は
///    「既知の固定アドレス」から「未定義シンボル」へ変わる
///
/// @section pipeline 処理の流れ
/// 入力 1 本を読み切り，関数単位で以下を回してフラットバイナリを出力する。
///
///   ソース -> 字句 -> 再帰下降構文解析 -> IR 構築
///          -> fold (定数畳み込み) -> dce (不要コード削除)
///          -> regalloc (線形走査割付け) -> emitfn (命令出力)
///
/// scc までは構文解析が直接命令を出力していた (テンプレート展開) ため，
/// 式の値は常にメモリ上のデータスタックを往復していた。occ は一旦 IR に
/// 変換することで，(1) 定数式をコンパイル時に評価し，(2) 一時値をレジスタに
/// 割り付けられる。関数間 ABI は scc と同一に保つので，生成物は従来の
/// ランタイム前置部でそのまま動く。
///
/// @section why_sc なぜ sc 言語で書かれているか
/// occ 自身は sc 言語で書かれ，前段 scc でブートストラップされる
/// (B1 = scc(occ.sc))。その B1 が occ.sc を再コンパイルしたものが 2 段目
/// (B2)であり，B2 == B3 の固定点が Stage 7 の完了条件である。
/// このため occ.sc は「scc がコンパイルできる範囲」で書く必要がある。
/// 特に scc の条件分岐は B-type (±4KiB) を距離検査なしで出力するため，
