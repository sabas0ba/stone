/// @file pp20.sc
/// @brief C プリプロセッサ (Stage 17 第 5 部の世代)。GCC の gcc/ を読む容量。
///
/// pp19.sc の全文複製に，**マクロ表の容量と名前の引き方**だけを変えた
/// (docs/stage017-gcc.md 8.11)。展開の規則・探索パス・出力は変えていない。
///
/// @section why なぜ要ったか
/// GCC 4.7.4 の gcc/ の単位は，対象の記述 (tm.h・insn-flags.h など) を
/// 読むので**マクロを 7,000〜15,000 個定義する** (本体の合計 0.5〜1.5 MB)。
/// pp19 の表は 4096 個・本体 256 KiB で，gcc/ の全単位が表の上限 (6) で
/// 止まっていた。バンドルも 1 単位 2〜4 MB ある。
///
/// @section what 変えたもの
///   - マクロ数: 4096 -> 32768 / 名前: 256 KiB -> 2 MiB
///   - 本体: 256 KiB -> 4 MiB / 仮引数名: 128 KiB -> 2 MiB
///   - 入力 (src)・アリーナ 1 (xb): 4 MiB -> 8 MiB
///   - 識別子・マクロ名・仮引数名: 63 -> 127 バイト (スロット 128 バイト)。
///     gcc/ の最長のマクロ名は 64 バイトある
///     (ggc_alloc_zone_cleared_vec_gimple_statement_with_memory_ops_base)。
///     バンドルのメンバ名 (#include の綴り) は 63 バイトのまま
///   - **名前をハッシュ表で引く** (mhtab)。pp19 までは全スロットを先頭から
///     比べていた。15,000 個のマクロを識別子ごとに走査すると，3 MB の
///     入力の前処理が終わらない
///
/// ---- 以下は pp19 から引き継いだ来歴 ----
///
/// @file pp19.sc
/// @brief C プリプロセッサ (Stage 17 第 5 部の世代)。
///
/// pp18.sc の全文複製に，**押し戻しの器 (pbuf) を 16 -> 256 に広げた**
/// ことだけを足した (docs/stage017-gcc.md 8.10)。展開の規則・探索パス・
/// 出力の 1 バイトも変えていない。
///
/// @section why なぜ要ったか
/// 識別子を読むと，その直後の 1 文字を押し戻す。それがマクロなら置換結果の
/// フレームを積むが，**押し戻した文字は下の段に残ったまま**である (フレームを
/// 降ろすとき pbbase まで巻き戻すための仕組みで，誤りではない)。したがって
/// マクロの展開が 1 段入れ子になるごとに押し戻しが 1 つ溜まる。
///
/// 器は 16 だったので，**入れ子 15 段で容量超過 (6) になっていた**
/// (`#define A1 A0` ... `#define A15 A14` / `A15` で再現する。14 段までは
/// 通る)。フレームの上限は 63 段なので，押し戻しの器のほうが先に尽きていた。
/// GCC 4.7.4 の libiberty/regex.c はこれで止まっていた。
///
/// @section size 256 の根拠
/// 押し戻しが溜まるのは段ごとである。上の再現では 1 段に 1 つ (識別子の
/// 先読み) だった。gc / gcnc が 1 文字先を見て戻す分が重なっても 1 段に
/// 2 つと見積もり，フレームは 64 段 (ftop < 63) までなので 128 とした上で，
/// 倍の 256 を取った。超えたときはこれまでどおり 6 で止める (黙って捨てない)。
///
/// ---- 以下は pp18 から引き継いだ来歴 (pp18 は pp17 の全文複製で，
/// `#if` の式の中の defined を評価することだけを足した。stage017/pp18.md) ----
///
/// @file pp17.sc
/// @brief C プリプロセッサ (Stage 17 第 3 部の 3 の 2 世代)。
///
/// pp16.sc からの差は **`-I` を「探索パス」として持つこと**だけである。
/// 本体 (展開・条件・フレームの積み) には一切変更を加えていない。
///
/// @section why なぜ要ったか
/// cc18 の `-I dir` は「その階層のファイルを全部，バンドルのメンバに足す」と
/// いう意味だった。pp16 が引数を取らないので，そう表すしかなかった
/// (docs/stage017-cc.md 7.2)。ヘッダが十数個の /include ならこれで
/// 足りるが，tcc の木に `-I.` を与えるとメンバが 82 個になり，**pp16 の
/// 上限 64 を超えて容量超過の 6 で落ちる** (16.2)。
///
/// cc の側で `#include` を辿る手は使えない。cc は `#if` を評価しない
/// ので，通らない分岐の `#include` まで辿ってしまう。tcc の libtcc.c は
/// 各対象の実装を `#ifdef TCC_TARGET_*` で選ぶので，これが直接影響する。
/// **どの分岐が有効かを知っているのは前処理器だけである。**
///
/// @section super 上乗せであって置き換えではない
/// `#include` はこれまでどおり**まずバンドルのメンバを探す**。見つからな
/// かったときだけ `-I` の階層を順に見る。したがって既存のバンドルは
/// 1 バイトも変わらずに通る (回帰の検査はそれを見ている)。
///
/// @section name 見つからないときは名前を出す
/// pp16 は黙って 2 で落ちた。何が無いのか判らないと，呼んだ側は次に
/// 何をすればよいか決められない。名前を標準エラーへ出す。**終了コード
/// は 2 のまま**である (stage009 の検査がそれを見ている)。
///
/// @section os OS の上でしか走らない
/// sys_openat / sys_read / sys_close は ld16 の 'E' 前置部にしかない。
/// フラット ('F') で組むと未定義で落ちる。ホスト側の経路 (tool1 が
/// pp16.bin をベアメタルで回す経路) はこれまでどおり pp16 を使う。
///
/// ---- 以下は pp16 から引き継いだ来歴 ----
///
/// @file pp16.sc
/// @brief C プリプロセッサ (Stage 15 第 6 部世代 その 2)。テキストを受け取り，前処理済みテキストを返す。
///
...
///
/// pp15 (第 6 部): 容量だけをさらに広げた。tcc の翻訳単位 (ONE_SOURCE) は
/// #define を 2767 個持ち (elf.h だけで 2240 個)，バンドルは約 1.3 MB ある。
/// 実測: docs/stage015-tcc.md 12 章
///   - マクロ数: 1024 -> 4096 / 仮引数の畑: 32 Ki -> 128 Ki
///   - 入力・アリーナ: 1 MiB -> 4 MiB
///   - 識別子・マクロ名・メンバ名: 31 バイト -> 63 バイト (スロット 64 バイト)
/// pp16 (第 6 部その 2): **再帰抑止が実引数の事前展開で失われる不備**を
/// 直した (docs/stage015-tcc.md 12.7)。tcc.h の
/// `#define eh_frame_section s1->eh_frame_section` のような自己参照
/// マクロを実引数に渡すと，抑止して素通しした名前が置換結果の再走査で
/// もう一度展開され，s1->s1->eh_frame_section になっていた。
/// @section hygiene 抑止の印
/// 抑止して素通しする識別子には，事前展開の出力 (eb) へ書くときだけ
/// **印 (バイト 1) を前置する**。再走査で印つきの識別子に出会ったら
/// 展開せずそのまま通す (C99 6.10.3.4「二度と展開しない」)。最終出力へは
/// 印を出さないので，前処理結果は変わらない。
...
char src[8388608];         ///< 標準入力の全体 (EOT 0x04 まで。pp20 で 4 -> 8 MiB)
int srcn;                 ///< src の有効長

char xb[8388608];          ///< アリーナ 1: フレームが参照するテキスト (pp20 で 4 -> 8 MiB)
...
char mbname[16384];       ///< メンバ名 (64 バイトスロット x 256。pp17 で 4 倍)
int mbof[256];            ///< src 内の開始位置
int mbln[256];            ///< 長さ
int mbcnt;                ///< メンバ数

// ---- 探索パス (-I) ----
// バンドルに無い #include をここから探す。**順は与えられた順**である
// (本物の cc と同じで，先に書いた -I が勝つ)。

char idir[8192];          ///< 階層の綴り (256 バイトスロット x 32)
int nidir;                ///< 階層の数

// ld16 の 'E' 前置部が持つ生のスタブ。戻り値は -errno のままである
...
char mcname[4194304];     ///< マクロ名 (128 バイトスロット x 32768。pp20 で名前 127 バイト・16 倍)
int mcfun[32768];           ///< 1 = 関数形式
int mcnp[32768];            ///< 仮引数の個数 (可変長なら __VA_ARGS__ を含む)
int mcvar[32768];           ///< 1 = 可変長 (最後の仮引数が __VA_ARGS__)
int mcbo[32768];            ///< 本体の位置 (mbody 内)
int mcbl[32768];            ///< 本体の長さ
int mcpo[32768];            ///< 仮引数名の位置 (mprm 内。128 バイトスロットが mcnp 個)
int mcon[32768];            ///< 1 = 現在定義されている
int mccnt;                ///< スロット数

char mbody[4194304];      ///< マクロ本体の実体 (pp20 で 256 KiB -> 4 MiB)
int mbp;                  ///< mbody の有効長
char mprm[4194304];       ///< 仮引数名の実体 (128 バイトスロット。pp20 で 128 KiB -> 4 MiB)
int mpp;                  ///< mprm の有効長
...
int fpb[64];              ///< 積む直前の押し戻しバッファの下限位置 (pbbase の退避先)
int ftop;                 ///< 積みの頂点。-1 = 空

// 押し戻しはフレームごとに区切る。識別子を読むときの 1 文字先読みは
// 「マクロ名の次の文字」であり，展開結果よりも後ろに位置する。フレームを
// 積んだ後もそれが先に読めてしまうと，展開結果と前後が入れ替わる
// (`return A;` が `return ;1` になる)。そこで積むときにバッファの下限位置を更新し，
// 降ろすときに戻す。下限位置より前の押し戻し文字は，そのフレームからは見えない。
int pbuf[256];            ///< 押し戻した文字 (pp19 で 16 -> 256。段ごとに溜まる)
int pbff[256];            ///< 同上: その文字がファイル由来だったか
int pbn;                  ///< 押し戻しの個数
int pbbase;               ///< 現在のフレームから見える押し戻しバッファの下限位置
...
int cact[64];             ///< 1 = この段の現在の分岐を採用している
int cany[64];             ///< 1 = この段で既に真の分岐を通った
...
char idn[128];            ///< 走査中に読んだ識別子 (pp20 で 127 バイトまで)
int idl;                  ///< その長さ
char nbuf[128];           ///< 名前の作業域 (指令の解析用)
char wbuf[128];           ///< 指令名
// **nbuf とは別に持つ。** #if の式を読む enext() は，dodefined() が
// nbuf を使い終えた後に走るが，同じ枠を貸し借りすると「どちらがいつ
// 使うか」を追わなければ読めなくなる。63 バイト + 終端で足りる (pp18)
char ebuf[128];           ///< #if の式の中で読んだ識別子 (defined の判定)
...
/// @brief 名前スロットへ NUL 終端で複写する。
/// @return 常に 0。127 バイトを超える名前は容量超過 (6)。バンドルのメンバ名は
///         呼び手が 63 バイトまでに抑えている (メンバのスロットは 64 バイト)
...
/// @brief 入力がバンドルのマジックで始まるか調べる。
/// @return マジックの長さ (15)。バンドルでなければ 0
int hasmagic() {
  char *m;
  int i;
  m = "#!stone-bundle\n";
...
// **名前はハッシュ表で引く** (pp20)。pp19 までは全スロットを先頭から
// 比べていた。GCC の gcc/ の単位はマクロを 7,000〜15,000 個定義し，
// 入力は 3 MB ある。識別子 1 つごとに全スロットを走査すると，1 単位の
// 前処理が現実的な時間で終わらない。
//
// 表は 65536 項目の開番地法 (線形探索) で，項目にはスロット番号 + 1 を
// 置く (0 = 空)。スロットは 32768 までなので，詰まり具合は半分を超えない。
// スロットは消さない (#undef は mcon を 0 にするだけ) ので，表から項目を
// 消す操作も要らない。

int mhtab[65536];         ///< 名前のハッシュ -> スロット番号 + 1 (0 = 空)
int mhlast;               ///< mhfind が最後に見た項目 (見つからなければ空き項目)

/// @brief 名前のハッシュ (0〜65535)。
int mhash(char *nm) {
  int h;
  int k;
  h = 0;
  k = 0;
  while (nm[k]) {
    h = (h * 31 + (nm[k] & 255)) & 65535;
    k = k + 1;
  }
  return h;
}

/// @brief 名前のスロットをハッシュ表で探す (#undef されたものも含む)。
/// @return スロット番号。無ければ -1 (そのとき mhlast が空き項目を指す)
int mhfind(char *nm) {
  int h;
  int s;
  h = mhash(nm);
...
int mslot(char *nm) {
  ...
  i = mhfind(nm);
  if (i >= 0) return i;
  if (mccnt >= 32768) exit(6);
  i = mccnt;
  mccnt = mccnt + 1;
  nput(mcname + i * 128, nm);
  mcon[i] = 0;
  mhtab[mhlast] = i + 1;
  return i;
}
...
int pfind(int m, char *nm) {
  ...
  i = 0;
  while (i < mcnp[m]) {
    if (streq(mprm + mcpo[m] + i * 128, nm)) return i;
    i = i + 1;
  }
  return 0 - 1;
}
...
///          空白の連なりは空白 1 個にまとめる (# の文字列化の規定に合わせる)。
...
int domacro() {
  ...
  if (active(m)) {
    // 再帰抑止。事前展開の出力に入るときは印を前置し，置換結果の
    // 再走査でも展開しないようにする (@section hygiene)
    if (edep > 0) emit(1);
    return emitid();
  }
...
/// @details コメントは空白に置き換えるが，文字列・文字リテラルは逐語で残す
...
/// @brief eb から識別子を読んで ebuf へ入れる。長すぎるものは切る。
/// @note 切っても構わない —— この名前は defined の判定にしか使わず，
///       127 バイトを超える名前は mfind が持つ枠 (128 バイト) にそもそも
///       入らないので，切った名前は必ず「定義されていない」になる。
int ename() {
  int k;
  k = 0;
  while (evp < eve && isidc(eb[evp])) {
    if (k < 127) {
      ebuf[k] = eb[evp];
      k = k + 1;
    }
...
/// @brief **展開の結果に現れた** defined を評価する (pp18 で足した)。
/// @details `defined X` と `defined ( X )` の両方を受ける。`defined` の
///          綴りは既に読み終えている。
/// @return 定義されていれば 1，されていなければ 0。
int edefined() {
  int par;
  ...
  if (evp >= eve) exit(4);
  if (isidh(eb[evp]) == 0) exit(4);
  ename();
  if (par) {
    while (evp < eve && isws(eb[evp])) evp = evp + 1;
    if (evp >= eve || eb[evp] != ')') exit(4);
    evp = evp + 1;
  }
  if (mfind(ebuf) >= 0) return 1;
  return 0;
}

/// @brief eb から次のトークンを取り出し etok / enum_ へ入れる。
/// @note 識別子は「値 0 の数」として扱う (C89 の規定)。**ただし
///       `defined` だけは別**である —— pp18 の要点なので，以下に書く。
///
/// @section why 展開の後に現れた defined を評価する (pp18)
///
/// `defined` は本来**展開より先に** dodefined() が 1 / 0 へ置き換える。C89 が
/// そう定めているからで，その規定は pp18 でも変えていない。
///
...
/// しかない。展開してはじめて `defined(__cplusplus)` が現れるが，
/// そのときには dodefined() は終わっている。pp17 までは残った
/// `defined` を識別子として 0 に置き換え，続く `(` で構文が壊れて 4 で
/// 止まっていた。
///
/// **C89 6.8.1 はこの形を未定義動作としている。** 拒むのは規格の上では
/// 正しく，ホストの `gcc -std=c89 -pedantic-errors` も
/// `this use of "defined" may not be portable` で拒む。
///
/// **それでも通す。** GCC 4.7.4 の libcpp は自分自身がこの形を受けること
/// を前提に書かれており，13 単位すべてがここで止まる
/// (docs/stage017-gcc.md 8.4)。GCC を組むのが目的である以上，選ぶ余地は
/// 無い。ホストの cpp・clang・その他の実物の処理系はどれも評価する側に
/// 倒しており，**倒す先は 1 つしかない**。
///
/// 影響は `#if` の式に限られる。dodefined() の規定も，展開の仕組みも，
/// 出力の 1 バイトも変えていない —— 変わるのは「展開の後に defined が
/// 残っていたとき，4 で止まるか値を出すか」だけである。
int enext() {
  int c;
  int d;
  int done;
  ...
  if (isidh(c)) {
    ename();
    enum_ = 0;
    if (streq(ebuf, "defined")) enum_ = edefined();
    etok = te_num;
    return 0;
  }
  if (isdig(c)) {
...
/// @brief 標準エラーへ 1 行出す。前置部の sys_write を直に呼ぶ。
int errs(char *m) {
  int n;
  n = 0;
  while (m[n]) n = n + 1;
...
/// @brief 階層と名前を繋いで経路を作る。
/// @return 作った長さ。器を超えるなら 0 を返す (呼ぶ側が飛ばす)
int joinpath(char *dir, char *name, char *out, int cap) {
  int i;
  int k;
  k = 0;
  i = 0;
...
/// @brief 経路のファイルを src の末尾へ読み込み，メンバとして足す。
/// @return メンバの番号。開けなければ -1
/// @details **メンバとして足すので，以降の扱いはバンドルのメンバと同じ**になる。
///          読み終えた後にメンバを足すのは，途中で失敗したときに半端な
///          メンバを残さないためである。
int loadinc(char *path, char *name) {
  int fd;
  int n;
  int st;
  fd = sys_openat(0 - 100, path, 0, 0);
  if (fd < 0) return 0 - 1;
  st = srcn;
  n = 1;
  while (n > 0) {
    if (srcn >= 8388608) exit(6);
    n = sys_read(fd, src + srcn, 8388608 - srcn);
    // **読めなかったのを 0 バイトと取り違えない。** 取り違えると
    // 「空のヘッダ」として素通りし，欠けたまま先へ進む
    if (n < 0) exit(2);
    srcn = srcn + n;
  }
  sys_close(fd);
  if (mbcnt >= 256) exit(6);
  nput(mbname + mbcnt * 64, name);
  mbof[mbcnt] = st;
  mbln[mbcnt] = srcn - st;
  mbcnt = mbcnt + 1;
  return mbcnt - 1;
...
/// @brief 探索パスを順に見て，最初に開けたものをメンバにする。
/// @return メンバの番号。どこにも無ければ -1
int findinc(char *name) {
  int i;
  int r;
  char path[512];
  i = 0;
  while (i < nidir) {
    if (joinpath(idir + i * 256, name, path, 512)) {
      r = loadinc(path, name);
      if (r >= 0) return r;
    }
    i = i + 1;
...
/// @brief #include。"..." と <...> はどちらもバンドルのメンバを探す。
int d_include() {
  int q;
  int k;
  int i;
  ...
  // **バンドルに無ければ探索パスを見る。** ここが pp17 で足した唯一の経路
  i = findinc(nbuf);
  if (i >= 0) {
    pushfile(i);
    return 0;
  }
  // **黙って 2 で落ちない。** 何が無いのかを言う
  errs("pp: cannot find include: ");
  errs(nbuf);
  errs("\n");
  exit(2);
  return 0;
...
/// @brief #elif / #else が分岐を切り替える。
/// @param v 新しい分岐を採用するか (#else は 1)
int cnext(int v) {
  int k;
  k = cdn - 1;
  if (k < 0) exit(4);
...
int scan1() {
  ...
  if (c == 1) {
    // 事前展開が付けた抑止の印。続く識別子は展開せずに通す。
    // さらに内側の展開へ入るなら印も引き継ぐ (@section hygiene)
    c = gcnc();
    readid(c);
    if (edep > 0) emit(1);
    emitid();
...
/// @brief 2 文字の綴りが一致するか (選択肢の頭を見るため)。
int isopt(char *a, char c) {
  if (a[0] != '-') return 0;
  if (a[1] != c) return 0;
  return 1;
}
...
int main(int argc, char **argv) {
  ...
  init();
  nidir = 0;
  // **知らない選択肢は黙って受けない。** 受けて捨てると，有効な
  // つもりの指定が無効のまま通る (我々が bad と呼んでいる形である)
  i = 1;
  while (i < argc) {
    if (isopt(argv[i], 'I')) {
      if (argv[i][2]) {
        ...
        i = i + 1;
        if (i >= argc) {
          errs("pp: -I needs a directory\n");
          exit(1);
        }
        addidir(argv[i]);
      }
    } else {
      errs("pp: unknown option: ");
      errs(argv[i]);
      errs("\n");
      exit(1);
    }
  ...
  mbp = 0;
  mpp = 0;
  // ハッシュ表を空にする (pp20)。大域変数が 0 で始まることには頼らない
  i = 0;
  while (i < 65536) {
    mhtab[i] = 0;
    i = i + 1;
