 ...
 * ---- 書かないもの ----
 *
 * **符号の索引 (`s`) は書かない。** 我々の cc は書庫の中身を全メンバ
 * 並べるので索引を引かない。書くには各メンバの ELF 符号表を読む
 * 必要があり，本題から遠い (7.4)。`s` は受け取って黙って無視する。
 * host の `ar t` / `ar x` は索引が無くても動く。
 *
 * **長い名前 (16 バイト以上) は扱わない。** 本物は `//` という特別な
 * メンバに名前表を置いて `/offset` で参照するが，そこまでは作らない。
 * **黙って切り詰めず，はっきり失敗する。** 切り詰めると別のメンバと
 * 同じ名前になりうる。
 */
#include <stdio.h>
#include <stdlib.h>
