...
#define SYS_STATAT2  502        /* 第 24 世代 (docs/stage017-gcc.md 8.5) */
#define SYS_TIMES    153        /* 第 25 世代 (docs/stage017-gcc.md 8.9) */
...
int open(char *path, int flags, ...) {
  ...
  /* **知らないフラグを黙って捨てない** (第 21 世代)。カーネルの openat は
...
int fcntl(int fd, int cmd, void *arg) {
  ...
  if (cmd == F_GETLK || cmd == F_SETLK || cmd == F_SETLKW) {
     ...
     * 当てる。分岐させていたせいで，同じ判定を 2 度に分けて入れる
...
/* ナノ秒の 64 bit (下位語 lo・上位語 hi) を秒へ直す (第 24 世代)。
 *
 * **ここでだけ 64 bit の除算を使う。** 11.2 で「秒に直さない」と
 * 決めたのは，比べるだけなら要らない仕事だからである。st_mtime は
 * POSIX が秒で配ると決めている欄なので，ここでは直す */
static time_t ns2sec(unsigned lo, unsigned hi) {
  unsigned long long ns;
  ns = ((unsigned long long)hi << 32) | (unsigned long long)lo;
  return (time_t)(ns / 1000000000ULL);
}

/* 経路の情報を取る (第 19 世代。欄は第 24 世代で増えた)。
 *
 * **statat2 (502) を呼ぶ。** statat (79) は u32 4 語を書くと約束して
 * いて，その約束は変えていない。項目番号 (st_ino) を足した 5 語の形は
 * 新しい番号で配る (kernel26.c の註)。
 *
 * **許可・所有者は無い** —— 持っていない欄は返さない (11.5)。
 * st_dev / st_ino / st_mode / st_mtime が何であるかは
 * include/sys/stat.h の註にある */
int stat(char *path, struct stat *st) {
  unsigned raw[5];
  int r;
  r = wrap(sys_ecall(SYS_STATAT2, AT_FDCWD, (int)path, (int)raw));
  if (r < 0) return -1;
  ...
  st->st_mtime = ns2sec(raw[1], raw[2]);
  return 0;
}
...
/* プロセスの時間 (第 25 世代)。カーネル (kernel27) の times (153) が
 * struct tms の 4 語を書き，起動からの tick 数を返す。
 * 4 つの欄の意味は include/sys/times.h の註にある */
clock_t times(struct tms *buf) {
  return (clock_t)wrap(sys_ecall(SYS_TIMES, (int)buf, 0, 0));
}

/* 実行時の上限 (第 25 世代)。**答を持っている名前だけに答える。**
 * 知らない名前は EINVAL で -1 —— 「上限が無い」の -1 (errno を変えない)
 * とは区別される */
long sysconf(int name) {
  if (name == _SC_CLK_TCK) return 100;  /* kernel27 の tick は 10 ms */
  errno = EINVAL;
  return -1;
}

/* 経路についての上限 (第 25 世代)。path が無ければ stat と同じ errno で
 * -1 を返す。上限の値そのものは経路によらない (sfs の巻は 1 つ) */
 ...
 *
 * **呼び手の器 (resolved) は PATH_MAX バイトとみなす** (第 25 世代)。
 * POSIX の約束で，lrealpath (libiberty) は char buf[PATH_MAX] を渡す。
 * 第 24 世代までは内部の 1024 バイトまで書いていたので，PATH_MAX を
 * 定義した時点で呼び手の器を越えて書きうる。越える結果は書かずに
 * ENAMETOOLONG で返す */
#define _RPMAX 1024
