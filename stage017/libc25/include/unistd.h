...
/* 実行時の上限を訊く (第 25 世代。docs/stage017-gcc.md 8.9)。
 *
 * **答を持っている名前だけを定義する。** 名前を定義しておいて -1 を
 * 返すと，呼び手は「上限が無い」と読む (POSIX の約束)。持っていない
 * 上限をそう読ませるのは嘘になるので，名前ごと配らない。
 *
 *   _SC_CLK_TCK    1 秒あたりの tick 数。times() の単位 (100)
 *   _PC_PATH_MAX   経路の長さの上限。終端の NUL を含めて PATH_MAX (256)
 *
 * 番号は glibc と同じにしてある */
