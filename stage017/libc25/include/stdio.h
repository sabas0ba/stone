 ...
 * 末尾へ移動する**形で追記を libc の側に実装した。我々は単一の走行なので
 * (spawn は子の終わりを待つ)，これで POSIX の O_APPEND と同じ意味に
 * なる —— 途中で fseek しても、次に書くのは末尾である。
 *
 * **生の O_APPEND は open() が拒む。** カーネルが知らないフラグを黙って
...
/* putc は **マクロとして**置く。
 *
 * C89 7.9.1 は「この見出しで宣言する関数は，マクロとしても実装して
 * よい」と定めており，7.9.7.8 は putc について「関数ではなくマクロと
 * して実装してよい」と名指ししている。
 *
 * 関数宣言では無効である。**ビルドチェーンの前置部が 1 引数の putc を
 * primitive として持っている** (stage015/cc15ab.sc の bireg())。器の
 * 組込みが勝つので，2 引数の呼出しは引数個数の不一致 (5) になる。
 * 前置部の名前を替えると成果物がすべて変わるので，そこには触れない。
 *
 * **限界: 関数としては使えない。** C89 7.9.1 が認める `(putc)(c, f)` の
 * 形と `&putc` は，マクロが展開されないので前置部の 1 引数 putc に
 * 当たって 5 で落ちる。GCC 4.7.4 の libiberty / libcpp はどちらの形も
 * 使っていない (呼出しの形だけ)。**会ってから足す**
 ...
 * している。
 *
 * **これは第 24 世代で初めて見えた不足である。** libcpp/files が
