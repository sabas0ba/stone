...
/* 経路の長さの上限 (POSIX。第 25 世代)。**終端の NUL を含む**。
 *
 * カーネル (kernel27) の stat と spawn は経路を 255 バイト + NUL の
 * 器で受け，それより長ければ E2BIG で拒む。open や chdir はこれより
 * 長い経路も受けるが，**どの syscall にも渡せる長さ**の上限はこの値で
 * ある。POSIX の最小値 _POSIX_PATH_MAX (256) を満たす。
 *
 * kernel26 以前は写す側の容量が 1 バイト足りず，254 バイト + NUL
 * までしか通らなかった (kernel27.c の註)。
 *
 * realpath はこの長さを超える結果を呼び手の器へ書かない
 * (ENAMETOOLONG) */
#define PATH_MAX 256
