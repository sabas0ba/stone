...
/* C89 の空白は 6 種類ある (7.3.1.9 の isspace)。第 21 世代の
 * strtoul / strtoull は空白と水平タブしか読み飛ばさなかった
 * (docs/stage017-gcc.md 5.3) */
static int wsp(int c)
{
    return c == ' ' || c == '\t' || c == '\n' || c == '\v'
        || c == '\f' || c == '\r';
}
...
unsigned long strtoul(const char *s, char **endptr, int base)
{
     ...
     *
     * **第 21 世代は 2 つ誤っていた** (docs/stage017-gcc.md 5.3)。
     *   - 変換できる桁が 1 つも無いときに endptr を進めていた。C89
     *     7.10.1.6 は「変換が行われなければ nptr を格納する」と定める。
     *     "  -" や "xyz" で呼び手が 3 文字進んだことにしてしまう
     *   - "0x" の後ろが 16 進数字でないときも 0x を食べていた。
     *     主部は "0" であり，endptr は x を指さねばならない */
    const char *p;
    unsigned long v;
    int neg;
    int d;
