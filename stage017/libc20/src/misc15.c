...
/* getcwd は第 16 世代で posix/sys.c へ移した。
 *
 * 第 15 世代までは**ここに「常に "/" を返す」偽物**があった。名前空間が
 * 平らでルート直下しか無かったため問題が表面化しなかったが，作業ディレクトリを
 * 持った途端に「今どこにいても / と答える」ようになる。台帳でいう bad
 * (黙って間違う) そのものなので，実体のある実装に差し替えた
 * (docs/stage016-os.md 7 章)。
 *
 * ここ (src/) は syscall を呼ばない純粋部なので，実体は環境部へ置く。 */

/* unlink / remove は第 17 世代で posix/sys.c へ移した。
 *
 * 第 16 世代まではここに削除せず 0 を返す暫定実装があった。sfs に
 * 削除未対応のまま成功を返しており，「成功したのに
 * 消えていない」は台帳でいう bad である。第 4 部の 1 でカーネルに
 * unlinkat (35) を入れたので，実体のある実装に差し替えた
...
/* realpath は第 17 世代で posix/sys.c へ移した。
 *
 * 第 16 世代まではここに入力パスを複写するだけの暫定実装があった。名前空間が
 * 平らでルート直下しか無かったため問題が表面化しなかったが，木と作業
 * ディレクトリの導入後は誤ったパスを返す —— 相対経路がそのまま返り，
 * 別の場所から使うと別のファイルを指す。getcwd / open と同じ形の bad
 * である (docs/stage016-os.md 7.4 / 9.4)。
 *
 * パスの正規化には作業ディレクトリが必要なので，実体は環境部へ置く。 */
