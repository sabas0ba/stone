...
ビルドチェーンの側 (`cc19` が tcc を組む経路) は `libc20` のままである。そちらを
動かすと 21 章・29 章のバイト突き合わせが動くので，触っていない。

**当初この世代は「tcc に訳させる libc」として作った。その方式は撤回した**
—— 外部の処理系に我々のソースを訳させた出力も外部の生成物であり，ビルドチェーンにも
実行環境にも入れられない ([docs/artifacts.md](../docs/artifacts.md) 3 章 /
[docs/stage017-cc.md](../docs/stage017-cc.md) 34 章)。

**残す理由は別にある。** ここで足したものは，どれも**実物のソースを
読んで判った我々の libc の未実装機能・不具合**である。訳す処理系が何であっても不足は不足なので，
次に stone の処理系で実物を組むとき (`zlib` / `bzip2`。stage017-gcc.md 5.1) に
そのまま要る。

したがって本書に SHA-256 の表は無い。表が要るのは，この世代から
記録対象の成果物を作るようになったときである
([docs/artifacts.md](../docs/artifacts.md) 5)。
...
| | どうしたか | なぜ |
|---|---|---|
| `signal()` | 登録を受け付け，**何も起こさない**。返り値は常に `SIG_DFL` | シグナルが起きない環境では，ハンドラが呼ばれないことが正しい振舞いである |
| `raise()` | `EINVAL` で**拒む** | 0 を返すと，シグナルを送信していないのに成功を通知することになる |
| `O_APPEND` | 名前は与え，`open()` が `EINVAL` で**拒む** | カーネルは知らないフラグを黙って捨てるので，渡すと「追記のつもりが先頭から上書き」になる |
| `O_EXCL` / `O_CLOEXEC` | **定義しない** | zlib はどちらも `#ifdef` で守る。無ければその分岐が除かれて正しく動く |
