/* sh4.c --- POSIX 部分集合のシェルの第 4 世代 (docs/stage017-gcc.md 5.7)
 *
 * `sh3` との差は 2 つ。**正規表現機構を `re2` に替えた**ので
 * `grep -E` が使えるようになり，`grep` と `expr` が組の中へ後戻り
 * できるようになった。もう 1 つは，照合の手数が上限に当たったときに
 * 黙って進まず診断を出すことである。
 *
