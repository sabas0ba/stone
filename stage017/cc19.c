 ...
 *
 * **-I を束ねへ展開するのをやめ，pp17 へそのまま渡す。**
 *
 * 「階層ごと束ねる」は /include のような十数個の置き場でしか成り立た
 * ない。tcc の木に -I. を与えると員が 82 個になり pp16 の上限 64 を
 * 超えて落ちる。しかも本当に要るのは 9 個である (16.2)。
 *
 * cc の側で #include を辿って要るものだけ足す手は使えない。cc は
 * #if を評価しないので，通らない枝まで辿ってしまう。**どの枝が生きて
 * いるかを知っているのは前処理器だけである。**
 *
 * /include と /include/sys は**これまでどおり束ねる**。我々のもので
...
/* **-I はもう束ねない。** pp17 へ渡す (上の註)。ここで足すのは
 * 我々の /include だけである */
