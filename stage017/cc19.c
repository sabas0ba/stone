 ...
 * ——  コマンド行の順が意味を持つからである。
 *
 * ---- 第 19 世代で変えたこと (docs/stage017-cc.md 16.2 / 19 章) ----
 *
 * **-I をバンドルへ展開するのをやめ，pp17 へそのまま渡す。**
 *
 * 「階層ごとバンドル」は /include のような十数個の置き場でしか成り立た
 * ない。tcc の木に -I. を与えるとメンバが 82 個になり pp16 の上限 64 を
 * 超えて落ちる。しかも本当に要るのは 9 個である (16.2)。
 *
 * cc の側で #include を辿って要るものだけ足す手は使えない。cc は
 * #if を評価しないので，通らない分岐まで辿ってしまう。**どの分岐が有効で
 * あるかを知っているのは前処理器だけである。**
 *
 * /include と /include/sys は**これまでどおりバンドルする**。我々のもので
 * 数が知れており，既存のプログラムがこの形に乗っているからである。
 * 変えたのは -I だけ。
 */
#include <stdio.h>
#include <stdlib.h>
...
/* **-I はもうバンドルしない。** pp17 へ渡す (上の註)。ここで足すのは
 * 我々の /include だけである */
...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < nin; i = i + 1) {
    if (endswith(ins[i], ".a")) {
       ...
       * その中の adddir が同じバッファを上書きし，**出力が最後に走査した
       * ヘッダの上に書かれる** (cc18 にこの誤りがある。19.3)。
       * しかも壊した先を自分で読み直すので**辻褄が合い，リンクは
       * 通って走ってしまう**。我々が繰り返し遭遇している型である
