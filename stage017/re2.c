 ...
 * 1 つ返すので，続きが失敗したときに「組の取り方を変えてやり直す」
 * 手段が無い。`\(a*\)ab` が "aab" に合わないのはこれである。
 *
 * 命令列にすると，後戻りは**続きを引数に取った再帰**そのものになる。
 * SPLIT が 2 つの経路を順に試し，どちらも最後の MATCH まで辿る。組の中で
 * あろうが外であろうが同じ 1 つの仕組みで済む。選択 (`|`) も SPLIT が
 * そのまま担う。
 *
 * ## 手数と段数
 *
 * 単純に書くと `a*` のような形で**入力の長さぶんの再帰**が積む。行が
 * 8 KiB あると段数もそれに比例して危ない。そこで **1 文字の節の繰返し
 * だけは I_REP という 1 命令にまとめた** —— 回数を数える繰返しは再帰
 ...
 * 読むが，そちらに合わせると POSIX の台本で黙って違う答になる */
...
/* 節が 1 命令の「1 文字もの」か。繰返しを I_REP にまとめられるかの判定 */
