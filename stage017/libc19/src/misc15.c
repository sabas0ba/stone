 ...
 * 平らでルート直下しか無かったので嘘にならなかったが，作業ディレクトリを
 ...
 *
 * 第 16 世代までは**ここに「何もせず 0 を返す」偽物**があった。sfs に
 * 削除が無かったので嘘をつくしかなかったのだが，「消したと言ったのに
 ...
 *
 * 第 16 世代までは**ここに「複写するだけ」の偽物**があった。名前空間が
 * 平らでルート直下しか無かったので嘘にならなかったが，木と作業
 * ディレクトリを持った途端に嘘になる —— 相対経路がそのまま返り，
 * 別の場所から使うと別のファイルを指す。getcwd / open と同じ形の bad
 * である (docs/stage016-os.md 7.4 / 9.4)。
 *
 * 本物は作業ディレクトリを要るので，実体は環境部へ置く。 */
