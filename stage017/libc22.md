...
([libc21.md](libc21.md) 1 節)。ビルドチェーンの側 (`cc19` が tcc を組む経路) は
`libc20` のままである。したがって本書にも SHA-256 の表は無い。

ここで足したものは、**GCC 4.7.4 のソースを読んで判った我々の libc の
不足**である。`libcpp` の `lex` / `line-map` / `mkdeps` の 3 単位が
これ 1 つで止まっていた。
...
**ビルドチェーンの前置部が 1 引数の `putc` を primitive として持っている**
([stage015/cc15ab.sc](../stage015/cc15ab.sc) の `bireg()`)。器の組込みが
勝つので、header 側の宣言は届かない。

前置部の名前を替える方法もあるが、**ビルドチェーンの成果物がすべて変わる** ——
...
C89 7.9.1 は続けて、マクロを抑止して関数を呼ぶ方法を認めている。
...
呼出しの形である。**必要になってから足す**。足すときは前置部の側を触ることに
なるので、ビルドチェーンの成果物がすべて変わる覚悟が要る。
...
- `tests/stage017/test.sh` 第 7 部が、**stone の OS の上で**
