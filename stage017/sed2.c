/* sed2.c --- ストリームエディタの第 2 世代 (docs/stage017-gcc.md 5.6)
 *
 * `sed1` との差は 1 つだけ ——**正規表現機構を自分で持たず，
 * `re1` を使う**。`grep` も同じ機構を使うので，写しを 2 つ持つと
 * 必ず片方だけ直す誤りが出る (stage018-ext.md 3.2 / 5.1 と同じ形)。
 *
 * 受ける形と受けない形は `sed1` と同じで，`tools/diffsed.sh` の
 * 40 件がそのまま当たる。
 */
#include <stdio.h>
#include <stdlib.h>
...
static int matchaddr(int ty, int n, int re) {
  ...
  if (ty == A_RE) return re_search(re, pspace, &b, &e);
  return 0;
}
...
static int dosub(int i) {
  ...
  while (1) {
    ...
    me = 0;
    {
      ...
      t = s;
      while (1) {
        if (!re_search_from(cre[i], pspace, t, &mb, &me)) { mb = 0; break; }
