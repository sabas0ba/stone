 ...
 *      短くなる。awk はビルドチェーン中で最大のプログラムである。
 *   2. 書式は 1 か所にまとまっていたほうが，libc の世代が変わったとき
 *      に付け替える場所が 1 つで済む。第 1 世代を書いた時点では
 *      `libc22` が `%g` と `%e` を持っていなかったのでここで組んで
 *      いたが，`libc23` がその不足を解消したので**いまは libc に任せて
 *      いる** (5.8 / 6.3)。
 *
