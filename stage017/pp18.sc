...
///
/// pp16.sc からの差は **`-I` を「探す道」として持つこと**だけである。
/// 本体 (展開・条件・フレームの積み) には一切手を入れていない。
///
/// @section why なぜ要ったか
/// cc18 の `-I dir` は「その階層のファイルを全部，束ねの員に足す」と
/// いう意味だった。pp16 が引数を取らないので，そう表すしかなかった
/// (docs/stage017-cc.md 7.2)。ヘッダが十数個の /include ならこれで
/// 足りるが，tcc の木に `-I.` を与えると員が 82 個になり，**pp16 の
/// 上限 64 を超えて容量超過の 6 で落ちる** (16.2)。
///
/// cc の側で `#include` を辿る手は使えない。cc は `#if` を評価しない
/// ので，通らない枝の `#include` まで辿ってしまう。tcc の libtcc.c は
/// 各対象の実装を `#ifdef TCC_TARGET_*` で選ぶので，これが直に効く。
/// **どの枝が生きているかを知っているのは前処理器だけである。**
///
/// @section super 上乗せであって置き換えではない
/// `#include` はこれまでどおり**まず束ねの員を探す**。見つからな
/// かったときだけ `-I` の階層を順に見る。したがって既存の束ねは
...
/// フラット ('F') で組むと未定義で落ちる。ホスト側の道 (tool1 が
...
/// #define を 2767 個持ち (elf.h だけで 2240 個)，束ねは約 1.3 MB ある。
...
// ---- 探す道 (-I) ----
// 束ねに無い #include をここから探す。**順は与えられた順**である
...
///
/// `defined` は本来**展開より先に** dodefined() が 1 / 0 へ潰す。C89 が
...
/// `defined` を識別子として 0 に潰し，続く `(` で構文が壊れて 4 で
/// 止まっていた。
///
...
/// @brief 経路のファイルを src の末尾へ読み込み，員として足す。
/// @return 員の番号。開けなければ -1
/// @details **員として足すので，以降の扱いは束ねの員と同じ**になる。
///          読み終えた後に員を足すのは，途中で失敗したときに半端な
///          員を残さないためである。
...
/// @brief 探す道を順に見て，最初に開けたものを員にする。
/// @return 員の番号。どこにも無ければ -1
...
int d_include() {
  ...
  // **束ねに無ければ探す道を見る。** ここが pp17 で足した唯一の道
  i = findinc(nbuf);
  if (i >= 0) {
    pushfile(i);
    return 0;
...
int scan1() {
  ...
  if (c == 1) {
    ...
    // さらに内側の展開へ入るなら印も持ち回る (@section hygiene)
    c = gcnc();
    readid(c);
    if (edep > 0) emit(1);
    emitid();
...
int main(int argc, char **argv) {
  ...
  init();
  nidir = 0;
  // **知らない選択肢は黙って受けない。** 受けて捨てると，効いている
  // つもりの指定が効かないまま通る (我々が bad と呼んでいる形である)
  i = 1;
  while (i < argc) {
    if (isopt(argv[i], 'I')) {
      if (argv[i][2]) {
