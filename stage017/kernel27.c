/* kernel27.c --- 簡易 OS のカーネル (Stage 17 第 5 部の世代)
 *
 * kernel26 の写しに **times (153)** を 1 本足したものである
 * (docs/stage017-gcc.md 8.9)。既存の呼出しは 1 つも変えていない。
 *
 * ---- 何を数えるか ----
 *
 * プロセスごとに 4 つの時間を持つ。Linux の struct tms と同じ意味である。
 *
 *   utime   U モードで走っていた時間
 *   stime   そのプロセスの syscall を処理していた時間
 *   cutime  終了した子の utime + cutime の合計
 *   cstime  終了した子の stime + cstime の合計
 *
 * **CPU は 1 つで，走るプロセスは常に最も深い 1 つである** (spawn は
 * 親を退避して子を走らせ，子が終わるまで親は動かない)。したがって
 * トラップの出入りで goldfish RTC を読み，差をその時点の深さへ足せば
 * 各プロセスの時間になる。
 *
 *   トラップに入った時刻 - 前回トラップを出た時刻   -> utime
 *   トラップを出る時刻 - トラップに入った時刻       -> stime
 *
 * 数えないものが 1 つある。**タイマ割込みを使っていない**ので，
 * syscall を 1 度も呼ばずに走り続けるプロセスの utime は次の
 * トラップまで足されない。足されるのが遅れるだけで，失われはしない
 * (差は前回の時刻から取るので)。
 *
 * ---- 単位 ----
 *
 * 内部ではナノ秒を u32 2 語で持ち，times を呼ばれたときだけ tick
 * (1 tick = 10 ms。_SC_CLK_TCK = 100) へ割る。**割るのは 1 か所だけ**で，
 * 足し算の側に端数の丸めを持ち込まない。100 は Linux の USER_HZ と
 * 同じ値である。
 *
 * times の戻り値は起動してからの tick 数である。POSIX は「過去の
 * ある時点から」としか言っておらず，差だけに意味がある。
 *
 * ---- 経路の上限を定義どおりにした ----
 *
 * PATHMAX は「終端を除いて 255」と定義し，器も PATHMAX + 1 バイト
 * 取っていたが，写す側 (cpystr) に渡す容量が PATHMAX だった。
 * cpystr は容量の中に NUL が見つからなければ失敗するので，**実際に
 * 通るのは 254 バイトまで**で，定義より 1 バイト短かった。第 27 世代で
 * 容量を器の大きさ (PATHMAX + 1) に揃え，255 バイト + NUL を通す。
 *
 * これで libc25 の PATH_MAX (NUL を含む) が 256 になり，POSIX の最小値
 * _POSIX_PATH_MAX (256) を満たす。254 バイトまでの経路の扱いは
 * kernel26 と変わらない。
 *
 * ---- 以下は kernel26 から引き継いだ註である ----
 *
 * kernel25 の写しに **statat2 (502)** を 1 本足しただけのものである
 * (docs/stage017-gcc.md 8.5)。sfs も配置も経路の長さも kernel25 と
 * 同じで，既存の呼出しは 1 つも変えていない。
 *
 * ---- なぜ 79 を広げずに 1 本足したか ----
 *
 * `statat` (79) は out へ u32 4 語を書くと**約束している**。語を
 * 5 つに増やすと，4 語のバッファを渡している既存の実行形式で隣が上書きされる
 * —— 通ったうえで壊れる類で，libc23 で組んだ成果物がすべてこれに
 * あたる。**約束は変えない。新しい約束は新しい番号で配る。**
 *
 * 足したのは `st_ino` に使う値である。sfs の表の項目番号がそのまま
 * 「同じファイルか」の判定に使える —— `libiberty/fdmatch` と
 * `libiberty/getpwd` が `(st_dev, st_ino)` の組で見る。
 *
 * ---- 以下は kernel25 から引き継いだ註である ----
 *
 ...
 *   SFSA      0x8400_0000 -> 0xa000_0000。ウィンドウが 32 MiB -> 512 MiB
 ...
 * (0x8380_0000)・ユーザイメージのロード先 (0x8600_0000) は ld16 の前置部に
 ...
 *
 * **イメージがウィンドウに入らなければ起動しない。** 頭の大きさの欄を見て，
 * ウィンドウを超えるなら 'S' を出して止まる。黙って載せると，はみ出した先は
 * 何も無い番地なので，書いた中身が消える。
 *
 ...
 * のメンバを並べると語数は 11 になる。kernel22 は
 *
 *     while (kargc < 8) { ... }
 *
 * と書いていて，**溢れたぶんを黙って捨てていた**。ar は受け取った
 * 8 語だけを見てメンバ 5 個の書庫を作り，0 を返して終わる。呼んだ側からは
 ...
 * 書いたものが返る。動くように見えて意味が違う，我々が何度も遭遇した
 ...
 *
 *   unlinkat (35)  項目を未使用に戻す。データの空き領域は詰めない
 *
 ...
 * はっきり失敗するので bad ではない。
 *
 * 削除はディレクトリには適用されない (中身が残るため)。空のディレクトリを
 * 消す手段は要るときに足す。
 *
 ...
 * (0x8370_0000)・カーネルスタック (0x8380_0000)・ユーザイメージのロード先
 ...
 * それ以前からの引き継ぎ。
 *
 * stage013/kernel.c を出発点に，2 つの不足を解消したものである
 ...
 * プログラムヘッダの 1 本目だけを見ていた。tcc が生成する実行形式は
...
/* 第 26 世代で足した。statat の 4 語に**表の項目番号**を 1 語足した
 * 形を返す。79 の約束は変えていない (ファイル頭の註) */
#define SYS_STATAT2   502
/* 第 27 世代で足した。Linux と同じ番号・同じ形 (struct tms の 4 語を
 * 書き，起動からの tick を返す) */
#define SYS_TIMES     153

/* 1 tick のナノ秒数 (100 Hz) */
#define TICKNS  10000000
...
/* 深さごとの時間 (ナノ秒。下位語 / 上位語)。深さは 0〜MAXDEP の 9 段。
 * 退避レコードに入れないのは，**子が走っている間も親の欄は動かない**
 * から (親の欄に足すのは子が終わったときだけ) */
unsigned tul[9];                /* utime */
unsigned tuh[9];
unsigned tsl[9];                /* stime */
unsigned tsh[9];
unsigned tcul[9];               /* cutime */
unsigned tcuh[9];
unsigned tcsl[9];               /* cstime */
unsigned tcsh[9];
unsigned tlastl;                /* 最後にトラップを出た時刻 */
unsigned tlasth;
unsigned tbootl;                /* 起動した時刻 (times の戻り値の起点) */
unsigned tbooth;
...
/* ---- 時間の計上 (第 27 世代) ----
 *
 * ナノ秒は u32 2 語で持つ。我々の C は 32 bit なので，桁上がりと
 * 桁借りは下位語の比較で出す */

/* (*l, *h) += (bl, bh) */
void add64(unsigned *l, unsigned *h, unsigned bl, unsigned bh) {
  unsigned s;
  s = *l + bl;
  *h = *h + bh + (s < bl);      /* 下位語が回り込んだら桁上がり */
  *l = s;
}

/* (*dl, *dh) = (al, ah) - (bl, bh) */
void sub64(unsigned al, unsigned ah, unsigned bl, unsigned bh,
           unsigned *dl, unsigned *dh) {
  *dl = al - bl;
  *dh = ah - bh - (al < bl);    /* 下位語で借りたら上位語から 1 */
}

/* ナノ秒 (l, h) を tick に直す。**割り算はここ 1 か所だけ**である。
 *
 * 64 bit を 32 bit の定数で割る筆算を 1 bit ずつ行う。余りは常に
 * TICKNS 未満 (< 2^24) なので，1 bit 左へずらしても溢れない。商は
 * 32 bit に収まる前提である (2^32 tick は 497 日) */
unsigned ticks(unsigned l, unsigned h) {
  unsigned q;
  unsigned r;
  unsigned b;
  int i;
...
/* 深さ d の 4 つの時間を 0 にする。spawn で子が始まるとき */
void tclear(int d) {
  tul[d] = 0;
  tuh[d] = 0;
  tsl[d] = 0;
...
/* 終わった子 (深さ c) の時間を親 (深さ p) の cutime / cstime へ足す。
 * 足すのは子自身の分と，**子がさらに待った孫の分**の両方である
 * (POSIX: 待った子の utime と cutime の合計) */
void tfold(int c, int p) {
  add64(&tcul[p], &tcuh[p], tul[c], tuh[c]);
  add64(&tcul[p], &tcuh[p], tcul[c], tcuh[c]);
  add64(&tcsl[p], &tcsh[p], tsl[c], tsh[c]);
  add64(&tcsl[p], &tcsh[p], tcsl[c], tcsh[c]);
}

/* times (153)。out が 0 でなければ struct tms の 4 語を書く。
 * 返すのは起動からの tick 数 */
int sys_times(unsigned out) {
  unsigned nl;
  unsigned nh;
  unsigned dl;
  unsigned dh;
  if (out != 0) {
    st4(out, ticks(tul[depth], tuh[depth]));
    st4(out + 4, ticks(tsl[depth], tsh[depth]));
    st4(out + 8, ticks(tcul[depth], tcuh[depth]));
    st4(out + 12, ticks(tcsl[depth], tcsh[depth]));
  }
  rtcnow(&nl, &nh);
  sub64(nl, nh, tbootl, tbooth, &dl, &dh);
  return (int)ticks(dl, dh);
}
 ...
 * tcc が .o と .a を読むときに使う。標準入出力には適用されない */
...
int sys_statat(int dirfd, unsigned path, unsigned out) {
  ...
  if (cpystr(p, path, PATHMAX + 1) < 0) return 0 - E2BIG;
  i = sfsfind(p);
  if (i < 0) return 0 - ENOENT;
  e = ent(i);
...
/* statat の 4 語に**表の項目番号**を 1 語足して out へ書く (第 26 世代)。
 *
 * out の配置 (u32 5 語):
 *   0  長さ (バイト)
 *   4  更新時刻 (ナノ秒) の下位 32 bit
 *   8  同 上位 32 bit
 *   12 種別 (1 = ディレクトリ, 2 = ふつうのファイル)
 *   16 **表の項目番号 + 1** (呼び手が st_ino に使う)
 *
 * **+1 して配る。** 根の項目番号は 0 で，0 を「番号が無い」の意で
 * 使う呼び手がある。1 から始めれば，どのファイルの番号も 0 にならない。
 *
 * 項目番号は sfs の 1 つの巻の中で一意であり，名前を変えても追い続け
 * られる。**同じファイルか**の判定はこれで足りる (fdmatch / getpwd)。
 *
 * 許可・所有者は 79 と同じく返さない。持っていないものは持っていない */
int sys_statat2(int dirfd, unsigned path, unsigned out) {
  char p[PATHMAX + 1];
  int i;
  unsigned e;
  (void)dirfd;                          /* 常に AT_FDCWD 相当 */
...
/* ファイルを消す。項目を未使用に戻すだけで，データの空き領域は詰めない
 ...
 * データの空き領域は詰めない (docs/stage016-os.md 9.4 の D1) */
int freeent(int i) {
  unsigned e;
  e = ent(i);
  st4(e + E_FLAG, 0);
...
int sys_unlinkat(int dirfd, unsigned path, int flags) {
  ...
  e = ent(i);
  /* ディレクトリは中身が残るので消さない。空かどうかを見て許す方式も
   * あるが，要るまで作らない (要らない機能は bad の原因になる) */
 ...
 * 本体や spawn の退避領域——を上書きできた (#49)。載せる前にここで
...
int elfok(int i) {
  ...
  for (k = 0; k < pnum; k = k + 1) {
    ...
    if (pva < UBASE) return 0;                  /* ロード先が下に外れる */
    if (pva > UBRKMAX - msz) return 0;          /* ロード先が上に外れる */
  }
  return 1;
}
 ...
 * +304 からイメージ [UBASE, ubrk) とフレームスタック [sp, USP) の複写 */
/* 記録が 5 語 (err つき) かどうかを保持する。spawn2 (501) だけが立てる */
static int spawnerr;
...
int sys_spawn(unsigned sa) {
  ...
  if (cpystr(path, ld4(sa), PATHMAX + 1) < 0) return 0 - E2BIG;
  ci = sfsfind(path);
  ...
  p = ld4(sa + 8);
  if (p != 0) {
    if (cpystr(name, p, PATHMAX + 1) < 0) return 0 - E2BIG;
    ie = sfsfind(name);
...
int ktrap(void) {
  ...
  /* 入った時刻。前回出てからの時間はこのプロセスが U モードで使った */
  d0 = depth;
  rtcnow(&el, &eh);
  sub64(el, eh, tlastl, tlasth, &dl, &dh);
  add64(&tul[d0], &tuh[d0], dl, dh);
  n = tf[16];                   /* a7 = 番号 */
  r = 0;
  ...
  else if (n == SYS_STATAT2) r = sys_statat2((int)tf[9], tf[10], tf[11]);
  else if (n == SYS_TIMES) r = sys_times(tf[9]);
  else if (n == SYS_EXIT) {
  ...
  /* 出る時刻。入ってからの時間は**入ったときのプロセス**の stime である
   * (spawn でも exit でも，処理したのは呼んだ側の syscall である) */
  rtcnow(&xl, &xh);
  sub64(xl, xh, el, eh, &dl, &dh);
  add64(&tsl[d0], &tsh[d0], dl, dh);
  if (depth < d0) tfold(d0, depth);     /* 子が終わった */
  else if (depth > d0) tclear(depth);   /* 子が始まった */
  tlastl = xl;
  tlasth = xh;
  return 0;
}
...
int main(void) {
  ...
  depth = 0;
  savecur = SAVEA;
  /* 時間の計上 (第 27 世代)。全段を 0 にし，起動時刻を記録する。
   * 大域変数が 0 で始まることには頼らない */
  for (i = 0; i <= MAXDEP; i++) tclear(i);
  rtcnow(&tbootl, &tbooth);
  ...
  tf[31] = entry;                       /* mepc */
  /* ここから先が最初のプロセスの utime になる */
  rtcnow(&tlastl, &tlasth);
  urun();
  return 0;
}
