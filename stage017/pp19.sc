...
char src[4194304];         ///< 標準入力の全体 (EOT 0x04 まで)
int srcn;                 ///< src の有効長

char xb[4194304];          ///< アリーナ 1: フレームが参照するテキスト
...
char mcname[262144];      ///< マクロ名 (64 バイトスロット x 4096。pp15 で 4 倍)
int mcfun[4096];           ///< 1 = 関数形式
int mcnp[4096];            ///< 仮引数の個数 (可変長なら __VA_ARGS__ を含む)
int mcvar[4096];           ///< 1 = 可変長 (最後の仮引数が __VA_ARGS__)
int mcbo[4096];            ///< 本体の位置 (mbody 内)
int mcbl[4096];            ///< 本体の長さ
int mcpo[4096];            ///< 仮引数名の位置 (mprm 内。64 バイトスロットが mcnp 個)
int mcon[4096];            ///< 1 = 現在定義されている
int mccnt;                ///< スロット数

char mbody[262144];       ///< マクロ本体の実体
int mbp;                  ///< mbody の有効長
char mprm[131072];        ///< 仮引数名の実体 (64 バイトスロット。pp15 で 4 倍)
int mpp;                  ///< mprm の有効長
...
char ebuf[64];            ///< #if の式の中で読んだ識別子 (defined の判定)
...
///       63 バイトを超える名前は mfind が持つ枠 (64 バイト) にそもそも
...
int loadinc(char *path, char *name) {
  ...
  while (n > 0) {
    if (srcn >= 4194304) exit(6);
    n = sys_read(fd, src + srcn, 4194304 - srcn);
