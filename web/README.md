...
ことが読めないために置いた。コンパイラ側 (琥珀) と実行環境側 (菫) を
塗り分けてあるので，Stage 15 から 16 で律速が移ったことがそのまま
見える。**横軸が世代であるものは 1 枚にまとめてある** —— 成果物の
大きさも同じ格子の 1 行であり，世代セレクタの直下に置いて，Stage の
本文より前で全体像が読めるようにした。

- 升を押すとその世代でその系統が足したものが下に出る (もう一度で畳む)
- 行の名前を押すと**世代ごとに開く** (kernel12〜kernel18・cc15a〜cc15p・
  ld〜ld16 など)。どの世代が何を足したかが行の中で分かれる
