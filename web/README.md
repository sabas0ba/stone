...
**1 画面に「全容・概要・手を動かす所」が収まることを縦の設計基準にした。**
1180 x 900 で測ると，格子 (16 世代ぶん) と世代の要約に続いて，
プレイグラウンドの Run (y=845) かターミナルの Boot (y=878) が折り返しの
内側に入る。そのために次のようにしてある。

- 実行の釦を入力欄の**上**へ置く (説明はその下)。ターミナルも同様で，
  筋書きの切替えと Boot を同じ行に並べる
- 世代の見出しは 1 行に畳み，`reads` / `written in` / `built by` は
  1 行で省略して title で全文を見せる
- 新機能のチップ (`new_capabilities`) は畳んだ箱に入れる。**中身は
  能力の推移の格子と重なる**ので，既定で開いておく理由が無い
