/* mk20host.c --- mk20 をホストで走らせるための身代わり。
 *
 * mk17 のときの mkhost.c と同じ考えで，**mk20.c そのものを**取り込む
 * (mk20.c の側に検査用の分岐を入れない)。違いは 2 つある。
 *
 *   1. spawn2 を system() へ回す (mkhost.c と同じ)
 *   2. **libc19 の stat / newer をホストの本物から作る**
 *
 * 2 が要るのは，mk20 が sfs3 の時刻を見るからである。ホストには
 ...
 *
 * mk20.c の中の #include は，先にここで取り込んでおけば何もしない。
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
