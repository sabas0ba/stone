...
#
# 第 1 部は**コンパイラをコマンドとして持つ**ことである。 stone の OS の
# 上で `cc -o out in.c` と書けば動くものを作った。見るのは
#
#   1. 駆動役が pp16 -> cc15p -> ld16 を通して実行形式を出すこと
#   2. 出た実行形式が stone の OS の上で正しく走ること
...
# stone の OS の根を組む。**/lib に置くのは 1 揃いだけである。**
...
# **stone の OS が，複数のファイルからなるプログラムを書庫経由でビルドする** (7.6)。
#
...
# **stone の OS が作った書庫を，本物の ar が読めるか** (7.3)。
# 相互運用を見る。バイト一致は狙わない
...
# **第 4 部の 2 の完了条件である。**
#
# stone の OS の上で mk を 2 度走らせ，2 度目が何も作らないこと。そのあと
...
if [ ! -d docs/external/tcc ]; then
    ...
    # **stone の OS の上で作った tccdefs_.h** がホストのものと一致すること。
    #
...
# **stone の OS の上で stone の処理系が** zlib / bzip2 を訳し，我々の ar が書庫に
