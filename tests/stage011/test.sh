...
# ヘッダ一式と翻訳単位をバンドルして pp -> cc -> ld -> 実行し，expected/ と照合する。
# バンドルには全ヘッダを入れてよい (#include されたものだけが取り込まれる)。
...
runcase() {
    ...
    # 1 度目は打ち切り，2 度目は単なる FAIL で**何が違ったのか出て
    # いなかった**。stable_cmp なら
    #
    #   2 度とも作れない -> died   (打ち切りもここに分類される)
    #   2 度作って同じ   -> differ (退行。素性を出してすぐ FAIL とする)
    #   2 度作って違う   -> flaky  (環境。やり直す)
    #
    # と分かれ，どれであっても素性が出る。
    #
    # 標準入力を使う段があるので，やり直しは**手順の先頭から**でなければ
    # ならない。だから手順全体を 1 つの関数にする
