...
#
# QEMU を通す比較の結果が**同じ入力・同じツールでも変動する**ことがある
# (docs/dev-notes.md 1.6)。1 回の不一致で「ビルドチェーンが壊れた」と報告するのは
...
# 個別にこれを呼ぶと，通し実行ではブートストラップのビルドチェーンを Stage の数だけ
# 作り直すことになる (実測で CI の 2 割ほどがこの重複だった)。
# 単体で走らせたときは従来どおりその Stage までをビルドする。
# ---- 一致の検査: 「中身が違う」と「実行が再現していない」を分ける ----
#
# **同じ入力・同じツールでも結果が変動する環境がある** (docs/dev-notes.md 1.6)。
# ビット一致の検査が落ちたとき，落ちたのが
#
#   * 中身が違う (退行)          -> 直すべきもの。すぐ出す
#   * 実行が再現していない (環境) -> 比較の前提そのものが崩れている
#
# のどちらなのかを**区別して出す**。PASS に見せるための再試行ではない ——
# 区別が付かないまま FAIL にすると，本物の退行を見落とす。逆に，区別が
# 付かないまま黙って通すのはもっと悪い。
#
...
# 素性を 1 行で出す。存在しなければその旨を出す (空を返さない)
stable_id() {
    if [ -e "$1" ]; then
