...
/* まだ直っていない形: 多次元の char 配列。ginit() が平らに畳んだ後は
 * 要素型が char になっていて「配列」として見えないので，ginit1() の
 * 判断が効かない。tcc は使わないので世代を刻んでいない。
 * ここは **今どうなっているかを測って台帳に載せる** ためのものである */
int g(void) {
  static char m[2][8] = { "ab", "cd" };
  return (m[0][0] == 'a' && m[1][0] == 'c') ? 'y' : 'n';
}

int main() { putc(f()); putc(g()); putc('\n'); return 0; }
