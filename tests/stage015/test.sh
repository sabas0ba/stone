...
cc=tmp/build/cc15ab.bin  # 台帳は最前線の世代で測る
pp=tmp/build/pp.bin
ld=tmp/build/ld.bin
...
[ "$ok" -eq 0 ]
report $? "build: cc15a..cc15ab と pp15 / pp16 / ld16 / ld17 の SHA-256 が各 .md 記載値と一致"
...
[ "$ok" -eq 0 ]
report $? "regress: cc15ab が既存のソース (sh / ed / mk) を cc10l と同じ .o にする"
