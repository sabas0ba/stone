...
cc=tmp/build/cc15f.bin   # 台帳は最前線の世代で測る
pp=tmp/build/pp.bin
ld=tmp/build/ld.bin
...
for pair in cc15a.bin:stage015/cc15a.md cc15b.bin:stage015/cc15b.md \
        ...
        cc15e.bin:stage015/cc15e.md cc15f.bin:stage015/cc15f.md; do
    want=$(grep -Eo '^SHA-256: [0-9a-f]{64}' "${pair##*:}" | cut -d' ' -f2)
    got=$(sha256sum "tmp/build/${pair%%:*}"); got=${got%% *}
    [ -n "$want" ] && [ "$want" = "$got" ] || ok=1
done
[ "$ok" -eq 0 ]
report $? "build: cc15a..cc15f の SHA-256 が各 .md 記載値と一致"

{ cat stage015/cc15f.sc; printf '\004'; } \
    | sh tools/env.sh qemu tmp/build/cc15f.bin > tmp/s15/b3.o \
    && { cat tmp/s15/b3.o; printf '\0'; } \
        | sh tools/env.sh qemu "$ld" > tmp/s15/b3.bin \
    && cmp -s tmp/s15/b3.bin tmp/build/cc15f.bin
report $? "fixpoint: cc15f が自分自身を再生成する (B2 == B3)"
...
[ "$ok" -eq 0 ]
report $? "regress: cc15f が既存のソース (sh / ed / mk) を cc10l と同じ .o にする"
