...
cc=tmp/build/cc15s.bin   # 台帳は最前線の世代で測る
pp=tmp/build/pp.bin
ld=tmp/build/ld.bin
...
[ "$ok" -eq 0 ]
report $? "build: cc15a..cc15s と pp15 / pp16 / ld16 の SHA-256 が各 .md 記載値と一致"
...
[ "$ok" -eq 0 ]
report $? "regress: cc15s が既存のソース (sh / ed / mk) を cc10l と同じ .o にする"
