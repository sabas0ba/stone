...
cc=tmp/build/cc14b.bin    # 台帳は最前線の世代で測る (docs/stage014-external.md 5.3)
pp=tmp/build/pp.bin
ld=tmp/build/ld.bin
...
ok=0
for pair in cc14a.bin:stage014/cc14.md cc14b.bin:stage014/cc14b.md; do
    want=$(grep -Eo '^SHA-256: [0-9a-f]{64}' "${pair##*:}" | cut -d' ' -f2)
    got=$(sha256sum "tmp/build/${pair%%:*}"); got=${got%% *}
    [ -n "$want" ] && [ "$want" = "$got" ] || ok=1
done
[ "$ok" -eq 0 ]
report $? "build: cc14a.bin / cc14b.bin の SHA-256 が各 .md 記載値と一致"

# 世代を触ったら必ず固定点を見る (docs/dev-notes.md 3.1)
{ cat stage014/cc14b.sc; printf '\004'; } \
    | sh tools/env.sh qemu tmp/build/cc14b.bin > tmp/s14/b3.o \
    && { cat tmp/s14/b3.o; printf '\0'; } \
        | sh tools/env.sh qemu "$ld" > tmp/s14/b3.bin \
    && cmp -s tmp/s14/b3.bin tmp/build/cc14b.bin
report $? "fixpoint: cc14b が自分自身を再生成する (B2 == B3)"
...
[ "$ok" -eq 0 ]
report $? "regress: cc14b が既存のソース (sh / ed / mk) を cc10l と同じ .o にする"
