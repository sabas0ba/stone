...
int lexnum() {
  ...
  // 接尾辞 (u U l L) を読み捨てる。長さの区別は付けない (long は int と同じ幅)
  while (getch() == 'u' || getch() == 'U' || getch() == 'l' || getch() == 'L') adv();
  tok = t_num;
  return 0;
}
...
int ptype() {
  ...
  if (tok == k_void) { next(); k = t_void; }
    ...
    // long long は 64 bit の型であり，本処理系には無い。
    // 32 bit として通すと静かに値が壊れるので，ここで拒む
    // (docs/stage014-external.md 4.2)
    if (lng > 1) exit(2);
    if (sht) {
      if (uns) k = t_ushort;
      else k = t_short;
...
int plocal() {
  ...
  while (1) {
    ...
    lsz[i] = tsize(b);
    if (sta) {
      // 関数内 static。実体はフレームではなく .bss に取り，呼出しを
      // またいで値が残る。初期化子は未対応 (通すと初期値が付かないまま
      // 静かに壊れるので拒む。.bss なので値は 0 で始まる)
      if (tok == o_asn) exit(1);
      if (isarr(b)) larr[i] = 1;
      else larr[i] = 0;
      g = slocnew(i);
