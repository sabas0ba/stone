...
#      tmp/build の pp16.bin / cc15p.bin / ld16.bin は**フラットイメージ**で
#      あり，QEMU に -bios で直に置いて走らせる形をしている。OS の
#      上の実行形式にするには ELF (前置部 'E') でリンクし直す必要が
#      ある。Stage 13 の pp13cmd / cc13cmd / ld13cmd と同じ方法である
#      (docs/stage013-tools.md 7 章)。
#
#   2. cc17
#      ドライバ。上の 3 つを順に呼ぶ。libc の第 18 世代とリンクする。
...
build_stage017() {
    ...
    # tcc の作業領域で使うコンパイラ。cc15p は静的な初期化子の文字列を壊すので
    # (docs/stage017-cc.md 24〜28 章)，tcc を正しく組むには cc15q が要る。
    # ビルドチェーンのプログラムは今までどおり cc15pcmd で組む
    ...
    # **最前線を 2 つに分けない** —— 台帳を測るコンパイラと tcc を組むコンパイラは
    # 同じものにする
    cmdlink cc15scmd cc15s
    # 第 20 世代。宣言指定子の途中に来る型修飾子 (unsigned const char)。
    # zlib を stone の処理系で訳して判明した不足で，これが最前線である
    ...
    # 第 28 世代。GCC 4.7.4 の 70 単位を測って出た 6 つに対応した世代
    # (docs/stage017-gcc.md 8.3)。**これが最前線である。**
    # 検査の第 7 部が OS の上で使う
    cmdlink cc15abcmd cc15ab
    cmdlink ld16cmd ld16
    # 未定義シンボルの名前を言うリンカ (5.1)。cc19 はリンカのパスを
    # "/bin/ld16" と埋め込んでいるので、置くときは名前を ld16 にする
    ...
    # 第 2 部。cc17 は記録対象なので書き換えず新しい世代を作る
    # (roadmap.md 4 章)。ar17 は新しいツールなので第 1 世代
    step cc18 cc18 \
    ...
    # 翻訳は libc18 と同じ cc15k で行う (コンパイラの都合。stage016.sh と揃える)
    ...
    # make の第 18 世代 (第 4 部の 2)。**libc19 とリンクする** (stat が要る)
    step mk18 mk18 \
    ...
    #
    # **ヘッダも入力に数える。** libc*_run は include/*.h をバンドルしてから
    # 翻訳するので，ヘッダを直したら .o が変わりうる。ここに書かないと
    # 外側の stage のスタンプだけが変わって build_stage017 が走り，中の step は
    # 「前と同じ」と判定して**古い .o を再利用したまま新しいスタンプが書かれる**。
    # 数えるのはバンドルしているものと同じ並びにすること
    for f in src/string src/ctype src/stdlib src/morecore src/misc15 \
             posix/sys posix/morecore posix/stdio posix/assert posix/dir; do
        n=$(echo "$f" | tr / _)
    ...
    # ソースを読んで判明した不足 —— sys/types.h・signal.h・memchr・strerror・
    ...
    # ビルドチェーンの前置部の 1 引数 putc が優先されるので，pp の段で fputc へ書き換える。
    ...
    # —— printf のフラグ・精度・%o / %X、strtoul の空白と endptr、
    ...
    #
    # **4 つとも本当の値である** —— ダミー値は 1 つも無い。持っていない欄
    ...
    # 元は別の branch で作られた世代で、あちらの Stage 18 のコンパイラで組まれていたが、
    # ツールの側はそのコンパイラを要求していない (どれも C89 の内側で書かれている)
    ...
    # 正規表現機構 (5.6)。sed2 と sh3 が共有する。**複製を 2 つ持たない**
    step re1 re1.o \
    ...
    # 使うツール (tr / expr / basename / dirname / wc / sort / touch /
    # chmod) を足したもの
    step sh3 sh3 \
    ...
    # グループの中へバックトラックできる。awk はこれが無いと書けない
    step re2 re2.o \
    ...
    # awk の数と書式 (5.8)。**翻訳単位を分けてある** —— awk はこのビルドチェーンで
    ...
    # awk の第 1 世代 (5.8)。configure が使うツールの最後の 1 つで、
    # re2 の ERE を使う
    step awk1 awk1 \
    ...
    # 前処理器の第 17 世代 (第 3 部の 3 の 2)。-I を探索経路として持つ。
    # **libc をリンクしない** —— sys_* は 'E' 前置部のものを直に呼ぶ
    ...
    # 時刻を読む検査用のプログラム (第 4 部の 1)。**libc19 とリンクする**
    step stamp stamp \
    ...
    # 4 語の領域を渡している既存の実行形式を壊さないため、5 語を書く
    # 新しい約束は新しい番号で配る
...
# ツール間で共有する部品を 1 つ .o にする (re1 / re2 / awkfmt1)
osobj23_run() {
    sh tools/bundle.sh stage017/libc23/include/*.h \
        "sys/time.h=stage017/libc23/include/sys/time.h" \
        "sys/stat.h=stage017/libc23/include/sys/stat.h" \
...
# OS の上で動く実行形式を 1 つ作る。**libc19 とリンクする** 版
osprog19_run() {
    sh tools/bundle.sh stage017/libc19/include/*.h \
        "sys/stat.h=stage017/libc19/include/sys/stat.h" \
        "$2" \
...
# pp17 は .sc なので前処理を通さない (tool1 と同じ方式)。ただし 'E' で
...
# tests/stage017 の /lib が食い違うと，ドライバが組んだものだけ
