#!/bin/sh
# 我々の sed とホストの sed に**同じ台本と同じ入力**を与え，出力を
# 突き合わせる (docs/stage017-gcc.md 5.5)。
#
#   sh tools/diffsed.sh        ホストでビルドした我々の sed と突き合わせる
#   sh tools/diffsed.sh os     **我々の OS の上で走らせた** sed と突き合わせる
#
# ## なぜ要るか
#
# `sed` の値は「我々が正しいと思う値」では測れない。POSIX の文言は
# 短く，実際の振舞い (空に合う置換の進み方・グループの記録方法・番地の範囲の
...
# **グループの中へバックトラックする形**と**選択の最長一致**がここに要る (5.7)
...
# グループの中へバックトラックする形。**re1 はここで一致に失敗していた** —— グループが貪欲に
# 一致したあと，続きが一致しなくても一致の仕方を試し直せなかった (5.7)
mkcase in3.txt 0 's/\(a*\)ab/[\1]/'
mkcase in3.txt 0 's/\(a*\)\(ab\)/<\1|\2>/'
# 台本の中でグループを 2 度使う。**re1 は 2 つめの \1 が別のグループを指していた**
...
if [ "$mode" = host ]; then
    ...
    # 何を測ったかが変わる
    if [ -z "${STONE_SED:-}" ]; then
        "${CC:-gcc}" -w -o "$OURS" stage017/sed3.c stage017/re2.c \
    ...
    # ---- 我々の OS の上で走らせる ----
    #
    # 起動は 1 回だけ。1 つのイメージに台本と入力を詰め，シェル (sh2) に
    # 順に起動させて `@@名前` の行で切り分ける (tools/diff17.sh と同じ方法)
