...
#   STONE_QEMU_RAM    RAM 量 (既定 128M)。RAMFILE を使うときだけ有効。
...
# 始める (-device loader)。フラットイメージは従来どおり -bios で 0x8000_0000 へ置く。
#
# ビルドチェーンの成果物はすべてフラットイメージなので，この分岐を通るのは第 5 部で tcc に
# 出力させたものだけである (docs/stage015-riscv32.md)。tcc の出す ELF は
# 節ごとに 0x1000 境界へ揃うため，そのままフラットに変換すると番地が合わない。
...
#
# 2026-08-23 の CI で，1 つの実行が停止したまま 115 分走り続け，
# ジョブが 120 分の上限で cancelled になった。cancelled ではキャッシュが
# 保存されないので，次の回もコールドビルドから始まる。**停止が 1 つ
# あるだけで CI が前に進まなくなる。**
#
# 上限は「正常な実行が収まる長さ」ではなく「停止したと判る長さ」で
