#!/bin/sh
# sfs4 イメージとディレクトリの相互変換 (ホスト側のツール)。
...
# 出す。**GCC のツリーは 81,356 項目あり，この shell 実装では pack に約 50 分かかる。**
...
pack() {
    ...
    #
    # **深さのソートキーは 0 詰めにする。** 詰めずに並べると文字列として比べられ，
    # "10" が "2" より前に来る。深さ 10 以上のツリーで親より先に子が出て，
    # pack が "parent not found" で落ちる (GCC 4.7.4 のツリーは深さ 12)。
    # tcc のツリーは深さ 6 だったので，これまで表に出ていなかった
    list=$(mktemp)
...
# ナノ秒を touch -d が受け付ける形 (@秒.ナノ秒) にする
stamp() {
    _ns=$1
...
unpack() {
    ...
    check_magic "$img"
    mkdir -p "$dir"
    # ディレクトリの時刻は中身を作った後で設定し直す。先に設定すると
    # 子を作った時点で上書きされる
    dirs=$(mktemp)
    ...
    # その場で書き換え，awk は $1 を変更すると $0 を OFS (既定は空白) で
    # 組み立て直す。区切りの TAB が空白に変換されて，経路と時刻が 1 つの
