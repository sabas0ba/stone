...
# (本ファイル 1 枚に全 Stage の手順が入っていた頃は，どこを変更しても
...
#
# **これを入れたのは，失敗したビルドが成功として記録される事故が 3 度
...
# step がそれを正しい生成物としてスタンプを書く。次回は「できている」と
# 誤認して先へ進む。
#
# 各 *_run に set -e を追加していくより，**出来上がりを見る**ほうが確実で
# ある。このビルドチェーンの生成物に 0 バイトが正しい場面は無い。
#
# **キャッシュの経路でも見る。** スタンプは「0 バイトの SHA-256」とも一致して
# しまうので，一度 0 バイトのスタンプが書かれると sha256sum -c は通ってしまう。
...
# ---- 成果物ごとのスタンプ (step) ----
#
# run_stage は Stage 単位なので，**途中で強制終了されるとその Stage の頭から
# やり直し**になる。Stage 15 は 30 分を超えるので，これが実務上いちばん
# 影響が大きい (コンテナの再起動・CI のタイムアウト・Ctrl-C のどれでも起きる)。
#
# step は成果物 1 つ (あるいは 1 世代) ごとにスタンプを持ち，既にできて
# いるものを省略する。**強制終了されても失うのは高々 1 世代 (2〜3 分) である。**
# run_stage の外側のスタンプはそのまま残る。全部できていれば外側で
# 一度に省略されるので，キャッシュが有効な回の費用は変わらない。
#
#   step <名前> <生成物...> -- <入力...> -- <コマンド...>
step() {
    _name=$1; shift
    ...
    # **バンドル処理も入力である。** ほとんどの手順は tools/bundle.sh を通して
    # から翻訳するので，bundle.sh を直せば出るものが変わりうる。
    # 各 step の入力欄に書き忘れると，Stage のスタンプだけが変わって
    # build_stageNNN は走るが，中の step は「前と同じ」と判定して
    # **古い生成物に新しいスタンプが書かれる**。書き忘れを 20 か所で個別に防ぐのを
...
# cc の世代を 1 つ作る (2 段。1 段目で生成したコンパイラで自分自身を作り直す)。
...
# ツールを 1 つ作る (1 段。pp / ld のように自分自身では作らないもの)。
...
# stage010 までは 1 本のビルドチェーン (各段が直前の段の成果物を使う) なので順に作る
...
# の連鎖と stage013・stage014 は，いずれも stage010 までの成果物とスタンプに
