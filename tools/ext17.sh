...
# ここは **自作 OS の上で自作の器 (cc19 + cc15t) が**訳す。それが一段先の
# 指標になる。
#
...
do_root() {
    for f in pp16cmd pp17 cc15tcmd ld17cmd sh2.bin cc19 ar17 kernel24.bin; do
    ...
    # 凍結世代なので名前は変えられない。中身は最前線の cc15t を置く
    cp tmp/build/cc15tcmd "$root/bin/cc15p"
