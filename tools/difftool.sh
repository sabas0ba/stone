#!/bin/sh
# 我々の OS のツール (sh3 の組込み) を，ホストの同名のツールと突き合わせる
...
# なく，空の値を取得して進む** —— それがいちばん悪い。
#
# だから「動いた」ではなく「**同じ値が出た**」で測る。基準はホストの
# ツールで，我々が期待値を書かない (5.3 / 5.5 と同じ方針)。
#
# ## 引用は避ける
#
# 命令の並びはシェルを通るので，我々のシェルとホストのシェルで引用の
# 扱いが違えば，ツールの差ではないものが差として出る。**引用の要らない
# 形だけ**を並べてある。
set -u
...
# エスケープしない `*` は**両方のシェルが経路展開してしまう** (sh5 が glob を
# 持ったので，いまはどちらも同じ振舞いである。5.9)。展開そのものは
# tools/diffglob.sh で測るので，ここではツールの差だけを見る
cat > "$out/cmds" <<'EOF'
grep ab*c t1.txt
grep ^ac t1.txt
grep -v ^# t1.txt
