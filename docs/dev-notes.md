...
- **実引数の「評価順」と「積む順」を分けると，コード生成規則が変わる。**
  ...
  固定点 (B2 == B3) で健全性を確かめる。
- **「型エラーで撥ねる」より「静かに値が壊れる」ほうが悪い。** 構造体の代入と
...
- **「仕様が揃った」は目次を追うだけでは確かめられない。** Stage 10 は C89 の
  文法を章立てで潰したが，字句の細部 (文字エスケープ) に穴が残っていた。
  見つかったのは Stage 11 の libc を書き始めて `isspace` が `\f` `\r` `\v` を
  必要としたときである。**最初の実利用者が穴を踏む。** 利用側を書いてみる
  までは「揃った」と言い切らないほうがよい。
  なお，利用側 (libc) で数値を直書きして回避してはいけない。処理系の穴を
  利用者に押しつけることになり，しかも理由が読む側に伝わらない。
- **未 commit の作業がある状態で `git checkout -B` してはいけない。** マージ済みの
  PR の後、次の作業のためにブランチを最新の main へ張り直す
  (`git checkout -B <branch> origin/main`) のは定石だが，**作業ツリーの未 commit の
  変更はそこで消える**。Stage 11 の実装一式 (ヘッダ 4 枚・`lib/` 2 本・ビルド組込み・
  テスト) を，検証待ちのまま張り直して失った。
  張り直す前に `git status --porcelain` が空であることを確かめる。空でなければ
  commit するか，捨ててよいと確認してから張り直す。
- **進捗の確認に `pgrep -f <名前>` を使うときは，その名前が実在することを先に確かめる。**
  存在しないスクリプト名で `pgrep -f` すると，親プロセスのコマンドラインに部分一致して
  「実行中」と誤検出することがある。上の件では，消えたファイルのビルドを延々と
  待っているつもりになっていた。**出力ファイルが育っているか**で見るほうが確実である。
