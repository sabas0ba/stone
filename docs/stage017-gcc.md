...
窓と同じ大きさ (512 MiB) のsfs4に全配布木を詰め、magic・使用量・経路の数を
...
経路の数は詰めたimageを`list`で読み直して数える。書く側と読む側の両方を
通さないと、親の索引付けの誤りが表に出ない。

**CIでは回さない。** `tools/sfs4.sh`のpackは項目ごとに`dd`と`od`を呼ぶPOSIX
shellであり、81,356項目では数時間かかる。手で測るための手順として置く。進行は
`SFS4_PROGRESS`が1000項目ごとにstderrへ出す。
...
**全配布木の実packはこの表に無い。** 7.5のとおり数時間かかるためCIでは
回さない。roadmap 6.1の4へ進む前に手で1度通す。
