# Stage 17 第 5 部: GCC のビルド方針と対応状況 (roadmap 2.1)

## 0. 本書の対象と初版からの訂正

本書の初版では roadmap の 3 案をすべて「tcc で GCC をコンパイルする案」と
誤記していた。3 案が比較しているのは，stone の処理系に追加する機能である。
利用者の指摘に基づき訂正した (経緯は
[stage017-cc.md](stage017-cc.md) 34 章)。

L1 の目標は，stone の処理系で GCC をコンパイルできることである。
tcc・GCC・Linux のソースは，stone の処理系の対応機能と処理容量を検証する
入力として使い，stone の標準処理系や実行環境には採用しない
([roadmap.md](roadmap.md) 71 行 / [artifacts.md](artifacts.md) 3 章)。

## 1. 方針を判断する前提条件

roadmap 2.1 は 3 案の判断を「tcc の `Makefile` を stone の OS の上で
...
## 2. stone の処理系で確認済みの動作

| | 中身 | 出典 |
|---|---|---|
| stone の処理系が tcc を訳せる | 我々の cc が tcc を訳し，出た tcc が自分自身を訳し，答が参照実装とバイト一致する (T2 == T3 == tccH) | roadmap 3 (Stage 15) |
| stone の OS の上で訳せる | 上流の `Makefile` を我々の `mk20` が回し，我々の `cc19` が全単位を訳す。出た `tcc` は手で命令を並べたものとバイト一致 | 21 章 |
| 書庫まで作れる | `lib/Makefile` が `lib 0` で終わり，員 10 本の `libtcc1.a` が出る | 28〜29 章 |
| 測ると stone の未実装機能・不具合が出る | tcc のソースを読ませたことで C 適合の誤りが 4 つ出た (`cc15q` / `cc15r` / `cc15s`)。さらに zlib / bzip2 で 2 つ (`cc15t` / `cc15u`)。Stage 15 の台帳から「通るが誤り」が 0 になった | 25 / 28 / 31 / 33 章 / 5.1 |
| 実物が組めて走る | zlib 1.3.1 と bzip2 1.0.8 を stone の OS の上で stone の処理系が訳し，我々の ar が書庫にし，我々の ld が繋ぎ，走らせて往復した。値はホストの gcc と一致 | 5.1 |
| libc の未実装機能・不具合も出る | `sys/types.h` / `signal.h` / `memchr` が無く，`strerror` と `lseek` は実体があるのに宣言が無かった。`fopen(path,"a")` は先頭を潰していた | 32.3 (`libc21`) |
| OS の側 | 木を持つファイル系 (sfs3。時刻つき)・255 MiB の記憶域・POSIX 部分集合のシェル (`sh2`)・組込みの道具 10 個・差分ビルドする `mk20` | stage016-os.md / 11 章 |

stone の処理系は，上流の `Makefile` を使って tcc をビルドできる。
外部プロジェクトのビルド検証では，既存のテストで検出できなかった未対応機能や
不具合も確認できている。

## 3. 確認済みの条件に基づく 3 案の比較
...
| 案 | stone 側で実装する機能 | 利点と保守上の課題 |
|---|---|---|
| A | C++ の部分集合をコンパイルする処理系 | RISC-V を維持できる。C に比べ，実装する言語機能が多い |
| B | **stone の処理系の x86-64 backend** | 実績のある経路。ただし RV32 が主対象でなくなる |
| C | **GCC 4.7 に載せる RISC-V backend** (我々が書く) | GCC 本体の fork を保守することになる |

**どの案でも，GCC を訳すのは stone の処理系である。** 案 C で「GCC 4.7 は C
だから訳しやすい」と言えるのは，**stone の処理系が C を訳せるから**であって，
他所に C 処理系があるからではない。
...
> **stone の処理系 (`cc19` + `cc15u`) が GCC 4.7 の C を訳せるか。**

tcc のビルド成功により，tcc が使う C99 機能への対応は確認できた。
ただし GCC 4.7 が使う構文や処理容量の要件は別途検証する必要がある。
4 章ではこれらの未確認項目を整理する。
...
それも C で書かれているので，**stone の処理系が訳せれば**stone のビルドチェーンの成果物に
なる。tcc を経由する必要はない。

ただし前提が 2 つある。

1. **stone の処理系が GCC 4.7 の C を訳せること** (3.3)
...
### 4.1 stone の処理系が GCC 4.7 の C を訳せるか

**これが最初の 1 つである。** tcc のときと同じ測り方ができる ——
ソースを読ませ，**通らなかった単位とその理由を数える**。tcc では
それが「C 適合の誤り 4 つ + libc の未実装機能・不具合 6 つ」という表になった
(31〜33 章)。GCC 4.7 でも同じ形の表が出るはずで，**その表の長さが
案 C の見積りそのもの**になる。

**測った (8 章)。** libiberty と libcpp の 70 単位のうち **56 単位**が
`.o` まで通り，表は「C 言語仕様への未対応・不適合 6 つ + libc の未実装機能・不具合 4 つ + 処理容量の上限 1 つ」に
なった。予想どおり同じ形の表が出ている。**表は測るたびに動く** ——
前処理器の未対応 1 つ (`defined`) は `pp18` で，C 言語仕様への未対応・不適合の 1 つ
(仮引数並びの抽象宣言子) は `cc15w` で埋め，そのたびに次の壁が出た。

### 4.2 GCC 4.7 の `configure` と `Makefile` が stone の OS で回るか

Stage 16 の教訓は「**次のビルドに不足していたのは OS の機能だった**」である。
...
対象の側の `libstdc++` / `libsupc++` とそのヘッダが要る。 stone の OS に
あるのは C の libc だけである。
...
| | 中身 | 大きさの見当 |
|---|---|---|
| 実行時を移す | `libstdc++-v3` を stone の OS 向けに組む | `configure` が大きく，libc に要求するものも多い |
...
判断を待つ間に進められるもの。**どれも主語は stone の処理系である。**

### 5.1 実物を， stone の処理系で， stone の OS の上で組む —— **済**

`zlib` 1.3.1 (15 単位) と `bzip2` 1.0.8 (7 単位) を，**stone の OS の上で
stone の処理系が**訳し，**我々の `ar` が書庫にまとめ**，**我々の `ld` が繋ぎ**，
...
| 出たもの | 中身 | 直し |
|---|---|---|
| C 言語仕様への未対応・不適合 | 型修飾子が宣言指定子の列の**途中**に来る形 (`unsigned const char`) を拒む。C89 6.5 は順序を縛らない | `cc15t` |
| **C 適合の誤り** | **複合代入 (`a op= b`) が符号を見ていない。** `unsigned` の `%=` が符号つき剰余に，`int` の `>>=` が論理シフトになる | `cc15u` |
| libc の未実装機能・不具合 | `libc21` を **`.o` にしていなかった** —— ヘッダだけ足して満足していたので，新しく実装したもの (`signal` / `memchr` / `strerror`，`fopen(path,"a")` の直し) はどこにも無かった | `l21_*` を組む |
...
### 5.2 stone の処理系の C 適合を，測って埋め続ける
...
同じソースを stone のビルドチェーンとホストの処理系の両方に訳させ，走らせて標準出力を
突き合わせる。**我々が拒む形**については，
...
| `fpsoft` | ビルドチェーンの内部の名前 (`__dadd` …) を直に呼ぶ |
...
4. ~~GCC 4.7.4のC translation unitを stone toolchainへ入力し、C対応とlibcの不足を記録する。~~ libiberty / libcppは完了 (8章)。70単位のうち42単位が`.o`まで通り、C適合の穴が8つ、libcの穴が3つ、前処理器の未対応が1つ、処理容量の上限が1つ出た。gcc/本体 (362単位) は8.7の1〜2を埋めてから測る。
...
## 8. GCC の翻訳単位を stone の処理系に読ませる (6.1 の 4)
...
> **初回は`stage015/libc`で測ってしまった。** それはベアメタル実行側
...
| `cap` | 処理容量の上限 (ppの束ね64員 / 256員・pp/ccの6) |
...
| 12 | ~~条件式の第2項が空pointer定数 `(k ? 0 : p)->f`~~ | 5 | **[cc15y](../stage015/cc15y.md)で通した** |
| 13 | ~~小数点から始まる浮動小数点定数 `.0001`~~ | 1 | **[cc15aa](../stage015/cc15aa.md)で通した** |
...
9〜13をすべて通した結果 (8.2の5回目)、`charset`と`symtab`が`.o`まで出た。
`lex`と`line-map`は**8 (`putc`) に合流した** —— どちらも
`putc (c, fp)` を呼ぶ。**libcppで残るのはこの3単位 (`mkdeps`を含む) だけ
である。**

> **`where`は塊までしか絞らない。** 10を最初「配列の大きさの定数式に
> `sizeof`」と書いたのは誤りで、実際は**配列型の`typedef`**だった
> ([cc15ab.md](../stage015/cc15ab.md))。`where`が指した塊に
> `typedef char ck[(sizeof(w) == 8 || ...) * 2 - 1];`があり、目についた
> `sizeof`を原因と書いてしまった。定数式の`sizeof`は`cuna()`が前から
> 受けている。
>
> **同じ取り違えを2度している** —— 8.6の`offsetof`も同じ筋である。
> 塊の中のどれが原因かは、**最小の形に落として1つずつ確かめるまで
> 判らない。**
...
したがってこれは stone の未実装機能・不具合ではない。**ただしGCCを組むには通す必要がある** ——
...
### 8.6 処理容量の上限 (`cap` の1単位) と、`offsetof` についての訂正
