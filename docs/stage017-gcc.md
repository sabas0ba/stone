...
### 8.8 別の枝が見つけた 5 つ —— **2 つは今の main に入ったままである**

[PR #93](https://github.com/sabas0ba/stone/pull/93) は差分試験を
**コンパイラ側へ広げ**、プローブ 4 本 (`bitfld` / `funcptr` / `intconv` /
`aggr`) であちらの `cc15ab` に 5 つの穴を見つけている。**そのうち 4 つは
この枝に持ち込んでいないので、今の器に残っている。**
...
5 つめ (関数ポインタの配列) は 8.3 の 7 と同じ形で、こちらでもまだ
`xatexit` 1 単位を塞いでいる。
...
この 4 つと Stage 18 (C89 の外側の 6 世代) は、あちらの枝の
`cc15w`〜`cc15ab` に実装がある。**枝を消す前に持ち込むこと** ——
`cc15ab` の上へ `cc15ac`〜 として derive すればよい。
