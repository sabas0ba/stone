...
### 6.3 libc23 の課題 (測って判った穴)

`awk1` を書く途中で libc22 の穴が 1 つ出た。**awk の側で回避したが，
直す場所は libc である。**

| | 状態 |
|---|---|
| `printf` の `%g` / `%e` | `%f` と同じ扱いになっている。`%.6g` が `3.141590` のようになる |
| `printf` の丸め | 半端を切り上げている (`%.0f` の `2.5` が `3`)。ホストは偶数へ丸める |

どちらも [tools/diff17.sh](../tools/diff17.sh) のプローブに足せば表に
出る形である。`awk1` が持っている変換 (`fmte` / `fmtf` / `fmtg` /
`scaleround`) がそのまま libc23 の下敷きになる。
