/// @file ld12.sc
/// @brief 実行イメージを作るリンカ (Stage 12 世代)。出力形式を 3 つ持つ。
///
/// 設計は docs/stage012-os.md 5.3。stage008/ld.sc を出発点に，
/// **カーネル ('K') と ELF 実行形式 ('E') の出力**を足したものである。
/// 再配置とシンボル解決の仕組みは stage008 から変えていない。
///
...
/// stage008 と同じく「イメージ先頭からのオフセット」で扱い，ロードアドレスは
/// 再配置の値を作る瞬間にだけ足す。'E' では先頭に ELF ヘッダを置くので，
...
/// @brief リンカ本体。標準入力からオブジェクト列を読み，イメージを書き出す。
...
int main() {
  ...
  if (fmt == 'E') { base = 0x86000000; prosz = ehsz + 448; }
