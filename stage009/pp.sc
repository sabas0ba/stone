...
///
/// @section model 中核となる考え方: 入力フレームのスタック
/// pp の処理はすべて「文字を 1 個引き出す」操作の上に成り立つ。引き出し元は
/// 単一のソースではなく，フレームのスタックである。底が翻訳単位で，#include は
...
/// 「フレームを積む・降ろす」に還元される。マクロの再帰抑止も，スタックの中に
...
char src[262144];         ///< 標準入力の全体 (EOT 0x04 まで)
int srcn;                 ///< src の有効長

char xb[262144];          ///< アリーナ 1: フレームが参照するテキスト
...
char mbname[2048];        ///< メンバ名 (32 バイトスロット x 64)
...
char mcname[16384];       ///< マクロ名 (32 バイトスロット x 512)
int mcfun[512];           ///< 1 = 関数形式
int mcnp[512];            ///< 仮引数の個数 (可変長なら __VA_ARGS__ を含む)
int mcvar[512];           ///< 1 = 可変長 (最後の仮引数が __VA_ARGS__)
int mcbo[512];            ///< 本体の位置 (mbody 内)
int mcbl[512];            ///< 本体の長さ
int mcpo[512];            ///< 仮引数名の位置 (mprm 内。32 バイトスロットが mcnp 個)
int mcon[512];            ///< 1 = 現在定義されている
int mccnt;                ///< スロット数

char mbody[131072];       ///< マクロ本体の実体
int mbp;                  ///< mbody の有効長
char mprm[16384];         ///< 仮引数名の実体 (32 バイトスロット)
int mpp;                  ///< mprm の有効長
...
char idn[32];             ///< 走査中に読んだ識別子
int idl;                  ///< その長さ
char nbuf[32];            ///< 名前の作業域 (指令の解析用)
char wbuf[32];            ///< 指令名
...
int mfind(char *nm) {
  ...
  i = 0;
  while (i < mccnt) {
    if (mcon[i] && streq(mcname + i * 32, nm)) return i;
    i = i + 1;
  }
  return 0 - 1;
}
...
int mslot(char *nm) {
  ...
  i = 0;
  while (i < mccnt) {
    if (streq(mcname + i * 32, nm)) return i;
    i = i + 1;
  }
  if (mccnt >= 512) exit(6);
  i = mccnt;
...
int pfind(int m, char *nm) {
  ...
  i = 0;
  while (i < mcnp[m]) {
    if (streq(mprm + mcpo[m] + i * 32, nm)) return i;
    i = i + 1;
  }
  return 0 - 1;
}
...
/// @details 展開結果の再走査もこの関数が行う。フレームのスタックから読むので，
