...
/// 入力言語の基礎は docs/stage005-sc.md 2 章。第 3 部の 1 の cc7.sc を
/// 出発点とし，**可変長引数を入れた** ものである (Stage 10 第 3 部の 2)。
///
/// @section diff7 第 3 部の 1 からの変更点
...
int ecallseq(int e) {
  ...
  if (gvar[e]) {
    ...
    if (n < k) exit(5);
    i = n;
    while (i > k) { i = i - 1; emit(c_arg, av[i], 0); }
    i = 0;
    while (i < k) { emit(c_arg, av[i], 0); i = i + 1; }
  } else {
    if (k >= 0 && k != n) exit(5);
    if (k < 0) gused[e] = 1;
    i = 0;
    while (i < n) { emit(c_arg, av[i], 0); i = i + 1; }
  }
  ety = gty[e];
  elv = 0;
  earr = 0;
