/* kernel.c --- 簡易 OS のカーネル (Stage 13 世代)
 *
 * stage012/kernel.c を出発点に，Stage 13 第 1 部の 3 つを足したものである
...
/* 項目 ei の pos へ n バイト書く。必要なら長さとカーソルを伸ばす */
int sfswrite(int ei, int pos, unsigned buf, int n) {
  ...
  if (pos + n > len) {
    st4(e + E_LEN, (unsigned)(pos + n));
    /* 末尾を伸ばしたのでカーソルも進める (4 バイト境界へ) */
    st4(SFSA + 16, (off + pos + n + 3) / 4 * 4);
  }
  return n;
}
...
int sys_write(int fd, unsigned buf, int n) {
  ...
  if (fd == 1 && fd1ent >= 0) {
    n = sfswrite(fd1ent, fd1pos, buf, n);
    fd1pos = fd1pos + n;
    return n;
  }
  if (fd == 1 || fd == 2) {
  ...
  n = sfswrite(fdent[fd], fdpos[fd], buf, n);
  fdpos[fd] = fdpos[fd] + n;
  return n;
}
...
int sys_spawn(unsigned sa) {
  ...
  /* 親を壊す前に ELF であることを確かめる */
  p = SFSA + ld4(ent(ci) + E_OFF);
  if ((*(char *)p & 255) != 127) return 0 - ENOEXEC;
  if (*(char *)(p + 1) != 'E') return 0 - ENOEXEC;
  ...
  stksz = USP - psp;
  b = savecur;
  if (b + 292 + imgsz + stksz > SAVETOP) return 0 - ENOMEM;
  sbase[depth] = b;
  wcopy(b, TFA, 132);
  for (i = 0; i < NFD; i++) {
