...
/// 入力言語の基礎は docs/stage005-sc.md 2 章。第 3 部の 2 の cc8.sc を
/// 出発点とし，**構造体を値として扱えるようにした** ものである
/// (Stage 10 第 3 部の 3)。
///
/// @section diff8 第 3 部の 2 からの変更点
///   局所の構造体変数   struct S s;
...
/// 求めて 2 回使う」を直接的に書ける (docs/stage010-c89.md 4.2)。
///
...
int nsta;                 ///< ローカル側へ並べ替えた static 記号の数
int gcnt;                 ///< 大域記号の登録数
...
int brkl[64];             ///< break の飛び先ラベル (スタック)
int brkn;                 ///< その深さ
int conl[64];             ///< continue の飛び先ラベル (スタック)
int conn;                 ///< その深さ
...
/// @brief その型が関数型か (ポインタを除いた後の基底で見る)。
int isfn(int t) {
  if ((t >> 16) != 0) return 0;
  if (t < t_fn) return 0;
  return t < t_fn + fncnt;
...
///       値を作る命令は dce が削除するので，ここで何かする必要はない。
int expr() {
  int v;
  v = assign();
  while (tok == o_comma) {
...
// ---- コード出力 ----
// 割付け結果 (vreg) を見ながら IR を 1 命令ずつ命令語へ変換する。
...
int emitins(int i) {
  ...
  if (k == c_call) {
    ...
    a = 4;
    if (gvar[b] && gna[b] >= 0) a = 4 + (ib[i] - gna[b]) * 4;
    outw(iw3(0x13, 9, 9, a));
...
int funcdef() {
  ...
  gty[e] = cty;
  // 構造体の返却は第 3 部の 4 で入れる。ここで撥ねておかないと，
  // 返却値の 1 語だけが返って先頭メンバ以外が壊れる形で静かに通る
  if (isstru(cty)) exit(5);
  lcnt = 0;
  cna = 0;
  cvarg = 0;
  cvaoff = -1;
...
int ginit(int e) {
  ...
  if (isarr(t)) {
    es = aelem[t - t_arr];
    n = acnt[t - t_arr];
    // 多次元は平らに並べる。要素型が配列でなくなるまで平坦化しておくと，
...
///       前置部はリンカの担当になったので，ここでは名前だけを登録する。
int bireg() {
  biadd("getc", 0);
  biadd("putc", 1);
  biadd("exit", 1);
...
///       大域の番号を書く決まりである。static の記号はローカル側へ並べ替える。
