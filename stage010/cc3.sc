...
/// 入力言語の基礎は docs/stage005-sc.md 2 章。第 2 部の 1 の cc2.sc を
/// 出発点とし，**宣言を広げた** ものである (Stage 10 第 2 部の 2)。
///
/// @section diff 第 2 部の 1 からの変更点
...
int plocal() {
  ...
  if (tok == o_lb) {
    ...
    if (lty[i] == 0) frame1((tval + 3) & 0xfffffffc);
    else frame1(tval * 4);
    next();
    if (tok != o_rb) exit(1);
    next();
