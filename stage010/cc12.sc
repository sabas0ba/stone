...
/// (Stage 10 の補遺 2)。
///
/// @section ampfix なぜ補遺 2 が要ったか
/// 式の中の配列は「先頭要素へのポインタ」へ退化し，退化した式は左辺値で
/// なくなるので，&a が一律にエラー 5 で撥ねられていた。C89 では &a は
/// 合法で，値は先頭要素のアドレスと同じ，型が「配列へのポインタ」になる。
/// Stage 11 の stddef.h を書き始めて offsetof が配列メンバに使えない
/// (&(((t *)0)->m) の m が配列だと通らない) ことで気づいた。
///
/// 退化の際に **退化前の配列型を eaty へ控えておき**，& はそれを 1 段
/// ポインタへ包んで返す。値は退化した式そのままでよい (配列のアドレスは
