...
/// 入力言語の基礎は docs/stage005-sc.md 2 章。Stage 8 の cc.sc を出発点とし，
/// **文と式を C89 の水準へ広げた** ものである (Stage 10 第 1 部)。
///
/// @section diff Stage 8 の cc からの変更点
/// 追加したのは構文だけで，IR の命令種別・コード生成規則・ABI・出力形式は
/// 一切変えていない。新しい制御構造はすべて既存のラベルと jmp / bz / bnz の
/// 組合せに帰着し，新しい演算子はすべて既存の二項演算とロード・ストアに帰着する。
///
///   文  for / do-while / switch-case-default / break / continue / goto・ラベル
///   式  ?: / 複合代入 (+= 等) / ++ / -- / カンマ演算子 / sizeof / キャスト
///
/// コード生成を変えていないことは，ブートストラップ 1 段目 (cc8 が作った
/// cc1.bin) と 2 段目 (cc1 が自分自身を再コンパイルした cc.bin) がバイト単位で
/// 一致することで確かめられる (docs/stage010-c89.md 2.2)。
///
...
int istype() {
  return tok == k_int || tok == k_char || tok == k_struct;
}
